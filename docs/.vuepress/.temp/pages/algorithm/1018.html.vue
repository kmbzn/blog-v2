...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>그 뒤에 입력으로 주어지는 체스판 행렬을 받아옵니다. 이 때 계산의 편의를 위해 <code v-pre>B</code>는 <code v-pre>1</code>, <code v-pre>W</code>는 <code v-pre>0</code>이라는 정수 값으로 <code v-pre>list</code>에 저장하겠습니다.</p>
<div class="language-python line-numbers-mode" data-highlighter="prismjs" data-ext="py"><pre v-pre><code class="language-python"><span class="line">arr <span class="token operator">=</span> <span class="token punctuation">[</span><span class="token punctuation">]</span></span>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>실제 문제에서는 <code v-pre>8x8</code>보다 큰 size의 보드가 주어지겠지만, 결국 정사각형을 칠하는 행위는 <code v-pre>8x8</code> 사이즈로 자른 후에 이루어질 것이므로 우선 <code v-pre>8x8</code> 사이즈의 단위 예시를 가지고 문제를 풀어봅니다.</p>
<p>문제에서 언급하였듯이,</p>
<blockquote>
<p>체스판은 검은색과 흰색이 번갈아서 칠해져 있어야 한다.<br>
구체적으로, 각 칸이 검은색과 흰색 중 하나로 색칠되어 있고,<br>
변을 공유하는 두 개의 사각형은 다른 색으로 칠해져 있어야 한다.</p>
</blockquote>
...
<p>이 단위 예시에 대해 풀이하는 함수를 정의합니다.</p>
<div class="language-python line-numbers-mode" data-highlighter="prismjs" data-ext="py"><pre v-pre><code class="language-python"><span class="line"><span class="token keyword">def</span> <span class="token function">check</span><span class="token punctuation">(</span>i<span class="token punctuation">,</span> j<span class="token punctuation">,</span> res<span class="token punctuation">)</span><span class="token punctuation">:</span></span>
...
이 두 값의 차를 모두 더한 값이 해당 단위 체스판에서 지민이가 다시 칠해야 하는 정사각형의 개수가 될 것입니다.</p>
<p>또한 Idea 1에서 언급하였듯이, <code v-pre>r</code>값이 <code v-pre>32</code>보다 크다면, <code v-pre>64</code>에서 <code v-pre>r</code>값을 빼 주기만 하면 됩니다.<br>
앞에서 언급하였듯이, <code v-pre>r</code>과 <code v-pre>(64 - r)</code> 중에서 더 작은 값을 택하기만 하면 되기 때문입니다.</p>
