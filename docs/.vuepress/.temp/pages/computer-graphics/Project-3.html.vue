...
<ul>
<li>LMS 코스 홈 &gt; Weekly Learning (주차별 학습) &gt; Week 11 &gt; Project 3</li>
<li>파일을 zip으로 압축하여 다음과 같은 형식의 파일명을 사용하세요.
<ul>
<li>파일명은 자유롭게 지정해도 됩니다.</li>
</ul>
</li>
</ul>
...
<ul>
<li>
<p>단일 프로그램에서 모든 요구사항을 구현해야 합니다.<br>
이 Project는 별도의 프로그램으로 요구하지 않습니다.</p>
</li>
<li>
<p>창의 해상도는 <code v-pre>(800, 800)</code>일 필요는 없습니다.<br>
...
<ul>
<li>
<p><strong>뷰어 창에 bvh 파일을 드래그 앤 드롭하면 하나의 파일만 로딩되어야 합니다.</strong></p>
<ul>
<li>여러 개를 동시에 드롭해도 첫 번째 파일만 로드되어야 합니다.</li>
</ul>
</li>
<li>
<p><strong>bvh 파일을 읽고 skeleton (정지 포즈)만 렌더링하세요 (10점)</strong></p>
<ol>
<li>
<p>파일을 열 때 캐릭터를 자동으로 애니메이션하지 마세요.</p>
<ul>
<li>skeleton (정지 포즈)만 렌더링하세요.</li>
</ul>
</li>
<li>
<p>HIERARCHY 섹션의 데이터만 사용해서 캐릭터를 그리세요.</p>
<ul>
<li>translation은 <code v-pre>(0,0,0)</code>, rotation은 없도록 (단위 행렬)</li>
</ul>
</li>
<li>
<p><strong>박스 렌더링(box rendering)</strong> 사용</p>
<ul>
<li>점이나 선이 아니라 박스로 렌더링</li>
<li>조명은 Phong shading 사용</li>
<li>&quot;End Site&quot; 링크는 실제 관절이 아니므로 무시하세요</li>
</ul>
</li>
</ol>
</li>
...
<ul>
<li>
<p>스페이스바를 누르면 모션을 재생하세요.</p>
<ul>
<li>각 프레임이 지나갈 때마다 다음 포즈를 박스로 표시</li>
<li>루프마다 프레임을 하나씩만 진행</li>
<li>마지막 프레임이 지나면 다시 첫 프레임부터 반복</li>
<li>박스 렌더링 유지</li>
</ul>
</li>
</ul>
...
<ul>
<li>
<p><code v-pre>T</code> 키를 누르면 OBJ 렌더링 모드 진입</p>
<ol>
<li>
<p>각 링크는 별도의 OBJ 파일로부터 메쉬로 렌더링</p>
<ul>
<li>동일한 OBJ 파일을 반복 사용하면 안됨</li>
</ul>
</li>
<li>
<p><code v-pre>T</code> 키가 눌렸을 때:</p>
<ul>
<li>미리 정의된 BVH 파일 (예: <code v-pre>run.bvh</code>)을 불러오고</li>
<li>각 링크에 해당하는 OBJ 파일로 렌더링</li>
<li>링크 이름과 계층 구조는 BVH마다 다를 수 있음</li>
</ul>
</li>
<li>
<p><code v-pre>.bvh</code>와 <code v-pre>.obj</code> 파일은 Python 소스와 같은 폴더에 두세요</p>
<ul>
<li><code v-pre>os.path.join()</code> 사용 권장</li>
</ul>
</li>
</ol>
</li>
...
<li>
<p>애니메이션되는 skeleton 모델을 캡처한 영상</p>
</li>
<li>
<p><strong>다음 사이트들에서 bvh 파일 다운로드 가능</strong>:</p>
<ul>
<li>http://motion.hahasoha.net/</li>
<li>http://mocap.cs.sfu.ca/</li>
<li>http://dancebci.eu/main/performances</li>
</ul>
</li>
<li>
...
<ul>
<li>
<p><code v-pre>Python 3.8</code>에서 실행 가능해야 하며,<br>
사용 가능한 모듈은 다음으로 제한됨:</p>
<ul>
<li>NumPy</li>
<li>PyOpenGL</li>
<li>glfw</li>
...
<li>
<p>기타 Python 모듈 사용 불가</p>
</li>
<li>
<p><strong>glfw만 이벤트 처리 및 OpenGL context 용도로 사용</strong></p>
</li>
<li>
<p>위 조건을 만족하지 않으면 TA PC에서 실행되지 않으므로<br>
