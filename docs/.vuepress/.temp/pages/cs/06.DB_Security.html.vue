...
<li>데이터 항목과 데이터 항목 그룹 간의 관계를 포함</li>
<li>때로는 보안이 필요한 민감한 데이터를 포함할 수 있음.</li>
<li>데이터베이스 관리 시스템(DBMS, database management system)
<ul>
<li>데이터베이스를 구축하고 유지하기 위한 프로그램 모음</li>
<li>여러 사용자 및 애플리케이션에 임시 query(ad hoc query) 기능을 제공</li>
</ul>
</li>
<li>Query language
<ul>
<li>데이터베이스에 대한 통일된 인터페이스를 제공</li>
</ul>
</li>
</ul>
...
<li>Query Optimizer(쿼리 최적화기)는 SQL query를 효율적인 실행 계획으로 변환함.</li>
...
<li>각 열은 특정 유형의 데이터를 보유</li>
<li>각 행은 각 열에 대한 특정 값을 포함</li>
<li>이상적으로는 모든 값이 고유하여 해당 행의 식별자/key(키)를 형성하는 하나의 열을 가짐.</li>
<li>모든 테이블에 존재하는 고유 식별자에 의해 서로 연결된 여러 테이블의 생성을 가능하게 함.</li>
...
<li><code v-pre>Staff</code> 테이블은 <code v-pre>staffNo</code>(기본 키), <code v-pre>name</code>, <code v-pre>telNo</code>, <code v-pre>posn</code> 열을 가짐.</li>
...
<li>Tuple(튜플), Row(행), Record(레코드)</li>
<li>Attribute(속성), Column(열), Field(필드)</li>
<li>Primary key(기본 키)
<ul>
<li>행을 고유하게 식별</li>
<li>하나 이상의 열 이름으로 구성</li>
</ul>
</li>
<li>Foreign key(외래 키)
<ul>
<li>한 테이블을 다른 테이블의 속성과 연결</li>
</ul>
</li>
<li>View(뷰), Virtual table(가상 테이블)
<ul>
<li>하나 이상의 테이블에서 선택된 행과 열을 반환하는 query의 결과</li>
</ul>
</li>
</ul>
...
<li>관계형 데이터베이스의 데이터를 정의, 조작 및 query하기 위한 표준화된 언어</li>
<li>여러 유사한 버전의 ANSI/ISO 표준 존재</li>
<li>SQL 문은 다음을 위해 사용될 수 있음.
<ul>
<li>테이블 생성</li>
<li>테이블에 데이터 삽입 및 삭제</li>
<li>뷰 생성</li>
<li>query문을 사용하여 데이터 검색</li>
</ul>
</li>
</ul>
...
<li>데이터베이스는 신속한 검색 및 조회를 위해 필드와 열이 있는 테이블로 구성된 대규모 데이터 모음임.</li>
<li>예시 테이블 <code v-pre>CS166</code>:
<ul>
<li>필드/열: <code v-pre>First_Name</code>, <code v-pre>Last_Name</code>, <code v-pre>Code_ID</code></li>
<li>레코드/행: <code v-pre>(Bernardo, Palazzi, 345)</code></li>
</ul>
</li>
</ul>
...
<li>결과 집합은 결과 테이블에 저장됨.</li>
<li><code v-pre>WHERE</code> 절은 레코드를 필터링하는 데 사용됨.</li>
<li>기본 구문:<div class="language-sql line-numbers-mode" data-highlighter="prismjs" data-ext="sql"><pre v-pre><code class="language-sql"><span class="line"><span class="token keyword">SELECT</span> column_name<span class="token punctuation">(</span>s<span class="token punctuation">)</span> <span class="token operator">or</span> <span class="token operator">*</span></span>
...
<li><code v-pre>ORDER BY</code>는 하나 이상의 필드(열)를 따라 데이터를 정렬하는 데 사용됨.</li>
<li><code v-pre>LIMIT</code>는 특정 수의 레코드(행)만 검색하도록 허용함.</li>
<li>확장 구문:<div class="language-sql line-numbers-mode" data-highlighter="prismjs" data-ext="sql"><pre v-pre><code class="language-sql"><span class="line"><span class="token keyword">SELECT</span> column_name<span class="token punctuation">(</span>s<span class="token punctuation">)</span> <span class="token operator">or</span> <span class="token operator">*</span></span>
<span class="line"><span class="token keyword">FROM</span> table_name</span>
<span class="line"><span class="token keyword">WHERE</span> column_name operator <span class="token keyword">value</span></span>
...
<ol>
<li>인젝션(Injection)</li>
<li>Broken Authentication and Session Management(인증 및 세션 관리 취약점)</li>
<li>크로스 사이트 스크립팅(Cross-Site Scripting, XSS)</li>
<li>Insecure Direct Object References(안전하지 않은 직접 객체 참조)</li>
<li>Security Misconfiguration(보안 설정 오류)</li>
<li>Sensitive Data Exposure(민감한 데이터 노출)</li>
<li>Missing Function Level Access Control(기능 수준 access control 부재)</li>
<li>크로스 사이트 요청 위조(Cross-Site Request Forgery, CSRF)</li>
<li>Using Components with Known Vulnerabilities(알려진 취약점이 있는 구성요소 사용)</li>
<li>Unvalidated Redirects and Forwards(검증되지 않은 리다이렉트 및 포워드)</li>
</ol>
</li>
</ul>
...
<li>종종 이 사용자 입력은 데이터베이스에 제출되는 SQL query를 구성하는 데 문자 그대로 사용됨. 예:
<ul>
<li><code v-pre>SELECT user FROM table WHERE name = 'user_input';</code></li>
</ul>
</li>
...
<ul>
<li><code v-pre>select * from users where user='$usern' AND pwd='$password'</code></li>
</ul>
</li>
<li>전형적인 SQL 인젝션 공격
<ul>
<li>서버 측 코드가 웹 폼의 사용자 입력으로부터 변수 <code v-pre>$username</code>과 <code v-pre>$passwd</code>를 설정</li>
<li>변수가 SQL query에 전달됨.
<ul>
<li><code v-pre>select * from users where user='$username' AND pwd='$passwd'</code></li>
</ul>
</li>
<li>공격자에 의해 특수 문자열이 입력될 수 있음.
<ul>
<li><code v-pre>select * from users where user='M' OR 1=1 --' AND pwd='M' OR '1=1'</code></li>
</ul>
</li>
<li>결과: 암호 없이 접근 권한 획득</li>
</ul>
</li>
</ul>
...
<li>query 수정:
<ul>
<li><code v-pre>select user,pwd from users where user='$usern'</code></li>
<li><code v-pre>$usern=&quot;M' OR '1=1&quot;</code></li>
<li>결과: 전체 테이블</li>
</ul>
</li>
<li>우리는 확인할 수 있음.
...
<ul>
<li><code v-pre>Escape(&quot;t ' c&quot;)</code>는 <code v-pre>&quot;t \' c&quot;</code>를 결과로 제공</li>
</ul>
</li>
<li><code v-pre>select user,pwd from users where user='$usern'</code>에서 <code v-pre>$usern=escape(&quot;M' ;drop table user;&quot;)</code>를 적용하면,</li>
<li>결과는 안전한 query가 됨.
<ul>
<li><code v-pre>select user,pwd from users where user='M\' drop table user;\''</code></li>
</ul>
</li>
</ul>
...
<ul>
<li>사용자가 전체 데이터베이스에 접근할 수 있는지 또는 일부에만 접근할 수 있는지</li>
<li>사용자가 어떤 접근 권한(생성, 삽입, 삭제, 업데이트, 읽기, 쓰기)을 갖는지</li>
</ul>
</li>
<li>다양한 관리 정책을 지원할 수 있음.
<ul>
<li>Centralized administration(중앙 집중식 관리)
<ul>
<li>소수의 권한 있는 사용자가 접근 권한을 부여하고 취소할 수 있음.</li>
</ul>
</li>
<li>Ownership-based administration(소유권 기반 관리)
<ul>
<li>테이블의 생성자가 해당 테이블에 대한 접근 권한을 부여하고 취소할 수 있음.</li>
</ul>
</li>
<li>Decentralized administration(분산 관리)
<ul>
<li>테이블의 소유자가 다른 사용자에게 권한 부여 권한을 부여하고 취소할 수 있게 하여, 그들이 테이블에 대한 접근 권한을 부여하고 취소할 수 있도록 함.</li>
</ul>
</li>
</ul>
</li>
...
<ul>
<li><code v-pre>GRANT</code>: 하나 이상의 접근 권한을 부여하거나 사용자를 역할에 할당하는 데 사용</li>
<li><code v-pre>REVOKE</code>: 접근 권한을 취소</li>
</ul>
</li>
<li>일반적인 접근 권한:
<ul>
<li><code v-pre>SELECT</code>, <code v-pre>INSERT</code>, <code v-pre>UPDATE</code>, <code v-pre>DELETE</code>, <code v-pre>REFERENCES</code></li>
</ul>
</li>
</ul>
...
<li>데이터베이스 RBAC는 다음 기능을 제공해야 함.
<ul>
<li>역할 생성 및 삭제</li>
<li>역할에 대한 권한 정의</li>
<li>사용자의 역할 할당 및 취소</li>
</ul>
</li>
<li>데이터베이스 사용자 범주:
<ul>
<li>Application owner(애플리케이션 소유자): 애플리케이션의 일부로 데이터베이스 객체를 소유하는 최종 사용자</li>
<li>End user(최종 사용자): 특정 애플리케이션을 통해 데이터베이스 객체를 조작하지만 데이터베이스 객체를 소유하지 않는 최종 사용자</li>
<li>Administrator(관리자): 데이터베이스의 일부 또는 전체에 대한 관리 책임을 가진 사용자</li>
</ul>
</li>
</ul>
...
<li>Inference channel(추론 채널)
<ul>
<li>승인되지 않은 데이터가 획득되는 정보 전송 경로</li>
</ul>
</li>
</ul>
...
<!-- end list -->
<ol>
<li><code v-pre>(전공='CS' AND 성별='남')</code>인 사람의 수를 query하여 1이라는 결과를 얻음.</li>
<li><code v-pre>(전공='CS' AND 성별='남')</code>인 사람의 이름을 query하여 <code v-pre>Jones</code>라는 결과를 얻음.</li>
<li>이 두 결과를 통해, 남성 CS 전공자가 <code v-pre>Jones</code> 한 명뿐임을 알게 됨.</li>
...
<ul>
<li>데이터베이스 구조를 변경하거나 access control 체제를 변경</li>
</ul>
</li>
<li>query 시 추론 탐지
<ul>
<li>query를 모니터링하고 변경하거나 거부</li>
</ul>
</li>
<li>두 접근 방식 모두 추론 탐지 알고리즘이 필요함.
<ul>
<li>어려움.</li>
<li>지속적인 연구 주제임.</li>
</ul>
...
<li>두 가지 유형:
<ul>
<li>Pure statistical database(순수 통계 데이터베이스)
<ul>
<li>통계 데이터만 저장</li>
</ul>
</li>
<li>Ordinary database with statistical access(통계적 접근이 가능한 일반 데이터베이스)
<ul>
<li>개별 항목을 포함</li>
<li>DAC, MAC, RBAC를 사용</li>
</ul>
</li>
</ul>
</li>
...
<ul>
<li>
<p>데이터베이스는 레코드 <span v-pre class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><msub><mi>B</mi><mi>i</mi></msub></mrow><annotation encoding="application/x-tex">B_i</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:0.8333em;vertical-align:-0.15em;"></span><span class="mord"><span class="mord mathnormal" style="margin-right:0.05017em;">B</span><span class="msupsub"><span class="vlist-t vlist-t2"><span class="vlist-r"><span class="vlist" style="height:0.3117em;"><span style="top:-2.55em;margin-left:-0.0502em;margin-right:0.05em;"><span class="pstrut" style="height:2.7em;"></span><span class="sizing reset-size6 size3 mtight"><span class="mord mathnormal mtight">i</span></span></span></span><span class="vlist-s">​</span></span><span class="vlist-r"><span class="vlist" style="height:0.15em;"><span></span></span></span></span></span></span></span></span></span>의 집합으로 모델링될 수 있으며, 각 레코드는 M개의 속성 값 <span v-pre class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><msub><mi>x</mi><mrow><mi>i</mi><mi>j</mi></mrow></msub></mrow><annotation encoding="application/x-tex">x_{ij}</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:0.7167em;vertical-align:-0.2861em;"></span><span class="mord"><span class="mord mathnormal">x</span><span class="msupsub"><span class="vlist-t vlist-t2"><span class="vlist-r"><span class="vlist" style="height:0.3117em;"><span style="top:-2.55em;margin-left:0em;margin-right:0.05em;"><span class="pstrut" style="height:2.7em;"></span><span class="sizing reset-size6 size3 mtight"><span class="mord mtight"><span class="mord mathnormal mtight" style="margin-right:0.05724em;">ij</span></span></span></span></span><span class="vlist-s">​</span></span><span class="vlist-r"><span class="vlist" style="height:0.2861em;"><span></span></span></span></span></span></span></span></span></span>의 연결로 표현됨</p>
...
<li>속성 값에 대한 논리식</li>
<li>예: <code v-pre>(Sex=Male) AND ((Major=CS) OR (Major=EE))</code></li>
...
<ul>
<li>새로운 query와 이전 query 간의 중복을 제한</li>
<li>여러 문제점을 가짐.</li>
</ul>
</li>
<li>Partitioning(파티셔닝)
<ul>
<li>레코드를 여러 상호 배타적인 그룹으로 클러스터링</li>
<li>각 그룹 전체의 통계적 속성을 query</li>
</ul>
</li>
<li>Query denial and information leakage(query 거부 및 정보 유출)
<ul>
<li>거부는 정보를 유출할 수 있음.</li>
<li>대응하려면 사용자의 query를 추적해야 함.</li>
</ul>
</li>
</ul>
...
<li>query 제한
<ul>
<li>query 집합 크기 제어</li>
<li>query 집합 중복 제어</li>
<li>파티셔닝</li>
</ul>
</li>
<li>섭동
<ul>
<li>Data perturbation(데이터 섭동) (예: 데이터 스와핑)</li>
<li>Output perturbation(출력 섭동) (예: 잡음 추가, 무작위 표본)</li>
</ul>
</li>
</ul>
...
<li>Data perturbation(데이터 섭동) 기법
<ul>
<li>개별 레코드의 값을 추론하는 데 사용할 수 없는 통계를 생성하도록 데이터를 수정할 수 있음.</li>
</ul>
</li>
<li>Output perturbation(출력 섭동) 기법
<ul>
<li>시스템이 원본 데이터베이스가 제공할 통계와는 다른 수정된 통계를 생성함.</li>
</ul>
</li>
<li>Random-sample query(무작위 표본 query)
<ul>
<li>목표는 원본 결과와 섭동된 결과 간의 차이를 최소화하는 것임.</li>
<li>주요 과제는 사용할 오류의 평균 크기를 결정하는 것임.</li>
</ul>
</li>
</ul>
...
<li>여러 단계, 계층의 보안으로 보호됨.
<ul>
<li>방화벽, 인증, OS access control 시스템, DB access control 시스템, 데이터베이스 자체에 대한 암호화</li>
</ul>
</li>
<li>암호화는 특히 sensitive한 데이터에 주로 구현됨. (개인정보보호법에 의해 명시)</li>
<li>전체 데이터베이스, 레코드 수준, 속성 수준 또는 개별 필드 수준 중 어느 수준에 적용할지 결정 필요</li>
<li>암호화의 단점:
<ul>
<li>Key management(키 관리)</li>
<li>Inflexibility(유연성 부족)
<ul>
<li>query 처리 시</li>
</ul>
</li>
...
<li>Data owner(데이터 소유자): 통제된 release를 위해 데이터를 생성하는 조직</li>
<li>User(사용자): 시스템에 query를 제출하는 human entity</li>
...
<tr>
<td style="text-align:left">0111000111001010...</td>
...
<tr>
<td style="text-align:left">1100010010001101...</td>
<td style="text-align:left">2</td>
<td style="text-align:left">5</td>
<td style="text-align:left">1</td>
...
<tr>
<td style="text-align:left">0011010011111101...</td>
<td style="text-align:left">5</td>
<td style="text-align:left">5</td>
<td style="text-align:left">2</td>
<td style="text-align:left">4</td>
...
<li>3가지 service Models
<ul>
<li>서비스형 소프트웨어(SaaS, Software as a Service)</li>
<li>서비스형 플랫폼(PaaS, Platform as a Service)</li>
<li>서비스형 인프라(IaaS, Infrastructure as a Service)</li>
</ul>
</li>
<li>4가지 Deployment Models
<ul>
<li>Public</li>
<li>Private</li>
<li>Hybrid</li>
...
<ul>
<li>클라우드 컴퓨팅의 남용 및 악의적 사용</li>
<li>안전하지 않은(insecure) interface 및 API</li>
<li>악의적인 내부자</li>
<li>공유된 기술 문제</li>
<li>데이터 손실 또는 유출</li>
<li>Account 또는 Service hijacking</li>
<li>아직 발견되지 않은(unknown) risk profile</li>
</ul>
</li>
</ul>
...
<ul>
<li>클라우드 고유의 위험과 과제</li>
<li>클라우드 환경의 아키텍처 또는 운영 특성</li>
<li>Multi-instance model(다중 인스턴스 모델)
<ul>
<li>각 클라우드 가입자에게 가상 머신 인스턴스에서 실행되는 고유한 DBMS를 제공</li>
<li>가입자에게 보안 관련 관리 작업에 대한 완전한 제어권을 부여</li>
<li>Multi-tenant model(다중 테넌트 모델)
<ul>
<li>일반적으로 가입자 식별자로 데이터를 태그하여 다른 테넌트와 공유되는 사전 정의된 환경을 클라우드 가입자에게 제공</li>
<li>인스턴스를 독점적으로 사용하는 것처럼 보이지만, 안전한 데이터베이스 환경을 구축하고 유지하기 위해 클라우드 제공자에게 의존함.</li>
</ul>
</li>
</ul>
</li>
...
<li>데이터베이스 관리 시스템(DBMS): 데이터베이스를 구축하고 유지하기 위한 프로그램</li>
<li>SQL(Structured Query Language): 관계형 데이터베이스의 데이터를 정의/조작/query하는 데 사용되는 언어</li>
<li>관계형 데이터베이스: 행(튜플)과 열(속성)으로 구성된 데이터 테이블</li>
<li>데이터베이스 access control: 중앙 집중식/소유권 기반/분산 관리</li>
<li>역할 기반 access control(RBAC): 애플리케이션 소유자/애플리케이션 소유자 이외의 최종 사용자/관리자</li>
<li>Inference channel(추론 채널): 승인되지 않은 데이터가 획득되는 정보 전송 경로</li>
<li>통계 데이터베이스(SDB): query 제한/섭동/데이터 스와핑/무작위 표본 query</li>
<li>Database Encryption(데이터베이스 암호화)</li>
