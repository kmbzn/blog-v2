...
<ul>
<li>보안 시스템에 식별자(identifier)를 제시</li>
</ul>
</li>
<li>Verification(검증) 단계
<ul>
<li>개체와 식별자 간의 바인딩(binding)을 확증하는 인증 정보를 제시하거나 생성</li>
</ul>
</li>
...
<ol>
<li>개인이 아는 것(Something the individual knows)</li>
</ol>
<ul>
<li>암호(Password), PIN, 정해진 특정 질문에 대한 대답</li>
</ul>
<ol start="2">
<li>개인이 소유한 것(Something the individual possesses) (토큰)</li>
</ol>
<ul>
<li>스마트카드, 전자 열쇠 카드, 물리적 열쇠</li>
</ul>
<ol start="3">
<li>개인의 존재(Something the individual is) (정적인 생체 정보, static biometrics)</li>
</ol>
<ul>
<li>지문, 망막(Retina), 얼굴</li>
</ul>
<ol start="4">
<li>개인이 하는 행동(Something the individual does) (동적인 생체 정보, dynamic biometrics)</li>
</ol>
<ul>
...
<li>사용자는 name/login과 password를 제공</li>
<li>시스템은 해당 로그인에 대해 저장된 암호와 비교하여, 맞으면 로그인에 성공</li>
<li>사용자 ID:
<ul>
<li>사용자가 시스템에 접근할 권한이 있는지 결정</li>
<li>사용자의 권한을 결정</li>
<li>임의 접근 제어(DAC, discretionary access control)에 사용됨.</li>
</ul>
</li>
</ul>
...
<li>특정 계정 공격(Specific account attack): 특정 target user를 정해서, 개인정보도 함께 알아냄</li>
<li>주요한 암호 공격(Popular password attack): <code v-pre>1111</code> 같은</li>
<li>단일 사용자에 대한 암호 추측(Password guessing against single user)</li>
<li>워크스테이션 하이재킹(Workstation hijacking): 사용자가 잠시 자리를 비운 사이에 명령어 실행</li>
<li>사용자 실수 악용(Exploiting user mistakes): URL을 전송하고 접속하여 로그인하라는 식</li>
...
<li>침입 탐지(Intrusion detection) 조치</li>
<li>암호가 유출된 경우 신속한 암호 재발급 조치</li>
<li>계정 잠금(Account lockout) 메커니즘: 몇 번 이상 틀린 경우</li>
<li>사용자가 흔한, 간단한 암호를 선택하는 것을 금지하는 policy</li>
<li>암호 정책에 대한 교육 및 시행
<ul>
<li>9자 이상</li>
<li>대/소문자, 특수 문자, 숫자 등 포함</li>
</ul>
</li>
...
<p>사용자가 암호를 설정하면,</p>
<ul>
<li>Salt라고 하는 random한 값을 함께 concatnate함.</li>
<li>시스템은 hash 함수(hash function)를 적용하여 이를 암호화하고, hash값을 저장.</li>
...
<li>Salt 미사용 시:
<ol>
<li>사용자가 사용자 ID X와 암호 P를 입력.</li>
<li>시스템이 X의 암호에 대한 저장된 hash H를 조회.</li>
<li>시스템이 <span v-pre class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mi>h</mi><mo stretchy="false">(</mo><mi>P</mi><mo stretchy="false">)</mo><mo>=</mo><mi>H</mi></mrow><annotation encoding="application/x-tex">h(P) = H</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:1em;vertical-align:-0.25em;"></span><span class="mord mathnormal">h</span><span class="mopen">(</span><span class="mord mathnormal" style="margin-right:0.13889em;">P</span><span class="mclose">)</span><span class="mspace" style="margin-right:0.2778em;"></span><span class="mrel">=</span><span class="mspace" style="margin-right:0.2778em;"></span></span><span class="base"><span class="strut" style="height:0.6833em;"></span><span class="mord mathnormal" style="margin-right:0.08125em;">H</span></span></span></span>인지 테스트.</li>
</ol>
<ul>
<li>암호 파일: <code v-pre>... X: H ...</code></li>
</ul>
</li>
<li>Salt 사용 시:
<ol>
<li>사용자가 사용자 ID X와 암호 P를 입력.</li>
<li>시스템이 S와 H를 조회. 여기서 S는 사용자 ID X에 대한 무작위 salt(salt)이고 H는 S와 X의 암호를 결합한 값의 저장된 hash.</li>
<li>시스템이 <span v-pre class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mi>h</mi><mo stretchy="false">(</mo><mi>S</mi><mi mathvariant="normal">∣</mi><mi mathvariant="normal">∣</mi><mi>P</mi><mo stretchy="false">)</mo><mo>=</mo><mi>H</mi></mrow><annotation encoding="application/x-tex">h(S||P) = H</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:1em;vertical-align:-0.25em;"></span><span class="mord mathnormal">h</span><span class="mopen">(</span><span class="mord mathnormal" style="margin-right:0.05764em;">S</span><span class="mord">∣∣</span><span class="mord mathnormal" style="margin-right:0.13889em;">P</span><span class="mclose">)</span><span class="mspace" style="margin-right:0.2778em;"></span><span class="mrel">=</span><span class="mspace" style="margin-right:0.2778em;"></span></span><span class="base"><span class="strut" style="height:0.6833em;"></span><span class="mord mathnormal" style="margin-right:0.08125em;">H</span></span></span></span>인지 테스트.</li>
</ol>
<ul>
<li>암호 파일: <code v-pre>... X: S, H ...</code></li>
</ul>
</li>
</ul>
...
<ul>
<li>최대 8개의 printable characters in length</li>
<li>DES 암호화를 단방향 hash 함수로 수정하기 위해 12비트 salt 사용</li>
<li><code v-pre>0</code> 값을 25회 반복적으로 암호화</li>
<li>출력을 11개의 문자 시퀀스로 변환</li>
</ul>
</li>
<li>현재는 부적절한 것으로 간주됨.
<ul>
<li>여전히 기존 계정 관리 소프트웨어나 다중 벤더 환경과의 호환성을 위해 종종 요구됨.</li>
</ul>
</li>
</ul>
...
<li>권장되는 hash 함수는 MD5에 기반함.
<ul>
<li>최대 48비트의 salt</li>
<li>암호 길이는 무제한</li>
<li>128비트 hash 생성</li>
<li>속도 저하를 위해 1000회 반복되는 내부 루프 사용</li>
</ul>
</li>
<li>OpenBSD는 Bcrypt라는 Blowfish 블록 암호 기반 hash 알고리즘을 사용
<ul>
<li>Unix hash/salt 방식 중 가장 안전한 버전</li>
<li>128비트 salt를 사용하여 192비트 hash 값 생성</li>
</ul>
</li>
</ul>
...
<ul>
<li>가능한 암호들이 포함된 큰 규모의 사전을 개발하여 암호 파일들에 대해 각각 시도</li>
<li>각 암호는 각 salt값을 사용하여 hash된 후 저장된 hash값과 비교되어야 함.</li>
</ul>
</li>
<li>레인보우 테이블 공격(Rainbow table attacks)
<ul>
<li>모든 salt에 대한 hash 값 테이블을 미리 계산</li>
<li>거대한 hash값 테이블</li>
<li>충분히 큰 salt값과 충분히 긴 hash 길이를 사용하여 대응 가능</li>
</ul>
</li>
</ul>
...
<li>권한 있는 사용자에게만 접근 허용</li>
<li>섀도우 암호 파일(Shadow password file)
<ul>
<li>hash된 암호가 저장되는 사용자 ID와는 별개의 파일</li>
</ul>
</li>
<li>취약점
<ul>
<li>파일 접근을 허용하는 OS의 약점</li>
<li>권한 설정 실수로 파일을 읽을 수 있게 됨</li>
<li>다른 시스템에서 동일한 암호를 사용하는 사용자</li>
<li>백업 미디어로부터의 접근</li>
<li>네트워크 트래픽에서 plain text 암호 스니핑(sniffing)</li>
</ul>
</li>
</ul>
...
<ul>
<li>사용자에게 &quot;추측하기 어려운&quot; 암호 사용의 중요성을 알리고 강력한 암호 선택을 위한 guideline 제공</li>
</ul>
</li>
<li>컴퓨터 생성 암호(Computer generated passwords)
<ul>
<li>사용자들이 기억하는 데 어려움을 겪음.</li>
</ul>
</li>
<li>사후 암호 검사(Reactive password checking)
<ul>
<li>시스템이 주기적으로 자체 암호 cracker를 실행하여 추측 가능한 암호를 찾아냄.</li>
</ul>
</li>
<li>사전 암호 검사(Proactive password checking)
<ul>
<li>사용자가 자신의 암호를 선택할 수 있도록 허용하지만, 시스템은 암호가 허용 가능한지 확인하고 그렇지 않으면 거부함.</li>
<li>목표는 사용자가 기억하기 쉬운 암호를 선택하도록 허용하면서 추측 가능한 암호를 제거하는 것.</li>
</ul>
</li>
</ol>
...
<ul>
<li>암호가 준수해야 하는 특정 규칙</li>
</ul>
</li>
<li>암호 크래커(Password cracker)
<ul>
<li>사용하지 말아야 할 암호의 large dictionary를 compile</li>
</ul>
</li>
<li>블룸 필터(Bloom filter)
<ul>
<li>hash를 사용하여 dictionary에 기반한 테이블을 구축하는 데 사용</li>
<li>원하는 암호를 이 테이블과 비교하여 확인</li>
</ul>
</li>
</ul>
...
<li>각 항목을 여러 hash 함수에 통과시켜 비트 배열의 특정 위치를 1로 설정</li>
<li>특정 항목 <span v-pre class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mi>w</mi></mrow><annotation encoding="application/x-tex">w</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:0.4306em;"></span><span class="mord mathnormal" style="margin-right:0.02691em;">w</span></span></span></span>의 소속 여부를 확인하고자 할 때, 해당 항목을 동일한 hash 함수들로 처리하여 비트 배열의 해당 위치들이 모두 <code v-pre>1</code>인지 확인 모두 <code v-pre>1</code>이면 해당 항목은 집합에 속할 가능성이 높다고 판단</li>
...
<li>요청 데이터:
<ul>
<li>IP 주소(및 파생된 국가, ISP 등)</li>
<li>브라우저의 사용자 에이전트(user agent)(및 OS, 버전 등)</li>
<li>타임스탬프</li>
<li>쿠키</li>
</ul>
</li>
<li>Reputation scores(평판 점수)
<ul>
<li>이전에 관찰된 공격 IP</li>
</ul>
</li>
<li>전역 카운터
<ul>
<li>ISP 또는 국가 수준 데이터 사용</li>
</ul>
</li>
<li>회원의 이전 (성공적인) 로그인 기록</li>
</ul>
</li>
<li>사용자별 공격 데이터는 없음</li>
</ul>
<blockquote>
...
<th style="text-align:left">정의적 특징</th>
<th style="text-align:left">예시</th>
</tr>
</thead>
<tbody>
<tr>
<td style="text-align:left">Embossed (엠보싱)</td>
<td style="text-align:left">전면에만 양각 문자</td>
<td style="text-align:left">구형 신용카드</td>
</tr>
<tr>
<td style="text-align:left">Magnetic stripe (마그네틱 스트립)</td>
<td style="text-align:left">뒷면의 마그네틱 바, 전면의 문자</td>
<td style="text-align:left">은행 카드</td>
</tr>
<tr>
<td style="text-align:left">Memory (메모리)</td>
<td style="text-align:left">내부에 전자 메모리</td>
<td style="text-align:left">선불 전화카드</td>
</tr>
<tr>
<td style="text-align:left">Smart (스마트) <br> - Contact (접촉식) <br> - Contactless (비접촉식)</td>
<td style="text-align:left">내부에 전자 메모리 및 프로세서 <br> 표면에 노출된 전기 접점 <br> 내부에 내장된 radio 안테나</td>
<td style="text-align:left">생체 인식 ID 카드</td>
</tr>
</tbody>
</table>
...
<li>가장 흔한 것은 마그네틱 스트립 카드(magnetic stripe card).</li>
<li>내부 전자 메모리를 포함할 수 있음.</li>
<li>물리적 접근에 단독으로 사용 가능
<ul>
<li>호텔 룸</li>
<li>ATM 기계</li>
</ul>
</li>
<li>암호나 PIN과 결합될 때 훨씬 더 높은 보안을 제공 (분실할 수도 있기 때문)</li>
<li>메모리 카드의 단점:
<ul>
<li>special한 reader 필요</li>
<li>토큰 분실</li>
<li>사용자 불만족</li>
</ul>
</li>
...
<li>물리적 특징
<ul>
<li>내장된 마이크로프로세서(microprocessor) 포함, 약간의 계산 요구</li>
<li>은행 카드처럼 생긴 스마트 토큰</li>
<li>계산기, 열쇠, 작은 휴대용 물체처럼 보일 수 있음.</li>
</ul>
</li>
<li>인터페이스
<ul>
<li>수동 인터페이스는 상호작용을 위한 키패드와 디스플레이를 포함</li>
<li>전자 인터페이스는 호환되는 reader/writer와 통신</li>
</ul>
</li>
<li>인증 프로토콜
<ul>
<li>정적, 동적 암호 생성기, 챌린지-응답(challenge-response)의 세 가지 범주로 분류</li>
</ul>
</li>
</ul>
...
<li>패턴 인식(pattern recognition)에 기반함.</li>
<li>암호 및 토큰에 비해 기술적으로 복잡하고 비용이 많이 듦.</li>
<li>사용되는 신체적 특징:
<ul>
<li>얼굴 특징</li>
<li>지문</li>
...
<ul>
<li>Enrollment(등록)</li>
<li>Verification(인증) - 정확히 같은 값이 나오지는 않음, 따라서 어느 정도의 유사도가 나오면 인증 성공으로 간주해 주어야 함.</li>
<li>Identification(식별)</li>
</ul>
</li>
</ul>
...
<li>오인식률(FAR, False Accept Rate): 비정상적인 사용자가 승인될 확률</li>
<li>두 분포 곡선이 교차하는 지점을 교차 에러율(CER, Crossover Error Rate) 또는 등가 에러율(EER, Equal Error Rate)이라 함.
<ul>
<li>이 지점에서 FRR과 FAR이 같아짐.</li>
</ul>
</li>
</ul>
...
<li>다음과 같은 추가적인 보안 위협 존재:
<ul>
<li>도청(eavesdropping), 암호 캡쳐, 관찰된 인증 시퀀스 재전송(replaying)</li>
</ul>
</li>
...
<p>챌린지-응답 프로토콜의 예시</p>
<ul>
<li>사용자는 원격 호스트에 신원을 전송</li>
<li>호스트는 임의의 숫자(논스, nonce)를 생성</li>
<li>nonce가 사용자에게 return됨</li>
<li>사용자는 nonce와 암호 hash를 입력으로 하는 함수를 계산하여 결과를 host에 전송</li>
<li>호스트는 암호의 hash 코드를 저장</li>
...
<li>사용자는 호스트에 ID를 전송</li>
<li>호스트는 임의의 숫자와 암호화를 위한 식별자로 응답</li>
<li>클라이언트 시스템은 사용자 측의 생체 인식 장치를 제어</li>
<li>클라이언트는 생체 정보를 캡처하여 임의의 숫자로 암호화한 후 호스트에 전송</li>
...
<li>도청(Eavesdropping)
<ul>
<li>공격자가 사용자와의 물리적 근접성을 포함하는 어떤 종류의 공격으로 암호를 알아내려는 시도</li>
</ul>
</li>
<li>호스트 공격(Host attacks)
<ul>
<li>암호, 토큰 패스코드 또는 생체 인식 템플릿이 서버에 저장된 호스트의 사용자 파일을 대상으로 함</li>
</ul>
</li>
<li>재전송(Replay)
<ul>
<li>공격자가 이전에 캡처한 사용자 응답을 반복하여 전송하는 공격</li>
</ul>
</li>
<li>클라이언트 공격(Client attacks)
<ul>
<li>공격자가 원격 호스트나 중간 통신 경로에 접근하지 않고 사용자 인증을 달성하려는 시도</li>
</ul>
</li>
<li>트로이 목마(Trojan horse)
<ul>
<li>사용자 암호, 패스코드 또는 생체 인식을 캡쳐할 목적으로 진짜 애플리케이션이나 device인 것처럼 위장하는 애플리케이션 또는 물리적 device</li>
</ul>
</li>
<li>서비스 거부(Denial-of-service)
<ul>
<li>수많은 인증 시도로 사용자 인증 서비스를 마비시키려는 시도</li>
</ul>
</li>
</ul>
...
<ul>
<li>네트워크 트래픽이 정상인가? -&gt; 주기적 요청 -&gt; 네트워크 트래픽</li>
</ul>
</li>
<li>에이전트 기반 사용자 인증(Challenge-response 기반)
<ul>
<li>사용자 패턴이 정상인가? -&gt; 정상이 아니면, 사용자를 다시 확인하기 위해 챌린지를 만들어 전송! -&gt; 챌린지(이벤트) -&gt; 응답(행동 패턴) -&gt; 현재 행동 패턴</li>
</ul>
</li>
</ul>
...
<ul>
<li>프레임워크
Initial Login Authentication -&gt; Continuous Authentication &lt;-&gt; Enrollment Temlate Update &lt;-&gt; Re-login Authentication</li>
<li>생체인식(Biometrics)
<ul>
<li>소프트 생체인식(Soft biometric)
<ul>
<li>&quot;개인에 대한 일부 정보를 제공하지만, 두 개인을 충분히 구별할 수 있는 독특함이 부족한 특성&quot;으로 정의됨.</li>
<li>예: 성별, 민족, 눈/피부/머리 색깔, 키, 몸무게 등</li>
</ul>
</li>
<li>하드 생체인식(Hard biometric)
<ul>
<li>예: 지문, 얼굴, 홍채, 손바닥 정맥</li>
</ul>
</li>
</ul>
</li>
...
<li>모바일 장치의 센서 사용
<ul>
<li>수많은 sensor들이 있고, 이 정보를 수집하여 판단</li>
<li>Mobile device의 가속도계(accelerometer) 사용</li>
<li>서로 다른 두 사람(Person A, Person B)의 걸음걸이에서 측정된 가속도계 데이터(x, y, z축)는 뚜렷하게 다른 패턴을 보임.</li>
</ul>
</li>
</ul>
...
<li>특징 계산</li>
<li>특징 DB에 저장</li>
<li>저장된 특징 DB로 랜덤 포레스트(Random Forest) 모델 학습</li>
</ol>
</li>
<li>사용자 인증(Testing) 과정
<ol>
<li>사용자 데이터 추출</li>
<li>특징 계산</li>
<li>특징을 분류 모델(예: 랜덤 포레스트)을 사용하여 예측</li>
<li>현재 사용자인지 여부에 따라 인증 성공 또는 실패 결정</li>
</ol>
</li>
</ul>
...
<li>암호의 취약점
<ul>
<li>오프라인 dictionary attack, 특정 계정 공격, 인기 있는 암호 공격, 단일 사용자에 대한 암호 추측, 워크스테이션 하이재킹, 사용자 실수 악용, 다중 암호 사용 악용, 전자적 감시</li>
</ul>
</li>
<li>hash된 암호와 salt값</li>
<li>암호 파일 접근 제어</li>
<li>암호 선택 전략
<ul>
<li>사용자 교육</li>
