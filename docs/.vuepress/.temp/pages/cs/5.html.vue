...
<li>Support for fine and coarse specifications(세밀하고 포괄적인 명세 지원)</li>
<li>Least privilege(최소 권한)</li>
<li>Separation of duty(직무 분리)
<ul>
<li>하나의 작업을 완료하기 위해 <strong>두 명 이상</strong>이 필요</li>
</ul>
</li>
<li>Open and closed policies(개방 및 폐쇄 policy)</li>
<li>Policy combinations and conflict resolution(policy 조합 및 충돌 해결)</li>
<li>Administrative policies(관리 policy)
<ul>
<li>policy 개발 절차 등...</li>
</ul>
</li>
</ul>
...
<ul>
<li>process(process)의 개념과 동일</li>
<li>일반적으로 자신이 시작한 행동에 대해 책임이 있음.</li>
<li>종종 owner(소유자), group(그룹), world(월드)의 세 가지 class를 가짐.</li>
</ul>
</li>
<li>Object(객체): 접근이 control되는 resource(자원)
<ul>
<li>정보를 포함 and/or 수신하는 데 사용되는 entity</li>
<li>보호는 access control이 작동하는 환경에 따라 달라짐.</li>
</ul>
</li>
<li>Access right(접근 권한): subject가 object에 접근하는 방식을 기술
<ul>
<li>예: read(읽기), write(쓰기), execute(실행), delete(삭제), create(생성), search(검색)</li>
</ul>
</li>
</ul>
...
<li>소유권과 관계없이 사용자가 파일에 대한 권한을 정의하는 것을 허용하지 않음.</li>
<li>대신, 보안 결정은 central policy administrator(중앙 policy 관리자)에 의해 이루어짐.</li>
...
<li>SELinux policy은 이 context를 일련의 규칙에서 사용하여 process가 서로 및 다양한 시스템 resource과 상호 작용하는 방식을 정의함.</li>
<li>기본적으로, policy은 규칙이 명시적으로 접근을 허용하지 않는 한 어떠한 상호 작용도 허용하지 않음.</li>
...
<li>종종 access matrix(접근 행렬)을 사용하여 제공됨.
<ul>
<li>한 차원은 resource에 대한 데이터 접근을 시도할 수 있는 식별된 subject로 구성됨.</li>
<li>다른 차원은 접근될 수 있는 object를 나열함.</li>
</ul>
</li>
...
<tbody>
<tr>
<td style="text-align:left"><code v-pre>grant(subject, right, object)</code></td>
<td style="text-align:left">subject에게 object에 대한 권한을 부여</td>
</tr>
<tr>
<td style="text-align:left"><code v-pre>revoke(subject, right, object)</code></td>
<td style="text-align:left">subject로부터 object에 대한 권한을 회수</td>
</tr>
<tr>
<td style="text-align:left"><code v-pre>create(subject, object)</code></td>
<td style="text-align:left">subject가 object를 생성. 생성자는 소유자가 됨.</td>
</tr>
<tr>
<td style="text-align:left"><code v-pre>delete(subject, object)</code></td>
<td style="text-align:left">subject가 object를 삭제. 소유자만 가능.</td>
</tr>
<tr>
<td style="text-align:left"><code v-pre>read(subject, attribute, object)</code></td>
<td style="text-align:left">subject가 object의 속성을 읽음.</td>
</tr>
<tr>
<td style="text-align:left"><code v-pre>write(subject, attribute, object)</code></td>
<td style="text-align:left">subject가 object의 속성을 변경. 소유자만 가능.</td>
</tr>
</tbody>
</table>
...
<li>protection domains(보호 도메인)과 능력을 연관시킬 때 더 큰 유연성을 제공</li>
<li>Access matrix 측면에서, 한 행은 보호 도메인을 정의함.</li>
<li>사용자는 자신의 접근 권한의 일부를 가진 process를 생성할 수 있음.</li>
<li>process와 도메인 간의 연관은 정적이거나 동적일 수 있음.</li>
...
<ul>
<li>특정 파일에 필요한 주요 정보가 있는 제어 구조</li>
<li>여러 파일 이름이 단일 inode와 연관될 수 있음.</li>
<li>활성 inode는 정확히 하나의 파일과 연관됨.</li>
<li>파일 속성, 권한 및 제어 정보는 inode에 저장됨.</li>
<li>디스크에는 파일 시스템의 모든 파일의 inode를 포함하는 inode 테이블 또는 inode 목록이 있음.</li>
<li>파일이 열리면 해당 inode가 주 메모리로 가져와져 메모리 상주 inode 테이블에 저장됨.</li>
</ul>
</li>
<li>directory는 계층적 트리 구조로 구성됨.
<ul>
<li>파일 및/또는 다른 directory를 포함할 수 있음.</li>
<li>파일 이름과 연관된 inode를 가리키는 포인터를 포함함.</li>
</ul>
</li>
<li>user ID(고유 사용자 식별 번호, 사용자 ID)</li>
<li>그룹 ID로 식별되는 주 그룹의 멤버</li>
<li>특정 그룹에 속함.</li>
<li>12개의 보호 비트
<ul>
<li>파일의 소유자, 그룹 멤버, 그리고 다른 모든 사용자에 대한 읽기, 쓰기, 실행 권한을 명시</li>
</ul>
</li>
...
<li>&quot;set group ID&quot;(SetGID)
<ul>
<li>시스템이 access control decision을 내릴 때 실제 사용자의 권한에 더하여 파일 소유자/그룹의 권한을 일시적으로 사용함.</li>
<li>특권 프로그램이 일반적으로 접근할 수 없는 파일/resource에 접근할 수 있게 함.</li>
</ul>
</li>
<li>Sticky bit(스티키 비트)
<ul>
<li>directory에 적용될 때, 해당 directory 내의 파일 소유자만이 파일을 재명명, 이동 또는 삭제할 수 있도록 지정하는 것</li>
<li><code v-pre>/tmp</code> directory는 누구나 파일을 생성하고 삭제 가능한데, linux를 공동으로 여러 사용과 사용할 때 quater가 정해져 있는데, tmp directory는 system reboot될 때 다 삭제됨. <code v-pre>f1</code>라는 파일을 생성했는데, 다른 사용자가 파일을 삭제할 수 없도록 해야 하는데, <code v-pre>rwx</code>로 되어 있으니 누구나 directory 내용을 변경 가능함. 하지만 그렇게 하면 안 되므로 sticky bit가 설정된 directory는 해당 파일의 owner만 rename, move, delete할 수 있음.</li>
</ul>
</li>
<li>Superuser(슈퍼유저): 모든 것을 다 할 수 있는 사용자
<ul>
<li>일반적인 access control 제한에서 제외됨.</li>
<li>시스템 전반에 걸친 접근 권한을 가짐.</li>
</ul>
</li>
</ul>
...
<ul>
<li>FreeBSD, OpenBSD, Linux, Solaris 등에서</li>
</ul>
</li>
<li>FreeBSD
<ul>
<li><code v-pre>setfacl</code> 명령은 UNIX 사용자 ID 및 그룹의 목록을 할당함.</li>
<li>파일에 임의의 수의 사용자 및 그룹을 연관시킬 수 있음.</li>
<li>읽기, 쓰기, 실행 보호 비트</li>
<li>파일이 ACL을 가질 필요는 없음.</li>
<li>파일에 확장 ACL이 있는지 여부를 나타내는 추가 보호 비트를 포함함.</li>
</ul>
</li>
<li>process가 파일 시스템 object에 접근을 요청할 때 두 단계가 수행됨.
<ol>
<li>가장 적절한 ACL을 선택 (소유자, 지정된 사용자, 소유 그룹/지정된 그룹, 기타)</li>
<li>일치하는 항목에 충분한 권한이 포함되어 있는지 확인</li>
</ol>
</li>
...
<ol>
<li>Core RBAC(핵심 RBAC): user, role, permission, session의 기본 요소를 정의</li>
<li>Hierarchical RBAC(계층적 RBAC): role 간의 상속 관계를 지원하여 권한 관리의 효율성을 높임.</li>
...
<li>Role 간의 정의된 관계 또는 role과 관련된 조건</li>
<li>유형
<ul>
<li>Mutually exclusive roles(상호 배타적 역할)
<ul>
<li>사용자는 (세션 동안 또는 정적으로) 집합 내의 한 role에만 할당될 수 있음.</li>
<li>모든 권한(접근 권한)은 집합 내의 한 role에만 부여될 수 있음.</li>
</ul>
</li>
<li>Cardinality
<ul>
<li>role과 관련하여 maximum number를 설정</li>
</ul>
</li>
<li>Prerequisite roles(선행 조건 역할)
<ul>
<li>사용자가 특정 다른 role에 이미 할당된 경우에만 특정 role에 할당될 수 있음을 지시</li>
</ul>
</li>
</ul>
</li>
...
<li>Object
<ul>
<li>파일, 프린터, 터미널, 데이터베이스 레코드 등 access control의 대상이 되는 모든 시스템 resource</li>
</ul>
</li>
<li>Operation(연산)
<ul>
<li>프로그램의 실행 가능한 이미지로, 호출 시 사용자를 위해 일부 기능을 실행</li>
</ul>
</li>
<li>Permission(권한)
<ul>
<li>하나 이상의 RBAC 보호 object에 대해 연산을 수행하는 것에 대한 허용, 승인</li>
</ul>
</li>
</ul>
...
<ul>
<li>사용자 집합에서 사용자 추가 및 삭제</li>
<li>role 집합에서 role 추가 및 삭제</li>
<li>user-to-role 할당 인스턴스 생성 및 삭제</li>
<li>permission-to-role 할당 인스턴스 생성 및 삭제</li>
</ul>
</li>
<li>Supporting system functions(지원 시스템 기능)
<ul>
<li>기본 활성 role 집합으로 사용자 session 생성</li>
<li>session에 활성 role 추가</li>
<li>session에서 role 삭제</li>
<li>session subject가 요청된 연산을 object에 대해 수행할 권한이 있는지 확인</li>
</ul>
</li>
<li>Review functions(검토 기능)
<ul>
<li>관리자가 role assignment나 permission assignment를 확인하고 관리할 수 있게 하는 기능</li>
<li>모델의 모든 요소와 그 관계를 수정하지 않고 볼 수 있게 함.</li>
</ul>
</li>
</ul>
...
<li>role 집합에 카디널리티 제약을 둘 수 있음.
<ul>
<li>(<em>role set, n</em>) 쌍 형태로 정의되며, 어떤 사용자도 해당 role 집합에서 n개 이상의 role에 할당되지 않음.</li>
</ul>
</li>
...
<ul>
<li>따라서, 하나의 사용자가 여러 role에 assign될 수 있음.</li>
</ul>
</li>
<li>Application 관리자: 어떤 application이 어떤 access 권한을 갖는지 정의</li>
<li>Authorization 관리자: 어떤 role이 어떤 application에 접근할 수 있는지 정의</li>
...
<ul>
<li>사용자 속성, resource 속성, 환경 속성 등</li>
<li>예: 나이, role, 직함 등 / 읽기, 삭제, view 등 / object type 등</li>
</ul>
</li>
<li>다양한 속성을 평가할 수 있는 복잡한 Boolean rule set을 표현할 수 있음.</li>
<li>예:
<ul>
<li>policy: 관리자는 자신의 지역 내의 transaction을 볼 수 있음.</li>
<li>policy: <code v-pre>user.role == manager</code>인 사용자는 <code v-pre>user.region == transaction.region</code>인 경우 <code v-pre>table == TRANSACTIONS</code> 테이블에 대해 <code v-pre>action == SELECT</code> 작업을 수행할 수 있음.</li>
</ul>
</li>
</ul>
</div></template>
