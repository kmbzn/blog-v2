...
<li>스마트 가전 등 IoT 기기 공격 증가</li>
<li>원격 메디컬 펌웨어 공격 증가</li>
...
<li><strong>패킷 스니핑</strong>: 네트워크를 통한 펌웨어 업데이트 시 전송되는 이미지 추출</li>
<li><strong>플래시 메모리 덤프</strong>: 임베디드 장치의 플래시 메모리에 저장된 펌웨어 덤프 (디바이스 분해 및 데이터 시트 참고 필요)</li>
...
<li>Watchdog 사용 가능 (자체 변조 가능성으로 신뢰도 문제)</li>
</ul>
</li>
</ul>
...
<li>업데이트 모듈 분석</li>
<li>로그인 모듈 분석</li>
<li>Password/암호화 키의 하드코딩 여부 분석</li>
</ul>
</li>
</ul>
...
<li><strong>문자열 검색 기반 분석</strong>: 키워드 문자열 검색 (예: <code v-pre>Certificate</code>)으로 무결성 검증 방법 분석</li>
...
<li><strong>취약한 인자 전달 루틴</strong></li>
...
<li><strong><code v-pre>ls</code> 명령어 실행</strong>: <code v-pre>!@elqjrld&amp;*</code> 입력 및 Input Box에 열람할 html 파일 입력 (예: <code v-pre>logout.html</code>)</li>
</ol>
</li>
</ul>
...
<li>마이크가 기판 사이에 위치 (레이저 이용 공격 불가능)</li>
</ul>
</li>
...
<li><code v-pre>GND</code>: 그라운드</li>
<li><code v-pre>VCC</code>: 전압</li>
</ul>
</li>
</ul>
...
<li>Uboot 명령어는 Kernel 실행 전에 실행됨</li>
...
<li><strong>펌웨어 구조 분석</strong>: 섹션 별 offset 확인, 파일 시스템 확인 및 분석</li>
...
<li><code v-pre>-tx</code>, <code v-pre>-to</code>: 문자열의 offset 출력 (x: hexadecimal, o: octal)</li>
</ul>
</li>
</ul>
...
<li><strong>파일 탐색기 root 디렉토리</strong>: <code v-pre>\\wsl$</code></li>
<li><strong>WSL 확인</strong>: <code v-pre>wsl –l -v</code></li>
<li><strong>Ubuntu version 확인</strong>: <code v-pre>lsb_release –a</code></li>
<li><strong>백업</strong>: <code v-pre>wsl –export Ubuntu-20.04 backup.tar</code></li>
<li><strong>복구</strong>: <code v-pre>wsl –import Ubuntu-20.04 (install location) (file location)</code></li>
<li><strong>펌웨어 이미지 확인</strong>: <code v-pre>binwalk ****.bin</code> (커널 이미지와 주소 확인)</li>
<li><strong>커널 이미지 추출</strong>: <code v-pre>dd if=****.bin skip=시작주소 bs=1 of=kernel.lzma</code>
<ul>
<li><code v-pre>if</code>: input file</li>
<li><code v-pre>bs</code>: block size</li>
...
<li><code v-pre>file kernel</code></li>
<li><code v-pre>binwalk kernel</code> (kernel version 및 데이터 확인)</li>
</ul>
</li>
