...
<ul>
<li>Multiprogramming: 효율성 필요</li>
<li>Job (code, 데이터) 조직화 -&gt; CPU가 항상 하나를 실행하도록 보장</li>
<li>Job scheduling: Job 중 하나를 선택하여 실행</li>
<li>Wait (e.g., I/O) 발생 시, OS가 다른 job으로 전환</li>
<li>Timesharing (multitasking): CPU가 job을 매우 빈번하게 전환, interactive computing 생성</li>
<li>Response time 최소화</li>
<li>각 사용자는 memory에서 실행 중인 하나 이상의 프로그램 (process) 보유</li>
<li>CPU scheduling: 여러 job이 동시에 실행 준비될 때</li>
<li>Swapping: Process가 memory에 맞지 않을 때</li>
<li>Virtual memory: Process가 memory에 완전히 없어도 실행 허용</li>
</ul>
...
<ul>
<li>Dual-mode operation: OS가 자신과 다른 시스템 구성 요소 보호</li>
<li>User mode 및 kernel mode</li>
<li>Mode bit: Hardware가 제공</li>
<li>시스템이 user code 실행 중인지 kernel code 실행 중인지 구별</li>
<li>Privileged instructions: Kernel mode에서만 실행 가능</li>
<li>System call: Mode를 kernel로 변경, call로부터 return 시 user로 재설정</li>
</ul>
<h2 id="process-management" tabindex="-1"><a class="header-anchor" href="#process-management"><span>Process Management</span></a></h2>
<ul>
<li>Process: 실행 중인 프로그램 (A 프로그램 in execution)</li>
<li>시스템 내 작업 단위 (unit of work)</li>
<li>Program: 수동적 entity, Process: 능동적 entity</li>
<li>Process 자원 필요: CPU, memory, I/O, files, initialization 데이터</li>
<li>Process 종료: 재사용 가능한 자원 회수</li>
<li>Single-threaded process: 다음 실행할 instruction 위치 지정하는 프로그램 counter 1개</li>
<li>Multi-threaded process: Thread 당 프로그램 counter 1개</li>
</ul>
...
<li>Memory 공간 할당 및 해제</li>
</ul>
</li>
</ul>
<h2 id="storage-management" tabindex="-1"><a class="header-anchor" href="#storage-management"><span>Storage Management</span></a></h2>
<ul>
<li>OS: 정보 저장소에 대한 통일된 논리적 view 제공</li>
<li>물리적 속성을 논리적 저장 단위 (file)로 추상화</li>
<li>File-System management:</li>
<li>File을 directory로 조직</li>
<li>Access control: 접근 권한 결정</li>
<li>OS 활동:</li>
<li>File, directory 생성 및 삭제</li>
<li>File, directory 조작 primitive</li>
<li>File을 secondary storage에 mapping</li>
<li>File을 stable storage media에 backup</li>
</ul>
...
<ul>
<li>일반적으로 각 system call에 숫자(number) 연관</li>
<li>System-call interface: 이 숫자에 따라 index된 table 유지</li>
<li>Interface: OS kernel에서 의도한 system call 호출, system call 상태 및 return 값 반환</li>
<li>Caller: system call 구현 방식 알 필요 없음</li>
<li>API 준수 및 OS 결과 이해</li>
<li>API: OS interface 세부 사항 programmer에게 숨김</li>
</ul>
...
<ul>
<li>Use system call, directly</li>
</ul>
...
<h2 id="buffer-overflow-1" tabindex="-1"><a class="header-anchor" href="#buffer-overflow-1"><span>Buffer Overflow</span></a></h2>
<ul>
<li>A very common attack mechanism</li>
<li>1988년 Morris Worm이 처음 널리 사용</li>
<li>Prevention techniques 알려짐</li>
<li>여전히 주요 관심사:</li>
<li>Legacy buggy code</li>
<li>프로그래머의 부주의한 programming practices 지속</li>
</ul>
<h1 id="brief-history-of-buffer-overflow-attacks" tabindex="-1"><a class="header-anchor" href="#brief-history-of-buffer-overflow-attacks"><span>Brief History of Buffer Overflow Attacks</span></a></h1>
...
<li>NIST Glossary 정의:
<blockquote>
<p><em>&quot;A condition at an interface under which more input can be placed into a buffer or 데이터 holding area than the capacity allocated, overwriting other information. Attackers exploit such a condition to crash a system or to insert specially crafted code that allows them to gain control of the system.”</em></p>
</blockquote>
</li>
<li>(요약) Buffer 용량 초과 input -&gt; 다른 정보 덮어쓰기</li>
<li>Attacker: 시스템 crash 또는 code 삽입 (시스템 제어)</li>
</ul>
...
<ul>
<li>Programming error: process가 fixed-sized buffer 한계 초과 데이터 저장 시도</li>
...
<li>Memory access 위반</li>
<li>Attacker가 선택한 code 실행</li>
</ul>
</li>
</ul>
...
<p><img src="@source/cs/image-7.png" alt="alt text"></p>
<blockquote>
<p><strong>Figure 10.1</strong> Basic Buffer Overflow Example</p>
</blockquote>
...
<p><img src="@source/cs/image-8.png" alt="alt text"></p>
<blockquote>
...
<ul>
<li>Machine level: 데이터는 register 또는 memory에 저장</li>
<li>Assembly language: 프로그래머가 데이터 해석 책임</li>
</ul>
<p><img src="@source/cs/image-9.png" alt="alt text"></p>
...
<p><img src="@source/cs/image-10.png" alt="alt text"></p>
<blockquote>
...
<p><img src="@source/cs/image-11.png" alt="alt text"></p>
<blockquote>
...
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code class="language-text"><span class="line">void hello(char *tag) {</span>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>(a) Basic stack overflow C code</p>
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code class="language-text"><span class="line">$ cc -g -o buffer2 buffer2.c</span>
<span class="line"></span>
...
<span class="line">Segmentation fault (core dumped)</span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>(b) Basic stack overflow examples runs</p>
<blockquote>
<p><strong>Figure 10.5</strong> Basic Stack Overflow Example</p>
</blockquote>
<p><img src="@source/cs/image-13.png" alt="alt text"></p>
<blockquote>
<p><strong>Figure 10.6</strong> Basic Stack Overflow Stack Values</p>
</blockquote>
<p><img src="@source/cs/image-14.png" alt="alt text"></p>
<blockquote>
...
<p><img src="@source/cs/image-15.png" alt="alt text"></p>
<blockquote>
...
<ul>
<li>Attacker가 제공하는 code
<ul>
<li>Overflow된 buffer에 저장</li>
<li>전통적으로 user command-line interpreter (shell)로 제어 전달</li>
</ul>
</li>
<li>Machine code: 특정 processor, operating system 의존</li>
<li>Metasploit Project: Exploit 연구, IDS 서명 개발 등에 유용한 정보 제공</li>
</ul>
<h2 id="example-shellcode" tabindex="-1"><a class="header-anchor" href="#example-shellcode"><span>Example Shellcode</span></a></h2>
<p><img src="@source/cs/image-16.png" alt="alt text"></p>
<blockquote>
<p><strong>Figure 10.8</strong> Example UNIX Shellcode</p>
</blockquote>
<h2 id="common-x86-assembly-language-instructions" tabindex="-1"><a class="header-anchor" href="#common-x86-assembly-language-instructions"><span>Common <code v-pre>x86</code> Assembly Language Instructions</span></a></h2>
<h2 id="x86-registers" tabindex="-1"><a class="header-anchor" href="#x86-registers"><span><code v-pre>x86</code> Registers</span></a></h2>
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code class="language-text"><span class="line">$ dir -l buffer4</span>
...
<span class="line">...</span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="stack-overflow-variants" tabindex="-1"><a class="header-anchor" href="#stack-overflow-variants"><span>Stack Overflow Variants</span></a></h2>
<table>
<thead>
<tr>
...
<tbody>
<tr>
<td>a trust system utility</td>
</tr>
<tr>
<td>network service deamon</td>
</tr>
<tr>
<td>commonly used library code</td>
</tr>
</tbody>
</table>
<table>
...
<tbody>
<tr>
<td>Remote shell 실행</td>
</tr>
<tr>
<td>Reverse shell 생성 (hacker에게 연결)</td>
</tr>
<tr>
<td>Local exploit (shell 확보)</td>
</tr>
<tr>
<td>Firewall rules flush (다른 공격 차단 해제)</td>
</tr>
<tr>
<td>chroot (restricted execution) 환경 탈출</td>
</tr>
</tbody>
</table>
...
<li>Hardware resource 접근 제한 (e.g., device drivers 작성)</li>
</ul>
</li>
</ul>
...
<ul>
<li>C: Type safety &lt; space efficiency, performance 중시</li>
<li>프로그래머의 주의(due care) 가정</li>
<li>해결: Code 검사, unsafe coding 재작성</li>
<li>예: OpenBSD project (기존 code base 감사)</li>
<li>결과: 가장 안전한 OS 중 하나로 간주</li>
</ul>
...
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code class="language-text"><span class="line">int copy_buf(char *to, int pos, char *from, int len) {</span>
...
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code class="language-text"><span class="line">short read_chunk(FILE fil, char *to) {</span>
...
<ul>
<li>Dynamically allocated memory 처리 문제 (compile time size 정보 없음)</li>
<li>C의 unsafe standard library routines 사용 문제</li>
<li>접근: Safer variant로 대체</li>
<li>예: Libsafe (dynamic library, 기존 standard library보다 먼저 load)</li>
</ul>
...
<li>GCC extensions
<ul>
<li>Function entry: return address 복사본 safe region에 저장</li>
<li>Function exit: stack frame return address vs 복사본 비교 -&gt; 다르면 프로그램 중단</li>
</ul>
</li>
</ul>
</li>
...
<ul>
<li>Virtual memory 지원 사용 -&gt; 일부 memory region non-executable 설정</li>
<li>MMU (Memory Management Unit) 지원 필요</li>
<li>이슈:</li>
<li>Executable stack code 지원</li>
<li>특별 조항 필요</li>
</ul>
...
<ul>
<li>Key 데이터 structure (stack, heap, global 데이터) 위치 조작</li>
<li>Process마다 random shift 사용</li>
<li>Heap buffer 위치 randomize</li>
<li>Standard library functions 위치 randomize</li>
</ul>
...
<ul>
<li>Stack canary랑 비슷한 방법</li>
<li>Critical memory region 사이에 guard pages 배치</li>
<li>MMU에서 illegal address로 flag</li>
<li>접근 시도 시 process 중단</li>
<li>확장: Stack frame, heap buffer 사이에 guard pages 배치 (execution time cost)</li>
</ul>
...
<ul>
<li>Variant: buffer 및 saved frame pointer address 덮어쓰기</li>
<li>Saved frame pointer 값 변경 -&gt; dummy stack frame 참조</li>
<li>Current function return -&gt; replacement dummy frame</li>
<li>제어: Shellcode (in overwritten buffer)</li>
<li>Off-by-one attacks: 1 byte 더 복사 허용</li>
<li>Defenses:</li>
<li>Stack protection (modification 탐지)</li>
...
<ul>
<li>Stack overflow variant: Return address를 standard library function으로 교체</li>
<li>Non-executable stack 대응</li>
<li>Attacker: Return address 위 stack에 parameter 구성</li>
<li>Function returns -&gt; library function 실행</li>
<li>(2 library call chaining 가능)</li>
<li>Defenses:</li>
...
<ul>
<li>Code injection 대신 기존 code (e.g., libc function) 사용</li>
<li>예: <code v-pre>system(“/bin/sh”);</code> <code v-pre>execve (argv[0],~argv,~NULL);</code></li>
<li>Exploit 예: <code v-pre>“A”*80 + “B”*4 + “\xe0\x8a\x05\x40” + “AAAA” + “\xf9\xbf\x0f\x40”</code></li>
<li><code v-pre>echo()</code> return 시, <code v-pre>system()</code>이 new shell 실행</li>
<li><span v-pre class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mi>W</mi><mo>⊕</mo><mi>X</mi></mrow><annotation encoding="application/x-tex">W \oplus X</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:0.7667em;vertical-align:-0.0833em;"></span><span class="mord mathnormal" style="margin-right:0.13889em;">W</span><span class="mspace" style="margin-right:0.2222em;"></span><span class="mbin">⊕</span><span class="mspace" style="margin-right:0.2222em;"></span></span><span class="base"><span class="strut" style="height:0.6833em;"></span><span class="mord mathnormal" style="margin-right:0.07847em;">X</span></span></span></span> (W xor X) model Bypass</li>
</ul>
<h2 id="heap-overflow" tabindex="-1"><a class="header-anchor" href="#heap-overflow"><span>Heap Overflow</span></a></h2>
<ul>
<li>Heap에 위치한 buffer attack</li>
<li>Return address 없음 -&gt; 제어 전달 어려움</li>
<li>Function pointer exploit 또는 management 데이터 structure 조작</li>
<li>Defenses:</li>
<li>Making the heap non-executable</li>
<li>Randomizing the allocation of memory on the heap</li>
</ul>
...
<h2 id="global-데이터-overflow" tabindex="-1"><a class="header-anchor" href="#global-데이터-overflow"><span>Global 데이터 Overflow</span></a></h2>
<ul>
<li>Global 데이터에 위치한 buffer attack</li>
<li>Function pointer, vulnerable buffer</li>
<li>Function pointer 덮어쓰기 (나중에 호출됨)</li>
<li>Defenses:</li>
<li>Non-executable or random global 데이터 region</li>
<li>Move function pointers</li>
<li>Guard pages</li>
</ul>
<h2 id="global-데이터-overflow-example" tabindex="-1"><a class="header-anchor" href="#global-데이터-overflow-example"><span>Global 데이터 Overflow Example</span></a></h2>
...
<li>(상황) Attacker: <code v-pre>f(“foo”)</code> 실행 원함, but</li>
<li><code v-pre>f()</code>가 libc에 없음</li>
<li><code v-pre>f()</code> 위치 randomized</li>
</ul>
...
<ul>
<li>Arbitrary (Turing-complete) computation (다음 없이):</li>
<li>Malicious code injection</li>
<li>Library function (<code v-pre>system()</code>, <code v-pre>execve()</code>) call</li>
<li>Original code 수정</li>
<li>(단, stack contents (including return address) 변경 필요)</li>
<li>적용 시스템: Intel <code v-pre>x86</code>, ARM, SPARC, Atmel AVR, Z80, PowerPC</li>
<li>사례: Apple iPhone, Desktop PCs, Special-purpose machines</li>
</ul>
<h2 id="general-idea-of-rop" tabindex="-1"><a class="header-anchor" href="#general-idea-of-rop"><span>General Idea of ROP</span></a></h2>
<ul>
<li>Idea: Arbitrary computations (shellcode injection, lib function invocation, code modification 없이)</li>
<li>Approach:</li>
<li>전체 function 대신 small instruction sequences (e.g., libc의) 사용</li>
<li>Instruction sequence: 2 ~ 5 instructions, ret instruction으로 종료</li>
<li>Sequences: gadget으로 chaining</li>
<li>Gadget: 특정 task 수행 (e.g., load, store, xor, branch)</li>
<li>Adversary: Gadget 조합 -&gt; 원하는 action 강제</li>
</ul>
...
<ul>
<li>libc에 특정 instructions 존재 가정</li>
<li><code v-pre>b8</code> 대신 00 byte stream 해석 시, unintended instruction sequence 획득 가능</li>
</ul>
<h2 id="gadget-example-memory-load" tabindex="-1"><a class="header-anchor" href="#gadget-example-memory-load"><span>Gadget Example : Memory Load</span></a></h2>
<ul>
<li>Goal: Load <code v-pre>0xDEADBEEF</code> (pointed by <code v-pre>0x8010ABCD</code>) into <code v-pre>%eax</code> register</li>
<li>
<ol>
<li>Find Gadgets -&gt; BoF attack -&gt; Return -&gt; start Sequence 1</li>
</ol>
</li>
<li>Input value: <code v-pre>Pattern 1 + Pattern 2 + Ret_addr_1 + “\x8D\xAB\x10\x80” + Ret_addr_2</code></li>
<li>
<ol start="2">
<li><code v-pre>pop 0x8010AB8D</code> in register <code v-pre>%eax</code></li>
</ol>
</li>
<li>
<ol start="3">
<li>Control flow: Sequence 1의 ret instruction -&gt; Sequence 2로 이동</li>
</ol>
</li>
<li>
<ol start="4">
<li>move <code v-pre>0xDEADBEEF</code> in register <code v-pre>%eax</code></li>
</ol>
</li>
</ul>
<h2 id="countermeasures" tabindex="-1"><a class="header-anchor" href="#countermeasures"><span>Countermeasures</span></a></h2>
<ul>
<li>Return address 보호 방법</li>
<li>Compiler based solutions:</li>
<li>Return address backup (onto shadow stack)</li>
<li>Hardware facilitated solutions:</li>
<li>Stack 분리 (data-only, call/return addresses-only)</li>
<li>Call/return stack access control</li>
<li>Dynamic binary instrumentation (JIT-compiler 기반):</li>
<li>JIT: Runtime에 instruction block compile -&gt; instrumentation code 추가</li>
<li>프로그램 shepherding:</li>
<li>Return target이 valid call site인지 check (return -&gt; call instruction 직후)</li>
<li>ROPdefender:</li>
<li>Return address vs shadow stack (valid return addresses) check</li>
</ul>
