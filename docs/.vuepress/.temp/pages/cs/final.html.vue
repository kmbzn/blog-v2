...
<li>(2) Single-threaded process has only one program counter. (O)</li>
...
<li>(2) The dynamic analysis method analyzes malware by executing the malware in a controlled enviornment. (O)</li>
<li>(3) The static analysis method cannot analyze malware that applied with the packing techniques. (O)</li>
...
<ul>
<li>아무런 동작도 수행하지 않음. (safe)</li>
</ul>
</li>
<li><code v-pre>malloc(0)</code>
<ul>
<li>구현에 따라 <code v-pre>NULL</code> 또는 <code v-pre>free</code> 가능한 고유 pointer 반환</li>
</ul>
</li>
<li><code v-pre>realloc(p, 0)</code> (Suppose <code v-pre>p</code> is not a null pointer.)
<ul>
<li><code v-pre>p</code>가 가리키는 메모리를 해제 (<code v-pre>free(p)</code>와 동일한 효과)</li>
</ul>
</li>
<li>
<div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c"><pre v-pre><code class="language-c"><span class="line"><span class="token keyword">int</span> <span class="token operator">*</span>pn <span class="token operator">=</span> new <span class="token keyword">int</span><span class="token punctuation">;</span></span>
...
<li>표준 C++에서 <code v-pre>new</code>는 실패 시 <code v-pre>NULL</code> 반환 대신 exception을 발생시키므로, <code v-pre>if</code> 검사는 무의미하거나 항상 <code v-pre>true</code></li>
</ul>
</li>
<li><code v-pre>printf(&quot;%08x %08x&quot;);</code>
<ul>
<li>인자가 부족하므로 stack 상위의 임의 데이터를 읽어 16진수로 출력 (Information leak)</li>
</ul>
</li>
</ol>
...
<li><strong>Configuration Files</strong>: 하드코딩된 패스워드, 암호화 키, 네트워크 설정 정보</li>
...
<li>Hiding Activity
<ul>
<li>Kernal Object Manipulation (DKOM): 프로세스 리스트 등 커널 구조체를 직접 조작하여 자신을 리스트에서 제거</li>
<li>API Hooking: 파일 탐색이나 프로세스 조회 시스템 콜을 가로채어 자신의 파일이나 프로세스 정보를 결과에서 누락시킴.</li>
</ul>
</li>
</ul>
...
<ul>
<li>Structure: Local variable(buffer)와 SFP(Saved Frame Pointer) 사이에 Canary(Random variable)를 삽입</li>
...
<li><code v-pre>wchar_t</code> (Wide Character) 사용과 관련된 문법 및 메모리 할당 오류입니다.</li>
<li>Line 1 (Syntax Error):
<ul>
<li><code v-pre>wchar_twide_str1</code> → <code v-pre>wchar_t wide_str1</code> (자료형과 변수명 사이 공백 누락)</li>
</ul>
</li>
<li>Line 2 (Semantic Error):
<ul>
<li><code v-pre>strlen</code>은 <code v-pre>char</code> 단위 길이만 반환하며, <code v-pre>malloc</code>은 바이트 단위 크기를 요구합니다. <code v-pre>wchar_t</code> 문자열 길이 함수(<code v-pre>wcslen</code>)를 사용하고 <code v-pre>sizeof(wchar_t)</code>를 곱해야 합니다.</li>
<li>수정: <code v-pre>(wchar_t*) malloc((wcslen(wide_str1) + 1) - sizeof(wchar_t));</code></li>
</ul>
</li>
<li>Line 6 (Semantic Error):
<ul>
<li><code v-pre>strcpy</code>는 <code v-pre>char*</code> 전용 함수입니다. <code v-pre>wchar_t*</code> 전용 복사 함수를 사용해야 합니다.</li>
<li>수정: <code v-pre>wcscpy(wide_str2, wide_str1);</code></li>
</ul>
</li>
</ul>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><ul>
<li>Dangerous - No Null Termination
<ul>
<li><code v-pre>src</code>의 길이(10)가 <code v-pre>dst</code>의 크기(6)보다 큽니다. <code v-pre>strncpy</code>는 정확히 6바이트만 복사하고 멈추기 때문에, <code v-pre>dst</code> 마지막에 NULL 문자(\0)가 삽입되지 않습니다.</li>
<li>이후 <code v-pre>dst</code>를 문자열로 사용하려 할 때(예: <code v-pre>printf</code>, <code v-pre>strcpy</code> 등) 메모리 경계를 넘어 읽는 Buffer Over-read가 발생합니다.</li>
</ul>
</li>
</ul>
...
<ul>
<li>가능합니다.</li>
<li>Line 4의 <code v-pre>sprintf(outbuf, buffer);</code>에서 <code v-pre>buffer</code> 자체가 Format String으로 사용되고 있습니다 (Format String Vulnerability).</li>
<li>예시
<ul>
<li>공격자가 <code v-pre>user</code> 입력값으로 <code v-pre>&quot;%520d&quot;</code>와 같은 문자열을 넣으면, <code v-pre>buffer</code>에는 <code v-pre>&quot;ERR ... %520d&quot;</code>가 저장됩니다.
Line 4 실행 시 <code v-pre>sprintf</code>는 이를 해석하여 stack의 값을 읽어 520자리 숫자로 변환해 <code v-pre>outbuf</code>에 쓰려고 시도합니다. 이때 <code v-pre>outbuf</code> 크기(512 bytes)를 초과하여 buffer overflow가 발생합니다.</li>
</ul>
</li>
</ul>
</div></template>
