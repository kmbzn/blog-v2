...
<ul>
<li>탐지가 어려움.</li>
<li>여러 특성을 가짐 (worm/바이러스, 백도어, 스파이웨어, rootkit 등)</li>
<li>변종이 많음.</li>
<li>봇은 일반 사용자 PC에 감염됨.</li>
<li>스푸핑된 IP 주소가 아닌 정상 IP 주소를 사용</li>
<li>적은 수의 공격 패킷을 생성</li>
<li>대부분의 시간 동안 악의적인 활동을 하지 않음.</li>
</ul>
</li>
</ul>
...
<li>감염된 봇이 C&amp;C 서버에 접속</li>
<li>봇 마스터가 C&amp;C 서버에 명령 전달</li>
...
<li>Botmaster는 C&amp;C(Command &amp; Control) 서버를 통해 명령을 전송
<ul>
<li>C&amp;C 서버를 어떻게 설정하는가?</li>
<li>경찰에 의해 C&amp;C 서버가 폐쇄되면 어떻게 되는가?</li>
</ul>
</li>
</ul>
...
<li>하드코딩된 IP 주소
<ul>
<li>봇이 하드코딩된 C&amp;C 서버의 IP 주소로 직접 연결</li>
<li>서버가 차단되면 무용지물</li>
</ul>
</li>
...
<li><code v-pre>mirai</code> 봇넷: CCTV 봇넷, GitHub에 올라와 있음.
<ul>
<li>Default password들이 hard-coding되어 포함되어 있음.</li>
</ul>
</li>
</ul>
...
<li>봇을 worm과 구별하는 특징
<ul>
<li>Worm은 스스로 전파하고 활성화</li>
<li>봇은 초기에 중앙 시설에서 제어</li>
</ul>
</li>
<li>원격 제어 기능의 일반적인 구현 방법은 IRC 서버를 이용하는 것</li>
...
<li>키로거(Keylogger)
<ul>
<li>공격자가 민감한 정보를 모니터링할 수 있도록 키 입력을 캡처</li>
<li>일반적으로 &quot;login&quot;, &quot;password&quot;와 같은 키워드에 가까운 정보만 반환하는 필터링 메커니즘을 사용</li>
</ul>
</li>
<li>스파이웨어(Spyware)
<ul>
<li>침해된 기계를 변조하여 시스템의 광범위한 활동을 모니터링할 수 있도록 함.</li>
<li>브라우징 활동의 이력 및 내용 모니터링</li>
<li>특정 웹 페이지 요청을 가짜 사이트로 리디렉션</li>
<li>브라우저와 특정 웹 사이트 간에 교환되는 데이터를 동적으로 수정</li>
</ul>
</li>
</ul>
...
<li>은행, 게임 또는 유사한 사이트의 로그인 페이지를 모방한 가짜 웹 사이트로 연결되는 URL을 스팸 이메일에 포함</li>
<li>사용자가 계정을 인증하기 위해 긴급한 조치가 필요하다고 제안</li>
<li>공격자는 캡처된 자격 증명을 사용하여 계정을 악용</li>
<li>스피어 피싱(Spear-phishing)
<ul>
<li>수신자는 공격자에 의해 신중하게 조사됨.</li>
<li>이메일은 수신자에게 특별히 맞게 제작되며, 종종 진위성을 확신시키기 위해 다양한 정보를 인용함.</li>
</ul>
</li>
</ul>
...
<li>공격자가 보안 접근 절차를 우회하여 접근할 수 있도록 하는 프로그램의 비밀 진입 지점</li>
...
<li>컴퓨터의 프로세스, 파일, 레지스트리를 모니터링하고 보고하는 메커니즘을 변조하여 자신을 숨김.</li>
...
<li>Memory based (메모리 기반)</li>
<li>User mode (사용자 모드)</li>
<li>Kernel mode (커널 모드)</li>
...
<li><code v-pre>Vigilare</code>: 시스템 버스 모니터를 사용하여 커널 무결성을 모니터링하는 연구
<ul>
<li>하드웨어 수준에서 메모리 접근을 감시하여 커널 코드의 무단 변경을 탐지</li>
</ul>
</li>
...
<li>Messenger Phishing(메신저 피싱)</li>
<li>Smishing(SMS 피싱)</li>
<li>Email Phishing(이메일 피싱)
<ul>
<li>정상적인 사이트처럼 보이는 가짜 피싱 사이트로 유도하여 정보를 탈취</li>
</ul>
</li>
</ul>
...
<li><code v-pre>0</code>인 경우 5V, <code v-pre>1</code>인 경우 0V</li>
...
<ul>
<li>실행 파일에서 필요한 라이브러리를 동적으로 로딩함.</li>
<li>리눅스에서는 <code v-pre>.so</code> 파일</li>
</ul>
</li>
<li><code v-pre>dll injection</code> 공격
<ul>
<li>공격자가 제작한 <code v-pre>dll</code> 파일을 정상적인 프로세스가 로딩하도록 하여, 해당 프로세스의 권한으로 악성 코드를 실행시키는 공격</li>
</ul>
</li>
</ul>
...
<li>브라우저 관련 공격
<ul>
<li>브라우저 기능이 향상되고 웹이 풍부해지면서 공격 표면이 넓어짐.</li>
<li>스팸 이메일을 통해 사용자 유인</li>
</ul>
</li>
<li>웹 서버 공격
<ul>
<li><code v-pre>IFRAME</code>을 이용해 공격 서버로 연결</li>
<li>&quot;drive-by-download&quot;: 악성 코드 전파의 주요 경로로 사용됨.</li>
<li>SQL 인젝션 공격</li>
<li><code v-pre>.htaccess</code>를 통한 리디렉션: 특정 조건(예: HTTP Referrer 헤더)에 따라 URL을 다른 목적지로 선택적으로 리디렉션</li>
</ul>
</li>
</ul>
...
<li>서버 측 웹 애플리케이션의 결함(공격자가 임의의 HTML을 삽입하도록 허용)의 결과</li>
<li>취약한 스크립트를 삽입함으로써,
<ul>
<li>이상한 데이터를 삽입하거나</li>
<li>다른 위치로 점프(리디렉션)</li>
</ul>
</li>
...
<li>트위터 스팸
<ul>
<li>해킹된 계정을 스팸 발송에 이용</li>
<li>성공적인 이유: 많은 청중, URL 단축 서비스, 필터링 메커니즘 부족</li>
</ul>
</li>
<li>Clickthrough rate(CTR, 클릭률)
<ul>
<li>스팸 트윗의 0.13%만이 방문을 유도</li>
<li>이는 스팸 이메일의 CTR(0.003%-0.006%)보다 훨씬 높음.</li>
</ul>
</li>
<li>스팸 분류
<ul>
...
<li>예방이 실패할 경우, 기술적 메커니즘을 사용하여 다음 위협 완화 옵션을 지원할 수 있음:
<ul>
<li>detection(탐지)</li>
...
<li>policy(정책)</li>
<li>awareness(인식)</li>
<li>vulnerability mitigation(취약점 완화)</li>
<li>Threat mitigation(위협 완화)</li>
</ul>
</li>
</ul>
...
<li>실행 파일은 다음 요소를 포함하는 GD 스캐너를 통해 실행됨.
<ul>
<li>CPU 에뮬레이터</li>
<li>바이러스 시그니처 스캐너</li>
<li>에뮬레이션 제어 모듈</li>
...
<li>잠재적으로 악의적인 행동이 시스템에 영향을 미치기 전에 차단</li>
<li>실시간으로 소프트웨어를 차단하므로 시그니처나 휴리스틱과 같은 anti-virus 탐지 기술보다 장점이 있음.</li>
<li>한계점
<ul>
<li>악성 코드가 모든 행동을 식별하기 전에 대상 기계에서 실행되어야 하므로, 탐지 및 차단되기 전에 피해를 줄 수 있음.</li>
</ul>
</li>
</ul>
...
<li>IDS의 트래픽 분석 구성 요소에 포함될 수도 있음.</li>
<li>의심스러운 트래픽의 흐름을 차단하는 침입 방지 조치를 포함할 수 있음.</li>
<li>이 접근 방식은 악성 소프트웨어 스캔에 국한됨.</li>
<li>모니터링 소프트웨어의 두 가지 유형
<ul>
<li>Ingress(인그레스) 모니터: 기업 네트워크와 인터넷 경계에 위치. 사용되지 않는 로컬 IP 주소로 들어오는 트래픽을 찾는 기법.</li>
<li>Egress(이그레스) 모니터: 개별 LAN의 출구 지점 및 기업 네트워크와 인터넷 경계에 위치. 스캐닝이나 기타 의심스러운 행동의 징후를 찾기 위해 나가는 트래픽을 모니터링.</li>
</ul>
</li>
</ul>
...
<li>Worm이 기계에 상주하면 anti-virus 소프트웨어를 사용하여 탐지하고 제거할 수 있음.</li>
<li>경계 네트워크 활동 및 사용량 모니터링은 worm 방어의 기반이 될 수 있음.</li>
<li>Worm 방어 접근 방식:
<ul>
<li>시그니처 기반 worm 스캔 필터링</li>
<li>필터 기반 worm 격리</li>
<li>payload 분류 기반 worm 격리</li>
<li>임계치 랜덤 워크(TRW) 스캔 탐지</li>
<li>속도 제한(Rate limiting)</li>
<li>속도 중단(Rate halting)</li>
</ul>
</li>
</ul>
...
<li>이 정보는 모든 클라이언트에게 자동으로 전파되어 새로운 위협에 신속하게 대응
<img src="@source/cs/image.png" alt="alt text"></li>
</ol>
</li>
</ul>
...
<ul>
<li>설치 시 API 수준 분석</li>
<li>악성 코드에서 자주 사용되는 중요한 API 호출에 초점</li>
<li>API + 매개변수 조합으로 위험도 판단</li>
<li>정적 분석 기법</li>
</ul>
</li>
<li>MADAM
<ul>
<li>안드로이드를 커널 수준과 사용자 수준에서 모니터링</li>
<li>정상 행위와 악성 행위를 구별</li>
<li>다중 수준 모니터링: 1단계(시스템 콜 모니터링), 2단계(사용자 활동 및 SMS 전송 모니터링)</li>
</ul>
</li>
<li>머신러닝을 이용한 안드로이드 악성코드 분석
<img src="@source/cs/image-6.png" alt="alt text">
<ul>
<li>다중 모드 신경망(Multi-modal Neural Networks)과 같은 기법을 사용하여 높은 탐지 정확도(98%)를 달성</li>
</ul>
</li>
</ul>
...
<li>악성코드 분석의 쟁점
<ul>
<li>수동 분석은 행동을 이해하고 대응책을 개발하며 시그니처를 생성하는 데 중요함.</li>
<li>하지만 악성코드의 증가 속도를 따라잡기 어려움. (예: 하루에 약 9만 개)</li>
</ul>
</li>
</ul>
<p><img src="@source/cs/image-5.png" alt="alt text"></p>
<ul>
<li>정적 분석 대 동적 분석
<ul>
<li>Static Analysis(정적 분석): 실행 없이 바이너리를 분석
<ul>
<li>모든 코드를 분석할 수 있지만, 패킹(packing)이나 다형성/변형 worm에는 한계가 있음.</li>
</ul>
</li>
<li>Dynamic Analysis(동적 분석): 가상 환경에서 악성 코드를 직접 실행하여 행위를 분석
<ul>
<li>패킹된 코드의 실제 행위를 관찰할 수 있음.
<img src="@source/cs/image-4.png" alt="alt text"></li>
</ul>
</li>
</ul>
</li>
...
<ul>
<li>Malware Visualization(악성코드 시각화)
<ul>
<li>악성코드 바이너리를 이미지로 변환.</li>
<li>변환된 이미지를 ML(머신 러닝) 기법에 적용하여 유사한 악성코드 패밀리를 분류하거나 새로운 변종을 탐지</li>
</ul>
</li>
</ul>
