...
<span class="line">    └─ ipcam.sh</span>
<span class="line"></span>
<span class="line">/system</span>
<span class="line">    ├─ bin</span>
<span class="line">    │    ├─ encoder</span>
<span class="line">    │    ├─ gmail_thread</span>
<span class="line">    │    ├─ jpeg</span>
<span class="line">    │    └─ daemon.v5.14</span>
<span class="line">    ├─ drivers (비어 있음)</span>
<span class="line">    └─ lib (비어 있음)</span>
<span class="line"></span>
<span class="line">/www        (비어 있음)</span>
<span class="line">/Wireless   (비어 있음)</span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>이는 linux 기반 IP 카메라 펌웨어에서 흔히 발견되는 구조이며,
웹 UI, 설정 파일, 초기화 스크립트 등이 포함되어 있었다.</p>
<p><img src="@source/cs/image-17.png" alt="alt text"></p>
<ul>
...
<ul>
<li><code v-pre>encoder</code>: 영상 인코딩 + 웹 서버 + 계정 인증 루틴까지 통합된 메인 바이너리</li>
<li><code v-pre>daemon.v5.14</code>: 백그라운드 서비스 관리</li>
<li><code v-pre>jpeg</code>: <code v-pre>JPEG</code> 코덱 기능</li>
...
<li>이는 장치의 root 계정이 고정된 값으로 펌웨어에 포함되어 있음을 의미하며,<br>
패스워드 해시(<code v-pre>vRNT.4lCplkng</code>)는 특정 해시 알고리즘(주로 DES 기반 <code v-pre>crypt()</code> 함수)으로 암호화된 값이다.</li>
<li>여기서 앞의 두 글자(<code v-pre>vR</code>)은 salt값에 해당한다.</li>
<li>단방향 알고리즘에 의해 hash된 값이기 때문에 복호화하여 원래 password를 직접 얻어내기에는 어려움이 있었다.
<ul>
<li>4자리 숫자(<code v-pre>0000</code> ~ <code v-pre>9999</code>)를 Brute-force 방식으로 시도해봤으나 실패하였다.</li>
<li>6자리 숫자(<code v-pre>000000</code> ~ <code v-pre>999999</code>)를 Brute-force 방식으로 시도해봤으나 실패하였다.</li>
<li>다만, 해당 암호화 알고리즘이 현재는 더 이상 사용되지 않는 오래된 방식의 취약한 알고리즘으로 알려져 있기 때문에, 유출된 password 기반의 dictionary attack을 시도한다면 원래 password를 알아낼 가능성도 있다.</li>
</ul>
</li>
</ul>
...
<span class="line">GET /api/v2/ddns?ddnsname=%s&amp;ddnspwd=%s ...</span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div></div></div><p>이는 사용자 계정 정보(<code v-pre>username/password</code>)를 평문으로 외부 서버로 전송하는 코드이며,
보안적으로 매우 위험한 설계이다.</p>
<ul>
<li>HTTPS가 아닌 HTTP 사용 → <strong>중간자 공격</strong>에 취약</li>
<li>인증 정보 탈취 가능</li>
...
<p>이번 분석을 통해 확인한 바,</p>
<ul>
<li>펌웨어는 <code v-pre>ZIP</code> 기반으로 패키징되어 있어 압축 프로그램만으로 file system을 추출할 수 있었음.</li>
<li>encoder 바이너리 안에 <code v-pre>root</code> 하드코딩 계정이 존재하였음.</li>
<li><code v-pre>username/password</code>를 평문으로 외부 서버로 전송하는 취약 설계가 존재하였음.</li>
<li>웹 UI 기반 admin 계정 흔적이 다수 포착됨.</li>
<li>종합적으로, 보안 수준이 낮고 <strong>사용자 정보 보호 측면에서도 위험성</strong>이 있다.</li>
</ul>
