...
<li>Leaf Page: Key와 <code v-pre>120B</code> 크기의 Value를 저장하는 <code v-pre>record</code> 배열을 가진다. Order(Branching Factor)는 32이다.</li>
<li>Internal Page: Key와 자식 페이지의 Offset을 저장하는 <code v-pre>inter_record</code> 배열을 가진다. Order는 249이다.</li>
...
<ul>
<li>
<p>Insertion &amp; Splitting:
Leaf Page가 가득 찬 상태(<code v-pre>LEAF_MAX = 31</code>)에서 삽입 시 <code v-pre>insert_into_leaf_after_splitting</code>이 호출된다. 임시 배열을 생성하여 기존 레코드와 새 레코드를 정렬한 후, 중간 지점(<code v-pre>cut</code>)을 기준으로 두 개의 페이지로 분할한다. 이후 새 페이지의 첫 번째 Key를 부모로 올리는(<code v-pre>insert_into_parent</code>) 재귀적 logic을 구현하였다.</p>
</li>
<li>
<p>Deletion &amp; Merging/Redistribution:
<code v-pre>db_delete</code>는 레코드를 물리적으로 제거(<code v-pre>memmove</code>)한다. 삭제 후 Key의 개수가 최소 조건 미만(Underflow)이 되면 <code v-pre>get_neighbor_index</code>를 통해 형제 노드를 탐색한다.</p>
<ul>
<li>Redistribution: 형제 노드에 여유가 있다면 Key를 하나 빌려와 균형을 맞춘다.</li>
<li>Coalesce (Merge): 형제 노드도 여유가 없다면 두 노드를 병합하고, 부모 노드에서 해당 포인터를 제거(<code v-pre>delete_entry</code>)하는 과정을 재귀적으로 수행한다.</li>
</ul>
</li>
</ul>
...
<ul>
<li>
<p>Logical Deletion (<code v-pre>db_delete</code>)</p>
<ul>
<li>물리적인 데이터 이동이나 병합(Merge) 과정을 제거하였다.</li>
<li>대신 해당 레코드의 인덱스 <code v-pre>i</code>에 대해 <code v-pre>page-&gt;reserved[i] = 1</code>로 마킹하고 페이지를 디스크에 저장한다.</li>
<li>이로 인해 삭제 연산의 비용이 <span v-pre class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mi>O</mi><mo stretchy="false">(</mo><mi>log</mi><mo>⁡</mo><mi>N</mi><mo stretchy="false">)</mo><mo>+</mo><mi>O</mi><mo stretchy="false">(</mo><mn>1</mn><mo stretchy="false">)</mo></mrow><annotation encoding="application/x-tex">O(\log N) + O(1)</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:1em;vertical-align:-0.25em;"></span><span class="mord mathnormal" style="margin-right:0.02778em;">O</span><span class="mopen">(</span><span class="mop">lo<span style="margin-right:0.01389em;">g</span></span><span class="mspace" style="margin-right:0.1667em;"></span><span class="mord mathnormal" style="margin-right:0.10903em;">N</span><span class="mclose">)</span><span class="mspace" style="margin-right:0.2222em;"></span><span class="mbin">+</span><span class="mspace" style="margin-right:0.2222em;"></span></span><span class="base"><span class="strut" style="height:1em;vertical-align:-0.25em;"></span><span class="mord mathnormal" style="margin-right:0.02778em;">O</span><span class="mopen">(</span><span class="mord">1</span><span class="mclose">)</span></span></span></span>로 크게 감소하였다.</li>
</ul>
</li>
<li>
<p>Revival on Insertion (<code v-pre>db_insert</code>)</p>
<ul>
<li>이미 존재하는 Key에 대한 삽입 요청이 들어왔을 때, 해당 Key가 논리적으로 삭제된 상태(<code v-pre>reserved == 1</code>)라면 새로운 레코드를 추가하는 대신 기존 위치의 Value를 갱신하고 <code v-pre>reserved = 0</code>으로 변경하여 데이터를 '부활'시킨다.</li>
</ul>
</li>
<li>
<p>Visibility Check (<code v-pre>db_find</code>)</p>
<ul>
<li>탐색 시 Key가 존재하더라도 <code v-pre>reserved == 1</code>이라면 <code v-pre>NULL</code>을 반환하여 사용자에게는 삭제된 것처럼 보이도록 구현하였다.</li>
</ul>
</li>
</ul>
...
<li>Split: 32개 이상의 레코드 삽입 시 Leaf Page가 분할되고, 새로운 Root가 생성되어 트리의 높이가 증가함을 확인하였다.</li>
<li>Delete (Merge): 대량 삭제 수행 시 Page Merge가 발생하고, Root가 다시 Leaf로 내려오거나 변경되는 과정을 확인하였다.</li>
</ol>
<div class="language-bash line-numbers-mode" data-highlighter="prismjs" data-ext="sh"><pre v-pre><code class="language-bash"><span class="line">kmbzn@p31:~/workspace/Assignment3_2021024057/bptree1$ <span class="token function">rm</span> <span class="token parameter variable">-f</span> *.db</span>
...
<li>Reorganization: <code v-pre>q</code>를 눌러 종료 시 <code v-pre>db_reorganize</code>가 호출되어, 삭제된 데이터가 정리된 상태로 DB 파일이 갱신됨을 확인하였다.</li>
</ol>
<div class="language-bash line-numbers-mode" data-highlighter="prismjs" data-ext="sh"><pre v-pre><code class="language-bash"><span class="line">kmbzn@p31:~/workspace/Assignment3_2021024057/bptree2$ <span class="token function">rm</span> <span class="token parameter variable">-f</span> *.db</span>
...
<ol>
<li>Rename: 현재 사용 중인 DB 파일을 <code v-pre>backup.db</code>로 이름을 변경한다.</li>
<li>Create New: 원래 이름으로 새로운 빈 DB 파일을 생성한다 (<code v-pre>open_table</code>).</li>
<li>Migrate:
<ul>
<li><code v-pre>backup.db</code>를 Read-only로 열고, Root부터 시작하여 모든 Leaf Page를 순회한다.</li>
<li>Leaf Page 내에서 <code v-pre>reserved == 0</code>인(삭제되지 않은) 유효한 레코드만 추출한다.</li>
<li>추출된 레코드를 새 DB에 <code v-pre>db_insert</code> 한다.</li>
</ul>
</li>
...
<li>이 방식이 갖는 장점들
<ul>
<li>Sequential I/O: 기존 트리를 순차적으로 읽고(Sequential Read), 새 트리에 순차적으로 씀(Append-only Write)으로써 디스크 헤드 이동을 최소화할 수 있다.</li>
<li>Fragmentation 제거: 새로 생성된 트리는 중간에 빈 공간 없이 꽉 채워지므로(Compacted), 디스크 공간 낭비가 0에 수렴한다고 볼 수 있다.</li>
<li>안전성: 작업 도중 실패하더라도 원본(<code v-pre>backup.db</code>)이 보존된 상태이므로 복구가 용이하다.</li>
</ul>
</li>
</ul>
</div></template>
