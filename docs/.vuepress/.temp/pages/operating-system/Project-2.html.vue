...
<li>새로운 시스템 호출 <code v-pre>join()</code>을 구현해 커널 수준 스레드 종료 대기</li>
...
<div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c"><pre v-pre><code><span class="line"><span class="token keyword">int</span> <span class="token function">clone</span><span class="token punctuation">(</span><span class="token keyword">void</span><span class="token punctuation">(</span><span class="token operator">*</span>fcn<span class="token punctuation">)</span><span class="token punctuation">(</span><span class="token keyword">void</span><span class="token operator">*</span><span class="token punctuation">,</span> <span class="token keyword">void</span><span class="token operator">*</span><span class="token punctuation">)</span><span class="token punctuation">,</span> <span class="token keyword">void</span> <span class="token operator">*</span>arg1<span class="token punctuation">,</span> <span class="token keyword">void</span> <span class="token operator">*</span>arg2<span class="token punctuation">,</span> <span class="token keyword">void</span> <span class="token operator">*</span>stack<span class="token punctuation">)</span><span class="token punctuation">;</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><ul>
<li>프로세스의 주소 공간을 공유하는 새로운 커널 수준 스레드를 생성</li>
<li><code v-pre>fcn</code>: 스레드가 실행할 함수</li>
<li><code v-pre>arg1</code>, <code v-pre>arg2</code>: 스레드에 전달될 인자</li>
<li><code v-pre>stack</code>: 스레드 사용자 공간의 스택 (페이지 정렬 필요, 최소 1 페이지 이상)</li>
<li>성공 시 새 스레드의 PID 반환, 실패 시 -1 반환</li>
</ul>
<div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c"><pre v-pre><code><span class="line"><span class="token keyword">int</span> <span class="token function">join</span><span class="token punctuation">(</span><span class="token keyword">void</span> <span class="token operator">*</span><span class="token operator">*</span>stack<span class="token punctuation">)</span><span class="token punctuation">;</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><ul>
<li>자식 스레드가 종료될 때까지 대기</li>
...
<li>Makefile에 <code v-pre>thread.o</code>를 추가하여 링크</li>
<li><code v-pre>int thread_create(void (*start_routine)(void *, void *), void *arg1, void *arg2);</code>
<ul>
<li>스택과 <code v-pre>clone</code>을 위한 메모리 할당</li>
<li>새 스레드의 PID 반환 또는 -1 반환</li>
</ul>
</li>
<li><code v-pre>int thread_join();</code>
<ul>
<li>종료될 스레드를 기다리고 해당 스택을 해제</li>
<li>종료된 스레드의 PID 반환 또는 -1 반환</li>
</ul>
</li>
</ul>
...
<li><code v-pre>sbrk</code>: 여러 스레드가 메모리를 안전하게 할당 가능해야 함</li>
<li><code v-pre>kill</code>: 종료되는 스레드가 모든 스레드를 함께 종료시켜야 함</li>
<li><code v-pre>sleep</code>: 호출한 스레드에만 영향을 미쳐야 함</li>
<li><code v-pre>pipe</code>: 모든 스레드가 화면에 출력할 수 있어야 함</li>
</ul>
</li>
</ul>
...
<ul>
<li>
<p>기본 xv6 스케줄러(라운드 로빈)를 그대로 사용</p>
</li>
<li>
<p>xv6의 <code v-pre>fork</code>, <code v-pre>exec</code>, <code v-pre>exit</code>, <code v-pre>wait</code> 코드를 면밀히 분석</p>
</li>
<li>
<p>경쟁 조건 방지를 위해 적절한 잠금 사용</p>
</li>
<li>
<p>Resource 누수를 방지하기 위해 적절한 해제 및 메모리 관리 수행</p>
</li>
<li>
<p>Thread와 process의 차이를 <strong>명확히 이해할 것</strong></p>
<ul>
<li>어떤 자원이 공유되고 어떤 자원이 독립적인지 <strong>주의 깊게 구분</strong></li>
<li>이 프로젝트에서 제공되는 기본 코드는 이미 일부 변경된 코드이며, 그 변경 내용을 <strong>위키에 문서화</strong>해야 함</li>
</ul>
</li>
</ul>
...
<li><strong>Implementation</strong>: 주요 코드 수정 및 그 목적 설명</li>
<li><strong>Results</strong>: 구현 결과 증거 제시 (컴파일 로그, 실행 화면, 프로그램 설명 등)</li>
...
<ul>
<li>
<p><strong>Completeness</strong>: xv6 시스템이 사양에 맞게 정상 작동해야 함</p>
</li>
<li>
<p><strong>Wiki &amp; Comment Grading</strong>: 위키 문서화 <strong>품질</strong>도 평가에 반영됨. 최대한 상세히 작성할 것</p>
</li>
<li>
<p><strong>Deadline</strong>: 마감 기한 엄수. 마감 후에는 GitHub 쓰기 권한이 철회됨</p>
<ul>
<li><strong>공유하거나 복사하지 마시오!</strong></li>
</ul>
</li>
</ul>
...
<ul>
<li>공지사항 참조하여 새 레포지토리 생성</li>
<li>레포지토리 이름은 <code v-pre>project02-[student ID]</code>로 설정</li>
<li>위키 파일 이름: <code v-pre>05_project02_[class number]_[student ID].pdf</code></li>
</ul>
</li>
<li>제출 마감일: <strong>2025년 5월 25일 23:59</strong>
<ul>
<li>마감일 이후 2025년 5월 26일 23:59까지 이메일 제출 가능. 단, 점수의 50%만 인정함.</li>
</ul>
</li>
</ul>
</div></template>
