...
<ul>
<li><code v-pre>clone()</code> 시 <code v-pre>np-&gt;ustack = (uint64)stack;</code>으로 저장</li>
...
<tr>
<td>Address Space</td>
<td><code v-pre>uvmshare()</code>로 parent의 pagetable 공유</td>
</tr>
<tr>
<td>User Stack</td>
<td><code v-pre>sbrk(PGSIZE)</code>로 확보, <code v-pre>clone()</code>에 전달하여 <code v-pre>trapframe-&gt;sp</code> 설정</td>
</tr>
<tr>
<td>Function Pointer Entry</td>
<td><code v-pre>trapframe-&gt;epc = fcn</code></td>
</tr>
<tr>
<td>Argument Passing</td>
<td><code v-pre>a0 = arg1</code>, <code v-pre>a1 = arg2</code></td>
</tr>
<tr>
<td>TCB Separation</td>
<td><code v-pre>allocproc()</code> → 독립된 trapframe/context</td>
</tr>
<tr>
<td>Thread Identification</td>
<td><code v-pre>is_thread == 1</code></td>
</tr>
<tr>
<td><code v-pre>exec</code> Cleanup</td>
<td>동일 pagetable 공유하는 proc 전부 <code v-pre>freeproc()</code></td>
</tr>
<tr>
<td>join Collection</td>
<td><code v-pre>copyout()</code>으로 stack 주소 유저에게 전달</td>
</tr>
<tr>
<td><code v-pre>kill()</code> Extension</td>
<td>같은 pagetable 가진 모든 proc에 대해 <code v-pre>killed = 1</code> 설정</td>
</tr>
<tr>
<td>Synchronization</td>
<td><code v-pre>sleep()</code> 전 <code v-pre>p-&gt;lock</code> 확보, <code v-pre>sched()</code> 이후 wakeup 가능 보장</td>
</tr>
</tbody>
</table>
...
<ul>
<li><strong>시스템 콜 인터페이스 (sysproc.c)</strong></li>
<li><strong>kernel 내부 기능 (proc.c)</strong></li>
<li><strong>사용자 수준 라이브러리 (user/thread.c, thread.h)</strong></li>
...
<div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c"><pre v-pre><code><span class="line">uint64 <span class="token function">sys_clone</span><span class="token punctuation">(</span><span class="token keyword">void</span><span class="token punctuation">)</span></span>
...
<li><code v-pre>argaddr()</code>는 반환형이 <code v-pre>void</code>이므로, <code v-pre>argraw()</code> 값을 간접적으로 참조하여 처리한다.</li>
...
<div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c"><pre v-pre><code><span class="line">uint64 <span class="token function">sys_join</span><span class="token punctuation">(</span><span class="token keyword">void</span><span class="token punctuation">)</span></span>
<span class="line"><span class="token punctuation">{</span></span>
...
<li><code v-pre>uvmshare()</code>로 부모와 동일한 pagetable을 공유한다.</li>
<li>trapframe은 부모의 것을 복사하고, epc/sp/a0/a1 등은 인자로 주어진 값을 반영해 설정한다.</li>
<li><code v-pre>ofile[]</code>과 <code v-pre>cwd</code>는 <code v-pre>filedup()</code>, <code v-pre>idup()</code>을 통해 복사한다.</li>
<li><code v-pre>RUNNABLE</code>로 전환되면 scheduler에 의해 실행된다.</li>
</ul>
<div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c"><pre v-pre><code><span class="line">np<span class="token operator">-></span>trapframe<span class="token operator">-></span>epc <span class="token operator">=</span> <span class="token punctuation">(</span>uint64<span class="token punctuation">)</span>fcn<span class="token punctuation">;</span></span>
...
<div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c"><pre v-pre><code><span class="line"><span class="token keyword">for</span><span class="token punctuation">(</span><span class="token keyword">struct</span> <span class="token class-name">proc</span> <span class="token operator">*</span>q <span class="token operator">=</span> proc<span class="token punctuation">;</span> q <span class="token operator">&lt;</span> <span class="token operator">&amp;</span>proc<span class="token punctuation">[</span>NPROC<span class="token punctuation">]</span><span class="token punctuation">;</span> q<span class="token operator">++</span><span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
...
<div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c"><pre v-pre><code><span class="line"><span class="token keyword">void</span> <span class="token operator">*</span>s <span class="token operator">=</span> <span class="token function">sbrk</span><span class="token punctuation">(</span>PGSIZE<span class="token punctuation">)</span><span class="token punctuation">;</span></span>
...
<div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c"><pre v-pre><code><span class="line"><span class="token keyword">void</span> <span class="token operator">*</span>s <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span></span>
...
<div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c"><pre v-pre><code><span class="line"><span class="token keyword">int</span> <span class="token function">thread_create</span><span class="token punctuation">(</span><span class="token keyword">void</span> <span class="token punctuation">(</span><span class="token operator">*</span>fcn<span class="token punctuation">)</span><span class="token punctuation">(</span><span class="token keyword">void</span><span class="token operator">*</span><span class="token punctuation">,</span> <span class="token keyword">void</span><span class="token operator">*</span><span class="token punctuation">)</span><span class="token punctuation">,</span> <span class="token keyword">void</span> <span class="token operator">*</span>arg1<span class="token punctuation">,</span> <span class="token keyword">void</span> <span class="token operator">*</span>arg2<span class="token punctuation">)</span><span class="token punctuation">;</span></span>
...
<div class="language-makefile line-numbers-mode" data-highlighter="prismjs" data-ext="makefile"><pre v-pre><code><span class="line"><span class="token target symbol"><span class="token variable">$U/_thread_test</span></span><span class="token punctuation">:</span> <span class="token variable">$U/thread_test.o</span> <span class="token variable">$U/thread.o</span> <span class="token variable">$</span><span class="token punctuation">(</span>ULIB<span class="token punctuation">)</span></span>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div></div></div><ul>
<li>별도로 <code v-pre>thread.o</code>를 링크에 포함해야 thread 라이브러리가 연결된다.</li>
</ul>
<div class="language-asm line-numbers-mode" data-highlighter="prismjs" data-ext="asm"><pre v-pre><code><span class="line">entry(&quot;clone&quot;);</span>
...
<tbody>
<tr>
<td><code v-pre>clone()</code></td>
<td>새로운 TCB(proc), 공유된 주소 공간, 독립된 trapframe 생성</td>
</tr>
<tr>
<td><code v-pre>join()</code></td>
<td>종료된 thread 회수 및 stack 주소 반환</td>
</tr>
<tr>
<td><code v-pre>exec()</code></td>
<td>기존 thread 정리 후 새로운 주소 공간 로딩</td>
</tr>
<tr>
<td><code v-pre>kill()</code></td>
<td>thread 포함 모든 실행 컨텍스트 종료</td>
</tr>
<tr>
<td><code v-pre>thread_create()</code></td>
<td>스택 할당 + clone 시스템 콜 래핑</td>
</tr>
<tr>
<td><code v-pre>thread_join()</code></td>
<td><code v-pre>join</code> 호출 후 스택 반납</td>
</tr>
</tbody>
</table>
...
<li>부모 thread가 자신의 자식 thread를 실행시켜 병렬적으로 처리</li>
...
<li>thread 0에서 <code v-pre>sbrk()</code>로 메모리 확장</li>
<li>각 thread는 공유된 주소 공간에 대한 접근 시도</li>
...
<li>모든 thread가 동일한 <code v-pre>PID(29)</code>로 출력됨</li>
...
<li><code v-pre>thread 0</code>이 <code v-pre>exec()</code>을 호출하여 새로운 프로그램으로 변환</li>
<li>다른 thread들은 예상대로 비정상 종료 (scause <code v-pre>0x2</code> / <code v-pre>0xd</code>)</li>
...
<li>xv6에 커널 수준의 thread를 안정적으로 도입 및 검증 완료</li>
<li>주요 기능(생성, 종료, 메모리 공유, PID 일치, <code v-pre>exec</code> 처리 등) 전반적으로 기대 동작 수행</li>
<li>수업에서 제공된 예시 출력과 실행 순서 또는 메시지가 일부 다르지만, 이는 xv6의 scheduler 특성상 자연스러운 차이이다.</li>
<li>모든 test의 핵심 기능은 정확히 구현되었으며, trap 발생 또한 메모리 보호 기법이 올바르게 작동했음을 의미한다.</li>
...
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code><span class="line">usertrap(): unexpected scause 0x2 pid=36 sepc=0x1000 stval=0xdeadbeef</span>
<span class="line"></span></code></pre>
...
<li><code v-pre>SLEEPING</code> 상태일 경우, 강제로 <code v-pre>RUNNABLE</code>로 만들어 <code v-pre>sched()</code>에 진입할 수 있도록 함</li>
</ul>
<div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c"><pre v-pre><code><span class="line"><span class="token keyword">for</span> <span class="token punctuation">(</span>p <span class="token operator">=</span> proc<span class="token punctuation">;</span> p <span class="token operator">&lt;</span> <span class="token operator">&amp;</span>proc<span class="token punctuation">[</span>NPROC<span class="token punctuation">]</span><span class="token punctuation">;</span> p<span class="token operator">++</span><span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
<span class="line">  <span class="token keyword">if</span> <span class="token punctuation">(</span>p<span class="token operator">-></span>pagetable <span class="token operator">==</span> target<span class="token operator">-></span>pagetable<span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
<span class="line">    p<span class="token operator">-></span>killed <span class="token operator">=</span> <span class="token number">1</span><span class="token punctuation">;</span></span>
...
<li>stack 주소가 0이 아닌 경우에만 해제</li>
</ul>
<div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c"><pre v-pre><code><span class="line"><span class="token keyword">if</span> <span class="token punctuation">(</span>pid <span class="token operator">></span> <span class="token number">0</span> <span class="token operator">&amp;&amp;</span> s<span class="token punctuation">)</span> <span class="token function">sbrk</span><span class="token punctuation">(</span><span class="token operator">-</span>PGSIZE<span class="token punctuation">)</span><span class="token punctuation">;</span></span>
<span class="line"></span></code></pre>
...
<li><code v-pre>sleep()</code> 내부에서 <code v-pre>p-&gt;lock</code>을 획득하며, 외부에서는 <code v-pre>wait_lock</code>만 보유해야 함</li>
</ul>
<div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c"><pre v-pre><code><span class="line"><span class="token function">acquire</span><span class="token punctuation">(</span><span class="token operator">&amp;</span>p<span class="token operator">-></span>lock<span class="token punctuation">)</span><span class="token punctuation">;</span></span>
...
<tr>
<td><code v-pre>thread_create</code></td>
<td><code v-pre>sbrk</code> 실패 또는 misalignment</td>
<td>실패 시 바로 <code v-pre>return</code></td>
</tr>
<tr>
<td><code v-pre>clone</code></td>
<td><code v-pre>fcn == 0</code> 주소 거부</td>
<td><code v-pre>fcn</code> null-check 제거, <code v-pre>walkaddr</code>만으로 유효성 확인</td>
</tr>
<tr>
<td><code v-pre>join</code></td>
<td>lock 보호 미흡</td>
<td>lock 순서 및 <code v-pre>freeproc()</code> 내부 동기화 보강</td>
</tr>
<tr>
<td><code v-pre>exec</code></td>
<td>다른 thread가 trap 발생</td>
<td>동일 pagetable 공유 thread 전부 제거</td>
</tr>
<tr>
<td><code v-pre>kill</code></td>
<td>일부 thread만 종료됨</td>
<td>pagetable 기준 전체 <code v-pre>killed</code> 처리</td>
</tr>
<tr>
<td><code v-pre>thread_join</code></td>
<td>stack 회수 누락</td>
<td>내부에서 <code v-pre>sbrk(-PGSIZE)</code> 호출 추가</td>
</tr>
<tr>
<td><code v-pre>sleep/wakeup</code></td>
<td>deadlock 또는 race condition</td>
<td>lock 순서 및 <code v-pre>wakeup</code> 시점 조정</td>
</tr>
</tbody>
</table>
...
<ul>
<li>
<p>본 project는 <strong>xv6 RISC-V 버전</strong>을 기반으로 하며, x86 기반 구현 예제들과는 구조적으로 다르다.</p>
<ul>
<li>특히 <code v-pre>ptable[]</code>이 존재하지 않고, <code v-pre>proc[NPROC]</code> 배열과 spinlock으로만 동기화를 수행한다.</li>
<li>따라서 x86의 <code v-pre>switchuvm()</code>, <code v-pre>loaduvm()</code> 등의 동작 방식과 달리, RISC-V에서는 <code v-pre>walk()</code> 및 <code v-pre>pagetable_t</code>에 직접 접근하는 방식이 필요했다.</li>
</ul>
</li>
<li>
<p>xv6의 trapframe, context, scheduler 등은 thread-safe하지 않으므로, thread마다 반드시 독립된 <code v-pre>trapframe</code>, <code v-pre>context</code>, <code v-pre>ustack</code>을 사용해야 한다.</p>
</li>
<li>
<p><code v-pre>uvmshare()</code>는 기존 <code v-pre>uvmcopy()</code>를 변형하여 구현한 함수로, <code v-pre>PTE_U</code> 비트가 설정된 사용자 영역 page에 대해서만 공유(reference mapping)하도록 설계하였다.</p>
<ul>
<li>이는 read-only 세그먼트에 대해서도 문제가 없으며, exec이나 fork와는 다른 동작을 보장한다.</li>
</ul>
</li>
<li>
<p><code v-pre>thread_fcn.c</code>, <code v-pre>thread_test.c</code>는 과제 명세에서 제공한 채점 기준 파일이다.</p>
<ul>
<li>이 파일의 구조나 <code v-pre>fcn = 0x0</code> 형태의 호출 방식은 정상 동작이며, panic 유발 요소가 아니다.</li>
</ul>
</li>
<li>
<p>모든 thread 구현은 xv6의 기존 기능과 완전히 호환되어야 하며, 특히 다음과 같은 system call들과 연동 가능해야 한다.</p>
<ul>
<li><code v-pre>fork()</code>, <code v-pre>exec()</code>, <code v-pre>exit()</code>, <code v-pre>wait()</code>, <code v-pre>kill()</code>, <code v-pre>sleep()</code>, <code v-pre>pipe()</code></li>
</ul>
</li>
</ul>
...
<p>실습 중 참고한 StackOverflow thread:</p>
<ul>
