...
<ul>
<li>
<p>가상 메모리: 논리 메모리와 물리 메모리를 분리하는 구조</p>
</li>
<li>
<p>장점:</p>
<ul>
<li>프로그램의 일부분만 메모리에 있어도 실행 가능</li>
<li>논리 주소 공간은 물리 주소 공간보다 훨씬 클 수 있음</li>
<li>여러 프로세스가 주소 공간 공유 가능</li>
<li>프로세스 생성을 더 효율적으로 수행 가능</li>
<li>페이지를 스왑인/스왑아웃 할 수 있어야 함</li>
</ul>
</li>
<li>
<p>구현 방식:</p>
...
<ul>
<li>
<p>페이지가 <strong>필요할 때만</strong> 메모리에 불러옴</p>
<ul>
<li>I/O 감소</li>
<li>메모리 절약</li>
<li>반응 속도 향상</li>
...
<li>
<p><strong>Lazy swapper</strong>:</p>
<ul>
<li>필요한 순간까지 페이지를 불러오지 않음</li>
<li>이 기능을 수행하는 모듈을 <strong>pager</strong>라고 부름</li>
</ul>
</li>
</ul>
...
<ul>
<li>
<p>각 페이지 테이블 항목에는 valid-invalid 비트가 있음</p>
</li>
<li>
<p><strong>Valid (V)</strong>: 메모리에 있음</p>
</li>
<li>
<p><strong>Invalid (I)</strong>:</p>
<ul>
<li>
<p>해당 페이지가 프로세스 주소 공간에 없음 (illegal)</p>
</li>
<li>
<p>디스크에서 아직 로딩되지 않음 (not-in-memory)</p>
</li>
<li>
<p>디스크에서는 유효하지만 메모리에는 없음 (obsolete)</p>
</li>
<li>
<p>예: 항공 예약 시스템</p>
<ul>
<li>하나의 전역 디스크 + 여러 컴퓨터/지점</li>
</ul>
</li>
</ul>
</li>
<li>
<p>모든 항목은 초기에는 invalid 상태로 설정</p>
</li>
<li>
<p>주소 변환 중, invalid이면 ⇒ <strong>page fault</strong></p>
</li>
</ul>
...
<ul>
<li>page table에는 valid-invalid 비트가 존재</li>
<li>물리 메모리에 없는 페이지는 디스크에 저장됨</li>
<li>접근 시 <strong>page fault</strong> 발생</li>
</ul>
</li>
</ul>
...
<ul>
<li>
<p>유효하지 않은 페이지 접근 → MMU가 trap 발생 → <strong>page fault trap</strong></p>
</li>
<li>
<p>Trap handler는 OS 내부에 구현되어 있음</p>
</li>
<li>
<p>OS가 page fault를 처리하는 순서:</p>
<ol>
<li>OS가 다른 테이블을 조회하여 원인을 결정
<ul>
<li>illegal reference? bad address? protection violation?</li>
<li>메모리에 없는 경우 → 계속 진행</li>
</ul>
</li>
<li><strong>빈 프레임 확보</strong>
<ul>
<li>없으면 교체 (replacement)</li>
</ul>
</li>
<li><strong>디스크에서 페이지를 읽어 옴</strong>
<ul>
<li>디스크 I/O 완료까지 프로세스는 wait 상태</li>
<li>완료 후 page table 갱신 (frame #, valid bit)</li>
<li>프로세스를 Ready 큐에 넣고 dispatch</li>
</ul>
</li>
<li>페이지 fault 처리 완료 → 프로세스 재할당</li>
<li><strong>fault가 발생했던 명령 재실행</strong></li>
</ol>
</li>
</ul>
...
<li>두 번째 블록 접근 중 fault 발생하면 이전 작업 <strong>Undo</strong> 필요</li>
</ul>
</li>
</ul>
</li>
...
<ul>
<li>
<p>페이지 폴트 확률 <span v-pre class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mn>0</mn><mo>≤</mo><mi>p</mi><mo>≤</mo><mn>1.0</mn></mrow><annotation encoding="application/x-tex">0 \leq p \leq 1.0</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:0.7804em;vertical-align:-0.136em;"></span><span class="mord">0</span><span class="mspace" style="margin-right:0.2778em;"></span><span class="mrel">≤</span><span class="mspace" style="margin-right:0.2778em;"></span></span><span class="base"><span class="strut" style="height:0.8304em;vertical-align:-0.1944em;"></span><span class="mord mathnormal">p</span><span class="mspace" style="margin-right:0.2778em;"></span><span class="mrel">≤</span><span class="mspace" style="margin-right:0.2778em;"></span></span><span class="base"><span class="strut" style="height:0.6444em;"></span><span class="mord">1.0</span></span></span></span></p>
<ul>
<li><span v-pre class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mi>p</mi><mo>=</mo><mn>0</mn></mrow><annotation encoding="application/x-tex">p = 0</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:0.625em;vertical-align:-0.1944em;"></span><span class="mord mathnormal">p</span><span class="mspace" style="margin-right:0.2778em;"></span><span class="mrel">=</span><span class="mspace" style="margin-right:0.2778em;"></span></span><span class="base"><span class="strut" style="height:0.6444em;"></span><span class="mord">0</span></span></span></span> → page fault 없음</li>
<li><span v-pre class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mi>p</mi><mo>=</mo><mn>1</mn></mrow><annotation encoding="application/x-tex">p = 1</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:0.625em;vertical-align:-0.1944em;"></span><span class="mord mathnormal">p</span><span class="mspace" style="margin-right:0.2778em;"></span><span class="mrel">=</span><span class="mspace" style="margin-right:0.2778em;"></span></span><span class="base"><span class="strut" style="height:0.6444em;"></span><span class="mord">1</span></span></span></span> → 모든 참조가 fault</li>
</ul>
</li>
<li>
<p><strong>유효 접근 시간 (Effective Access Time, EAT)</strong> 계산:</p>
<p v-pre class='katex-block'><span class="katex-display"><span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML" display="block"><semantics><mrow><mi>E</mi><mi>A</mi><mi>T</mi><mo>=</mo><mo stretchy="false">(</mo><mn>1</mn><mo>−</mo><mi>p</mi><mo stretchy="false">)</mo><mo>×</mo><mtext>memory access time</mtext><mo>+</mo><mi>p</mi><mo>×</mo><mo stretchy="false">(</mo><mtext>page fault overhead</mtext><mo stretchy="false">)</mo></mrow><annotation encoding="application/x-tex">EAT = (1 - p) \times \text{memory access time} + p \times (\text{page fault overhead})
...
<ul>
<li>
<p><strong>Page replacement</strong></p>
<ul>
<li>page-fault 처리 루틴을 수정하여 프레임이 부족할 때 page replacement 수행</li>
<li><strong>modify (dirty) bit</strong>를 사용하여 디스크 전송 비용 최소화 (수정된 페이지만 디스크로 swap-out)</li>
<li>논리 메모리와 물리 메모리의 분리를 완성
<ul>
<li>더 작은 물리 메모리로 큰 가상 메모리 제공 가능</li>
<li>같은 페이지가 실행 중 여러 번 메모리에 올라올 수 있음</li>
</ul>
</li>
</ul>
</li>
...
<ul>
<li>victim page(교체 대상 페이지)를 선택하는 알고리즘</li>
<li><strong>목표: page fault 수 최소화</strong></li>
</ul>
</li>
</ul>
...
<li><strong>빈 프레임 찾기</strong>
<ul>
<li>빈 프레임이 있으면 → 그대로 사용</li>
<li>없으면 → page replacement 알고리즘으로 <strong>victim frame</strong> 선택</li>
</ul>
</li>
...
<ul>
<li>
<p><strong>최소 page-fault 비율</strong>을 갖는 알고리즘이 이상적</p>
</li>
<li>
<p>알고리즘을 특정 <strong>reference string</strong>에 대해 실행하여 성능 평가</p>
</li>
<li>
...
<ul>
<li>
<p><strong>가장 오랫동안 사용되지 않을 페이지</strong>를 교체</p>
</li>
<li>
<p>예시:</p>
</li>
...
<li>LRU를 그대로 구현하려면:
<ul>
<li>각 페이지에 타임스탬프 필요 → page table traffic 증가</li>
<li>가장 오래된 타임스탬프를 찾는 연산 필요</li>
<li>커널에 적용하기에는 공간/시간 오버헤드가 큼</li>
</ul>
</li>
<li>→ <strong>근사 모델</strong> 필요</li>
</ul>
</li>
</ul>
...
<ul>
<li>
<p><strong>카운터 기반 구현</strong>:</p>
<ul>
<li>각 페이지마다 카운터 유지</li>
<li>CPU가 메모리 참조마다 카운터 증가</li>
<li>페이지 참조 시 현재 카운터 값을 저장</li>
<li>교체 시 가장 작은 카운터 값을 가진 페이지를 선택</li>
<li>단점:
<ul>
<li>매 접근 시 카운터 접근 비용</li>
<li>교체 시 전체 검색 필요</li>
<li>오버헤드 큼</li>
</ul>
</li>
...
<li>
<p><strong>스택 기반 구현</strong>:</p>
<ul>
<li>페이지 번호를 <strong>이중 연결 리스트(stack)</strong> 형태로 유지</li>
<li>참조 시 해당 페이지를 top으로 이동</li>
<li>이동 시 포인터 6개 수정 필요</li>
<li>교체 시 검색 불필요</li>
</ul>
</li>
</ul>
...
<ul>
<li>
<p><strong>Reference bit 방식</strong>:</p>
<ul>
<li>각 페이지에 1비트 레퍼런스 비트 할당 (초기값 0)</li>
<li>페이지가 참조되면 비트 = 1</li>
<li>교체 시 reference bit가 0인 페이지를 선택</li>
</ul>
</li>
<li>
<p><strong>추가 참조 비트 방식</strong>:</p>
<ul>
<li>각 페이지에 8비트 할당</li>
<li>일정 주기마다 reference bit를 가장 높은 비트로 시프트</li>
<li>나머지 비트는 오른쪽으로 쉬프트 (하위 비트 삭제)</li>
<li>예:
<ul>
<li><code v-pre>00000000</code>: 참조 안 됨</li>
<li><code v-pre>10101010</code>: 2주기마다 참조됨</li>
</ul>
</li>
</ul>
</li>
...
<ul>
<li>
<p><strong>Second-Chance (Clock) 알고리즘</strong></p>
<ul>
<li>reference bit 사용</li>
<li>원형 큐로 페이지 유지</li>
<li>포인터가 reference bit가 0인 페이지를 찾을 때까지 이동</li>
<li>reference bit가 1이면 0으로 리셋하고 다음 페이지로 이동</li>
<li>사실상 FIFO와 유사하지만 참조된 페이지에 기회 한 번 더 줌</li>
</ul>
</li>
<li>
<p>특징:</p>
<ul>
<li>포인터는 순환적으로 회전</li>
<li>1이면 bit만 0으로 바꾸고 지나침</li>
<li>0이면 victim으로 선택</li>
</ul>
</li>
<li>
<p><strong>향상된 Second-Chance 알고리즘</strong></p>
<ul>
<li>Reference bit + Modify bit 조합</li>
<li>우선순위:
<ul>
<li>Not-Referenced + Not-Modified: 최우선으로 교체</li>
<li>Referenced + Modified: 마지막 우선순위</li>
</ul>
</li>
</ul>
</li>
...
<ul>
<li>
<p>각 페이지에 대해 <strong>참조 횟수 카운터</strong>를 유지함</p>
</li>
<li>
<p><strong>LFU (Least Frequently Used) 알고리즘</strong></p>
<ul>
<li>참조 횟수가 가장 적은 페이지를 교체</li>
</ul>
</li>
<li>
<p><strong>MFU (Most Frequently Used) 알고리즘</strong></p>
<ul>
<li>가장 적게 참조된 페이지는 <strong>최근에 불러온 후 아직 사용되지 않았을 가능성</strong>이 높다는 가정에 기반하여,</li>
<li>참조 횟수가 가장 많은 페이지를 유지하고, 가장 적은 페이지를 유지하지 않음</li>
</ul>
</li>
</ul>
<hr>
<h1 id="virtual-memory-page-replacement-교수님-강의-요약" tabindex="-1"><a class="header-anchor" href="#virtual-memory-page-replacement-교수님-강의-요약"><span>Virtual Memory &amp; Page Replacement - 교수님 강의 요약</span></a></h1>
...
<li>우선순위 기반 정책(priority allocation): 우선순위가 높은 프로세스에 프레임을 더 많이 할당
<ul>
<li>하지만 우선순위가 높다고 무조건 많이 줄 필요는 없음</li>
</ul>
</li>
<li>중요한 목표는 <strong>page fault</strong>를 줄이는 것
<ul>
<li>자주 접근하는 페이지들을 식별해서 해당 프레임 수만큼만 할당해주는 것이 핵심</li>
</ul>
</li>
</ul>
...
<li>일정 시간 동안 참조된 페이지들의 집합을 기준으로 판단</li>
...
<li>I/O 대기 시간이 급격히 증가하며 CPU utilization은 감소</li>
<li>새로운 프로세스를 실행시키면 기존 프로세스의 프레임을 빼앗아야 해서 악화됨</li>
<li>해결 방법:
<ul>
<li>working set이 메모리에 항상 유지되도록 보장</li>
<li>또는 일부 프로세스를 suspend</li>
</ul>
</li>
</ul>
...
<h1 id="global-vs-local-replacement" tabindex="-1"><a class="header-anchor" href="#global-vs-local-replacement"><span>Global vs Local Replacement</span></a></h1>
<ul>
...
<h1 id="copy-on-write" tabindex="-1"><a class="header-anchor" href="#copy-on-write"><span>Copy-On-Write</span></a></h1>
<ul>
<li>부모와 자식 프로세스가 동일한 페이지를 공유</li>
<li>write가 발생할 경우 해당 페이지를 복사하여 각각 소유하도록 함</li>
<li>xv6에서는 <code v-pre>rsw</code> 비트를 활용하여 COW 여부 표시</li>
<li>page fault를 통해 write 시점에 새로 할당</li>
</ul>
<h1 id="memory-mapped-files" tabindex="-1"><a class="header-anchor" href="#memory-mapped-files"><span>Memory-Mapped Files</span></a></h1>
<ul>
<li>파일 내용을 메모리 주소에 직접 매핑</li>
<li>I/O 대신 메모리 연산으로 간접 접근 가능</li>
<li>성능 이점:
<ul>
<li>첫 접근 시 page fault를 통해 메모리에 로딩</li>
<li>이후 연산은 메모리 접근으로 수행되어 빠름</li>
<li>디스크 I/O 횟수 감소</li>
</ul>
</li>
</ul>
<h1 id="page-size-vs-tlb-reach" tabindex="-1"><a class="header-anchor" href="#page-size-vs-tlb-reach"><span>Page Size vs TLB Reach</span></a></h1>
<ul>
<li>TLB Reach: TLB가 커버할 수 있는 전체 메모리 공간</li>
<li>page size가 커지면 TLB Reach 증가</li>
<li>다단계 페이지 테이블의 depth 감소</li>
<li>단점:
<ul>
<li>내부 단편화(internal fragmentation)</li>
<li>불필요한 데이터까지 로딩될 수 있음</li>
</ul>
</li>
<li>장점:
<ul>
<li>Disk I/O 효율 증가</li>
<li>저장장치 접근 성능 향상</li>
</ul>
</li>
</ul>
<h1 id="xv6-disk-layer-buffer-cache-logging" tabindex="-1"><a class="header-anchor" href="#xv6-disk-layer-buffer-cache-logging"><span>xv6 Disk Layer, Buffer Cache, Logging</span></a></h1>
...
<li>구조체로 관리됨 (<code v-pre>bcache</code>)</li>
...
<li>crash 대비 복구 가능</li>
<li>commit 시:
<ul>
<li>dirty block들을 log에 먼저 작성</li>
<li>이후 실제 위치에 덮어씀</li>
<li>log header 지움</li>
</ul>
</li>
...
<ul>
<li>log에 쓸 작업 단위</li>
<li><code v-pre>begin_op</code> → 작업 수행 → <code v-pre>end_op</code> → <code v-pre>commit</code></li>
<li>commit 지점 기준으로 복구 여부 결정</li>
<li>group commit 방식으로 효율성 확보</li>
</ul>
<hr>
<h1 id="project-3-과제-안내" tabindex="-1"><a class="header-anchor" href="#project-3-과제-안내"><span>Project 3 과제 안내</span></a></h1>
...
<li>write 발생 시 page fault를 통해 개별 복사</li>
...
<li><code v-pre>T_SYMLINK</code> 타입 추가</li>
<li><code v-pre>symlink</code> system call 구현</li>
<li>open 시 경로 참조</li>
<li>무한 루프 방지를 위한 cycle check 필요</li>
<li>실제 대상 파일이 없어도 링크 생성 가능</li>
</ul>
<h1 id="기타-이슈" tabindex="-1"><a class="header-anchor" href="#기타-이슈"><span>기타 이슈</span></a></h1>
