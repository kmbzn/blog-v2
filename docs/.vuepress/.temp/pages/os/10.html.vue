...
<li><strong>Valid (V)</strong>: 메모리에 있음</li>
...
<li>Trap handler는 OS 내부에 구현되어 있음</li>
<li>OS가 page fault를 처리하는 순서:
<ol>
<li>OS가 다른 테이블을 조회하여 원인을 결정</li>
</ol>
<ul>
...
<ol start="2">
<li><strong>빈 프레임 확보</strong></li>
</ol>
<ul>
<li>없으면 교체 (replacement)</li>
</ul>
<ol start="3">
<li><strong>디스크에서 페이지를 읽어 옴</strong></li>
</ol>
<ul>
...
<li><strong>최악의 경우</strong>: 명령어가 여러 위치를 갱신할 때</li>
</ol>
<ul>
...
<li><strong>유효 접근 시간 (Effective Access Time, EAT)</strong> 계산:<p v-pre class='katex-block'><span class="katex-display"><span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML" display="block"><semantics><mrow><mi>E</mi><mi>A</mi><mi>T</mi><mo>=</mo><mo stretchy="false">(</mo><mn>1</mn><mo>−</mo><mi>p</mi><mo stretchy="false">)</mo><mo>×</mo><mtext>memory access time</mtext><mo>+</mo><mi>p</mi><mo>×</mo><mo stretchy="false">(</mo><mtext>page fault overhead</mtext><mo stretchy="false">)</mo></mrow><annotation encoding="application/x-tex">EAT = (1 - p) \times \text{memory access time} + p \times (\text{page fault overhead})
...
<li><strong>순수한 demand paging</strong>:
<ul>
...
<li>알고리즘을 특정 <strong>reference string</strong>에 대해 실행하여 성능 평가</li>
<li>예시 reference string:</li>
</ul>
...
<li><strong>향상된 Second-Chance 알고리즘</strong>
<ul>
...
<li><strong>LFU (Least Frequently Used) 알고리즘</strong>
<ul>
<li>참조 횟수가 가장 적은 페이지를 교체</li>
</ul>
</li>
<li><strong>MFU (Most Frequently Used) 알고리즘</strong>
<ul>
