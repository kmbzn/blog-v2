...
<li>Type – 시스템이 다양한 파일 유형을 지원하기 위해 필요</li>
<li>Location – 디바이스 내 파일 위치를 가리키는 포인터</li>
<li>Size – 현재 파일 크기</li>
<li>Protection – 읽기, 쓰기, 실행 권한 제어</li>
...
<li>대부분의 시스템은 다단계 디렉토리 구조를 지원<br>
→ 파일 이름은 루트부터 해당 파일까지의 경로를 포함함</li>
<li>대부분의 시스템은 현재 디렉토리(current directory)를 가짐<br>
...
<li>이 정보는 디스크의 자료구조에 저장됨
<ul>
<li>→ 따라서 일부 제한이 존재할 수 있음</li>
</ul>
</li>
<li>운영체제는 최근 접근한 파일들의 디렉토리 항목을 메모리에 캐시함
<ul>
...
<li>그룹화(grouping) 가능</li>
<li>홈 디렉토리 (home directory)</li>
<li>현재 디렉토리 (current working directory)</li>
<li>절대 경로, 상대 경로 모두 사용 가능
<ul>
<li>예: <code v-pre>cd /skyang/ch1</code></li>
</ul>
</li>
<li>검색 경로 (command interpreter)
<ul>
...
<li>순환이 없는 구조만 허용됨 (acyclic)</li>
<li>파일/디렉토리 공유 가능</li>
<li>동일한 하위 디렉토리나 파일이 둘 이상의 디렉토리에 존재할 수 있음
<ul>
<li>문제
<ol>
...
<li>순회(traverse), 삭제(delete) 알고리즘 복잡</li>
<li>순회 문제
<ul>
...
<li>링크 생성 시마다 cycle 탐지 알고리즘을 통해 허용 여부 판단</li>
</ul>
</li>
</ul>
...
<li>계층 구조로 구성됨</li>
<li><code v-pre>open()</code> 시스템 호출
<ul>
<li>디스크에서 해당 파일의 메타데이터를 로드
<ul>
<li>디렉토리 탐색이 필요</li>
</ul>
</li>
...
<li>블록들은 디스크 어디에나 흩어져 있을 수 있음</li>
<li>각 블록은 다음을 포함
<ul>
...
<li>변형 형태: FAT (File Allocation Table)
<ul>
...
<li>인덱스 블록은 디스크 블록에 대한 포인터 배열</li>
<li>직접 접근 속도가 향상됨</li>
...
<li>
<p>Virtual disk (RAM disk)</p>
<ul>
<li>전용 메모리 영역 할당을 통해 PC 성능 향상</li>
</ul>
</li>
<li>
<p>Directory entry cache (dentry cache)</p>
<ul>
...
<li>요약
<ul>
<li>“File System”은 디스크 상의 데이터 구조</li>
<li>“logical formatting”은 이 데이터 구조를 초기화하는 작업</li>
<li>디스크 파일 접근은 운영체제에 따라 동작이 달라짐</li>
</ul>
</li>
</ul>
</div></template>
