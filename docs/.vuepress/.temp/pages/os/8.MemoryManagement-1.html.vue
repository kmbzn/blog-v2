...
<li>main memory와 register만이 CPU가 직접 접근 가능한 저장 장치임</li>
<li>register 접근은 CPU clock cycle 1회 (또는 그 이하)</li>
<li>main memory 접근은 여러 cycle이 소요될 수 있음</li>
...
<blockquote>
<p>source program<br>
↓ (compiler 또는 assembler)<br>
object module<br>
↓ (linker 또는 linkage editor)<br>
...
<ul>
<li>
<p>논리 주소 공간(logical address space) 개념은 물리 주소 공간(physical address space)과 분리되어 있으며, 이는 적절한 메모리 관리의 핵심임</p>
</li>
<li>
<p>Logical address</p>
<ul>
<li>CPU에 의해 생성됨</li>
<li>virtual address라고도 불림</li>
</ul>
</li>
<li>
<p>Physical address</p>
<ul>
<li>메모리 장치에 의해 인식되는 주소</li>
</ul>
</li>
<li>
<p>논리 주소와 물리 주소는 컴파일 시간과 적재 시간 바인딩에서는 동일함</p>
</li>
<li>
<p>실행 시간 바인딩에서는 서로 다름</p>
</li>
</ul>
...
<ul>
<li>논리 주소를 물리 주소로 변환해주는 하드웨어 장치</li>
</ul>
</li>
<li>
<p>MMU 방식</p>
<ul>
<li>relocation register의 값을 CPU에서 전달된 모든 주소에 더한 후 메모리에 보냄</li>
</ul>
</li>
</ul>
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code class="language-text"><span class="line">  [ CPU ]</span>
...
<ul>
<li>프로세스는 일시적으로 메모리에서 backing store로 스왑됨</li>
<li>이후 다시 메모리로 복귀됨</li>
</ul>
</li>
<li>
<p><strong>Backing store</strong></p>
<ul>
<li>모든 사용자 메모리 이미지를 저장할 수 있을 만큼 큰 고속 디스크</li>
<li>사용자들이 직접 접근할 수 있어야 함</li>
</ul>
</li>
<li>
<p>스왑에서 가장 큰 비중을 차지하는 것은 전송 시간</p>
<ul>
<li>총 전송 시간은 스왑되는 메모리 양에 비례함</li>
</ul>
</li>
</ul>
...
<ul>
<li>
<p>주기억장치는 일반적으로 두 파티션으로 나뉨:</p>
<ul>
<li>Resident OS: interrupt vector가 있는 low memory에 위치</li>
<li>User process는 high memory에 위치</li>
</ul>
</li>
<li>
<p>사용자 프로세스를 보호하기 위해:</p>
<ul>
<li><strong>Relocation register</strong>: 가장 작은 물리 주소 값을 가짐</li>
<li><strong>Limit register</strong>: 논리 주소 범위를 가짐
<ul>
<li>각 논리 주소는 limit register로 경계 설정됨</li>
</ul>
</li>
</ul>
</li>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="contiguous-allocation-cont" tabindex="-1"><a class="header-anchor" href="#contiguous-allocation-cont"><span>Contiguous Allocation (Cont.)</span></a></h2>
<ul>
<li>Hole: 사용 가능한 메모리 블록
<ul>
<li>다양한 크기의 hole(빈 공간)은 메모리 전반에 흩어져 있음</li>
<li>프로세스가 도착하면, 해당 요청을 수용할 수 있는 크기의 hole에서 메모리가 할당됨</li>
</ul>
</li>
<li>운영체제는 다음 정보를 유지함:
<ul>
...
<ul>
<li>
<p>요청 크기 <code v-pre>n</code>을 충족시키기 위해 hole 리스트에서 어떻게 할당할 것인가</p>
</li>
<li>
<p><strong>First-fit</strong></p>
<ul>
<li>충분히 큰 <strong>첫 번째 hole</strong>에 할당</li>
</ul>
</li>
<li>
<p><strong>Best-fit</strong></p>
<ul>
<li>충분히 큰 <strong>가장 작은 hole</strong>에 할당</li>
<li>전체 리스트를 탐색해야 함 (정렬된 경우 예외)</li>
<li><strong>많은 작은 leftover hole</strong>을 생성함</li>
</ul>
</li>
<li>
<p><strong>Worst-fit</strong></p>
<ul>
<li><strong>가장 큰 hole</strong>에 할당</li>
<li>역시 전체 리스트를 탐색해야 함</li>
<li><strong>가장 큰 leftover hole</strong>을 생성</li>
</ul>
</li>
</ul>
<blockquote>
...
<ul>
<li>전체 메모리 공간은 요청을 만족할 수 있을 만큼 있지만, <strong>연속적이지 않음</strong></li>
</ul>
</li>
<li>
<p><strong>Internal fragmentation</strong></p>
<ul>
<li>할당된 메모리는 실제 요청보다 약간 클 수 있음 → 이 크기 차이가 <strong>internal fragmentation</strong> (파티션 내부 메모리이나 사용되지 않음)</li>
</ul>
</li>
<li>
<p>외부 단편화를 줄이는 방법: <strong>compaction</strong></p>
<ul>
<li>메모리 내 내용을 섞어 모든 빈 공간을 하나의 큰 블록으로 만듦</li>
<li>단, relocation이 <strong>동적으로 가능하고 실행 시간에 수행될 경우</strong>에만 compaction 가능</li>
</ul>
</li>
</ul>
...
<ul>
<li>주소 공간을 <strong>비연속적으로 만들 수 있게 하는</strong> 스킴</li>
</ul>
</li>
<li>
<p>기본 방식</p>
<ul>
<li>물리 메모리를 고정 크기 블록(frame) 으로 나눔<br>
(크기는 2의 제곱수, 512B ~ 8MB)</li>
<li>논리 메모리를 같은 크기의 블록(page) 으로 나눔</li>
<li>모든 frame의 상태를 추적</li>
<li>프로그램 실행 시, <code v-pre>n</code>개의 페이지에 대해 <code v-pre>n</code>개의 빈 frame과 page table이 필요</li>
<li><strong>페이지 테이블</strong>을 통해 논리 주소를 물리 주소로 변환</li>
<li><strong>내부 단편화는 있으나 외부 단편화는 없음</strong></li>
</ul>
</li>
</ul>
...
<li>
<p>Page number (p)<br>
→ 페이지 테이블에서 물리 메모리의 각 페이지에 해당하는 base address를 찾는 인덱스로 사용됨</p>
</li>
<li>
<p>Page offset (d)<br>
→ base address에 더해 실제 메모리 접근 주소를 결정</p>
</li>
</ul>
</li>
</ul>
...
<ul>
<li>
<p>Page Number = 2 / 4 = 0</p>
</li>
<li>
<p>Offset = 2 % 4 = 2</p>
</li>
<li>
<p>Logical address (m): 4비트</p>
</li>
<li>
<p>Page number (m-n): 2비트</p>
</li>
<li>
<p>Offset (n): 2비트</p>
</li>
<li>
<p><strong>4바이트 페이지 → n = 2비트</strong></p>
</li>
</ul>
...
<ul>
<li>
<p>페이지 테이블은 <strong>주기억장치에 저장됨</strong></p>
<ul>
<li>Page-table base register (PTBR): 페이지 테이블을 가리킴</li>
<li>Page-table length register (PTLR): 테이블의 크기를 나타냄</li>
</ul>
</li>
<li>
<p><strong>문제점</strong>: 메모리 접근마다 <strong>두 번의 접근</strong>이 필요함</p>
<ul>
<li>하나는 페이지 테이블, 다른 하나는 데이터/명령어 접근</li>
</ul>
</li>
<li>
<p><strong>해결책</strong>: <strong>TLB</strong>(Translation Look-aside Buffer) 또는 <strong>Associative memory</strong> 사용</p>
<ul>
<li>일반적으로, TLB는 <strong>컨텍스트 전환 시 플러시</strong>됨 (이전 항목 제거)</li>
<li>일부 TLB는 <strong>ASID</strong>(Address-Space Identifiers)를 저장하여 플러시를 피함<br>
→ 각 프로세스의 주소 공간 보호 지원</li>
</ul>
</li>
</ul>
...
<p>두 가지 종류의 메모리:</p>
<ol>
<li><strong>일반 메모리</strong> (예: DRAM)
<ul>
<li>주소를 주면 데이터를 반환</li>
</ul>
</li>
<li><strong>연관 메모리</strong> (Associative memory)
<ul>
<li>데이터 일부를 주면 전체 레코드를 반환 (ex: 전화번호부)</li>
<li>병렬 탐색 없이는 느림, 구현 비용이 큼</li>
</ul>
</li>
</ol>
</li>
<li>
<p>Associative Memory (TLB): 병렬 탐색<br>
→ 페이지 테이블의 일부만 TLB에 저장됨</p>
</li>
<li>
<p>주소 변환 과정 (p, d)</p>
<ul>
<li>페이지 번호 p를 TLB에서 먼저 찾음
<ul>
<li>존재하면 해당 frame 반환</li>
<li>없으면 페이지 테이블에서 frame 번호 검색</li>
</ul>
</li>
</ul>
</li>
...
<ul>
<li>
<p>연관 메모리(associative memory) 탐색 시간 = α time unit</p>
</li>
<li>
<p>메모리 접근 시간 = β</p>
</li>
<li>
<p><strong>Hit ratio = ε</strong> (연관 메모리에서 찾은 비율)</p>
</li>
<li>
...
<ul>
<li><strong>&quot;valid&quot;</strong>: 해당 페이지가 유효 (사용 가능한 page)
<ul>
<li>프로세스의 논리 주소 공간에 포함됨</li>
</ul>
</li>
<li><strong>&quot;invalid&quot;</strong>: 접근이 허용되지 않음 (해당 주소는 불법)</li>
</ul>
</li>
</ul>
...
<ul>
<li>페이지 크기 = 2KB</li>
<li>주소 범위: 0 ~ 10468만 사용</li>
<li>총 6개 페이지만 사용됨 (10469 / 2048 = 약 6)</li>
...
<ul>
<li>읽기 전용 (read-only, <strong>재진입 가능</strong>) 코드</li>
<li>여러 프로세스가 공유 (예: 텍스트 편집기, 컴파일러, 윈도우 시스템)</li>
<li>공유 코드는 <strong>논리 주소 공간의 동일한 위치</strong>에 있어야 함
<ul>
<li>공유 코드에서의 <strong>자기 참조</strong>가 가능하도록 하기 위함</li>
</ul>
</li>
</ul>
</li>
<li>
<p><strong>Private code and data</strong></p>
<ul>
<li>각 프로세스는 <strong>개별 복사본</strong>을 유지</li>
<li>해당 코드는 논리 주소 공간 내 <strong>임의의 위치</strong>에 존재할 수 있음</li>
</ul>
</li>
</ul>
...
<tr>
<td>Compile Time</td>
<td>physical address를 직접 코드에 삽입 (kernel 이후 주소부터)</td>
</tr>
<tr>
<td>Load Time</td>
<td>로딩 시 physical address를 삽입</td>
</tr>
<tr>
<td>Execution Time</td>
<td>실행 시 주소 변환 수행 (가장 유연, modern OS에서 사용)</td>
</tr>
</tbody>
</table>
<ul>
...
<li><strong>Associative Memory</strong> 사용</li>
...
<li>TLB hit: ( \alpha \ ) (access time)</li>
