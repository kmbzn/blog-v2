...
<li>여러 일반적인 agile 프랙티스 이면의 motivation(동기)을 설명하고 tradeoffs(트레이드오프)에 대해 추론</li>
...
<ul>
<li>가치(Value)</li>
<li>Processes and tools(프로세스와 도구)보다 Individuals and interactions(개인과 상호작용)</li>
<li>Comprehensive documentation(포괄적인 문서)보다 Working software(작동하는 소프트웨어)</li>
...
<li>Face-to-face conversation(대면 대화)는 최상의 communication(의사소통) 형태(co-location(동일 장소 근무))</li>
<li>Self-organizing teams(자기 조직화 팀)</li>
<li>Working software(작동하는 소프트웨어)를 frequently(자주)(months(몇 달) 단위가 아닌 weeks(몇 주) 단위로) 전달</li>
<li>Working software(작동하는 소프트웨어)가 progress(진행 상황)의 주된 척도</li>
<li>일정한 속도를 유지할 수 있는 Sustainable development(지속 가능한 개발)</li>
<li>technical excellence(기술적 탁월성)와 좋은 design(설계)에 대한 지속적인 관심</li>
<li>Simplicity(단순성)—수행하지 <em>않는</em> 작업의 양을 극대화하는 기술—가 필수적
<ul>
<li>(연관 가치: Individuals and interactions, Working software)</li>
</ul>
</li>
<li>유용한 소프트웨어의 rapid delivery(신속한 전달)을 통한 Customer satisfaction(고객 만족)</li>
<li>비즈니스 담당자와 개발자 간의 긴밀한 일일 cooperation(협력)</li>
<li>development(개발) 후반부일지라도 changing requirements(요구사항 변경) 환영</li>
<li>변화하는 환경(circumstances)에 대한 정기적인 adaptation(적응)
<ul>
<li>(연관 가치: Customer collaboration, Responding to change)</li>
</ul>
</li>
</ol>
...
<li>Behavior-driven development(BDD)(행동 주도 개발)</li>
<li>Cross-functional team(다기능 팀)</li>
<li>Continuous integration(CI)(지속적인 통합)</li>
<li>Domain-driven design(DDD)(도메인 주도 설계)</li>
<li>Information radiators(정보 방사기)(Kanban board, Task board, Burndown chart)</li>
<li>Acceptance test-driven development(ATDD)(인수 테스트 주도 개발)</li>
<li>Iterative and incremental development(IID)(반복적 및 증분 개발)</li>
<li>Pair programming(페어 프로그래밍)</li>
<li>Planning poker(플래닝 포커)</li>
<li>Refactoring(리팩토링)</li>
<li>Scrum meetings(스크럼 미팅)(Sprint planning, Daily scrum, Sprint review and retrospective)</li>
<li>Small releases(소규모 릴리스)</li>
<li>Simple design(단순한 설계)</li>
<li>Test-driven development(TDD)(테스트 주도 개발)</li>
<li>Agile testing(애자일 테스팅)</li>
<li>Timeboxing(타임박싱)</li>
<li>Use case(유스 케이스)</li>
<li>User story(사용자 스토리)</li>
<li>Story-driven modeling(스토리 주도 모델링)</li>
<li>Retrospective(회고)</li>
<li>On-site customer(상주 고객)</li>
<li>Agile Modeling(애자일 모델링)</li>
<li>40-hour weeks(주 40시간 근무)</li>
<li>Short development cycles(짧은 개발 주기)</li>
<li>Collective ownership(공동 소유권)</li>
<li>Open workspace(개방형 업무 공간)</li>
...
<li>다음 task(작업) 낭독</li>
<li>task(작업) 토론</li>
<li>Estimate(추정): 모두가 작업의 크기를 나타내는 카드를 선택하고 동시에 점수 공개</li>
<li>Discussion(토론): universal consensus(만장일치)가 아니면, 가장 낮거나 높은 점수를 낸 사람에게 이유 질문</li>
<li>Re-estimate(재추정)</li>
<li>Gain consensus(합의 도달)</li>
</ol>
</li>
<li>Benefits(장점):</li>
<li>task(작업)들을 서로 상대적으로 추정하기 쉬움</li>
...
<li>현실적으로, 전체 product(제품)에 대해 불가능</li>
...
<li>&quot;모든 것을 한 번만 말하라&quot;
<ul>
<li>매 순간, design(설계)는 모든 tests(테스트)를 실행하고, programmers(프로그래머)가 전달하고자 하는 모든 것을 전달하며, duplicate code(중복 코드)가 없고, 최소한의 classes(클래스)와 methods(메서드)를 가짐</li>
</ul>
</li>
</ul>
...
<li>두 가지 역할: Driver(드라이버)와 Navigator(내비게이터)</li>
...
<ul>
<li>Pros(장점)</li>
<li>Communication(의사소통)</li>
<li>Flexible(유연함)</li>
<li>Visually pleasing(시각적으로 만족스러움)</li>
<li>Cons(단점)</li>
<li>Lack of privacy(프라이버시 부족)</li>
...
<li>Short development cycle(짧은 개발 주기)</li>
<li>Small releases(소규모 릴리스)</li>
...
<li>sprint backlog(스프린트 백로그)는 해당 sprint(스프린트)에서 작업할 모든 features(기능). 이는 개별 tasks(작업)로 분할되어야 함:</li>
<li>Fine-grained(세분화됨)</li>
<li>Estimated(추정됨)</li>
<li>개별 team members(팀원)에게 Assigned(할당됨)</li>
...
<ul>
<li>Sprint Planning Meeting(스프린트 계획 회의)</li>
<li>Entire Team(전체 팀)이 해당 sprint(스프린트)에서 다룰 내용을 함께 결정</li>
<li>Daily Scrum Meeting(일일 스크럼 회의)</li>
<li>다음 사항을 확인하기 위한 Quick Meeting(빠른 회의):</li>
...
<li>Planning game(플래닝 게임): 1-3주 iterations(반복), 한 번에 하나의 iteration(반복), customer(고객)가 사용할 user stories(사용자 스토리) 결정</li>
<li>Whole team/on-site customer(전체 팀/상주 고객): &quot;고객은 하나의 목소리로 말함.&quot; Customer(고객)가 whole team(전체 팀)일 수 있음</li>
<li>Small releases(소규모 릴리스), 가치 있는 functionality(기능) 포함, 불만족스러운 customers(고객) 방지</li>
<li>System metaphor(시스템 메타포)는 작동 방식에 대한 단일 공유 스토리(architecture(아키텍처)와 유사)</li>
<li>Simplest thing that possibly works(작동 가능한 가장 단순한 것)(오늘을 위한 코딩)</li>
<li>프로그래밍 전 up-front design(사전 설계)가 없으므로 항상 Refactor(리팩토링)</li>
<li>Collective ownership(공동 소유권). 모두가 모든 것에 책임. Programmer(프로그래머)가 맘에 안 드는 것을 보면 변경 가능. Task ownership(작업 소유권)은 개인</li>
<li>Pair programming(페어 프로그래밍). prototypes(프로토타입) 같은 non-production code(비운영 코드)는 혼자 code(코딩) 가능</li>
<li>Continuous Integration(지속적인 통합). 최대 하루 단위 development(개발)</li>
...
<thead>
<tr>
<th style="text-align:left">Aspects(측면)</th>
<th style="text-align:left">Practices(프랙티스)</th>
...
<tbody>
<tr>
<td style="text-align:left">Iteration Length(반복 길이)</td>
<td style="text-align:left"></td>
<td style="text-align:left">2-4주</td>
<td style="text-align:left">1-2주</td>
</tr>
<tr>
<td style="text-align:left">Handle Changes with an Iteration(반복 중 변경 처리)</td>
<td style="text-align:left">요구사항 수정 허용 여부</td>
<td style="text-align:left">Scrum은 허용되지 않음. 반복이 완료되면 변경 불가, Scrum Master(스크럼 마스터)가 엄격히 확인</td>
<td style="text-align:left">구현되지 않은 need(요구)가 있을 때 다른 요구사항으로 교체 가능, 단 구현 시간은 동일</td>
</tr>
<tr>
<td style="text-align:left">Priority of Features(기능 우선순위)</td>
<td style="text-align:left">요구가 엄격히 우선순위에 따르는지 여부</td>
<td style="text-align:left">그럴 필요 없음</td>
<td style="text-align:left">예</td>
</tr>
<tr>
<td style="text-align:left">Engineering Practices(엔지니어링 프랙티스)</td>
<td style="text-align:left">진행 또는 품질 보증을 위한 엄격한 엔지니어링 방법 채택 여부</td>
<td style="text-align:left">개발자가 의식할 것을 요구</td>
<td style="text-align:left">매우 엄격</td>
</tr>
</tbody>
</table>
