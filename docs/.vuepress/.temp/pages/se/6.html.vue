...
<li>요구사항이 어떻게 그리고 왜 원하는 시스템과 그 환경 간의 관계를 명확히 하는지 설명. Assumptions(가정) 식별.</li>
...
<li>상세한 기술적 요구사항을 수립하는 것만큼 어려운 개념 작업은 없음...</li>
...
<li>Lack of resources(자원 부족)(10.6%)</li>
<li>Unrealistic expectations(비현실적인 기대)(9.9%)</li>
<li>Lack of executive support(경영진 지원 부족)(9.3%)</li>
<li>Changing requirements and specifications(요구사항 및 명세 변경)(8.7%)</li>
...
<ul>
<li>“고객이 구매 정보를 제출하고 결제가 접수된 후, 주문이 처리되어 고객의 배송 주소로 배송됩니다.”</li>
</ul>
</li>
<li>Optative statements(희망적 기술)
<ul>
<li>시스템은 고객에게 배송 상태를 알려야 함(shall)</li>
</ul>
</li>
<li>Domain Properties(도메인 속성) 및 Assumptions
<ul>
<li>모든 제품에는 고유한 제품 코드가 있음</li>
<li>결제는 승인 후 접수될 것임</li>
</ul>
</li>
</ul>
...
<ul>
<li>그 지식은 완전하고 일관성이 있는가?</li>
</ul>
</li>
<li>Knowledge representation(지식 표현) – 일단 포착되면, 어떻게 가장 효과적으로 표현하는가?
<ul>
<li>누구를 위해 표현하는가?</li>
<li>다른 사람들에게 일관되게 전달되는가?</li>
</ul>
</li>
...
<li>Response to events(이벤트에 대한 응답)</li>
</ul>
</li>
<li>기준:
<ul>
<li>Completeness(완전성): 모든 요구사항이 문서화됨</li>
<li>Consistency(일관성): 요구사항 간 충돌 없음</li>
<li>Precision(정확성): 요구사항에 모호함 없음</li>
</ul>
</li>
</ul>
...
<li>Functional requirements보다 더 중요할 수 있음
<ul>
<li>누락된 기능은 우회하여 작업 가능</li>
<li>품질이 낮은 시스템은 사용 불가능할 수 있음</li>
</ul>
</li>
</ul>
...
<ul>
<li>Ground spoilers(지상 스포일러)(양력을 줄이기 위해 펼쳐지는 날개판)</li>
<li>Reverse thrusters(역추진 장치)</li>
<li>주 착륙 장치의 휠 브레이크</li>
</ul>
</li>
...
<ol>
<li>어느 쪽이든 충격 흡수 장치가 6300 kg의 하중을 견딤</li>
<li>양쪽 바퀴가 72 knots(133.3 km/h) 이상으로 회전</li>
</ol>
<ul>
<li>Ground spoilers는 조건 1 또는 2에 활성화</li>
...
<li>제동 시스템이 활성화되지 않음
<ul>
<li>첫 번째 지점은 비행기가(측풍에 대응하기 위해) 기울어져 착륙했기 때문에 충족되지 않음.</li>
</ul>
</li>
<li>따라서 양쪽 착륙 장치에 필요한 압력에 도달하지 못함.
<ul>
<li>두 번째 지점은 젖은 활주로에서의 hydroplaning(수막 현상) 효과로 인해 충족되지 않음.</li>
</ul>
</li>
...
<li>Output</li>
<li>Interface</li>
<li>Response to events</li>
</ul>
</li>
<li>기준:
<ul>
<li>Completeness: 모든 요구사항이 문서화됨</li>
<li>Consistency: 요구사항 간 충돌 없음</li>
<li>Precision: 요구사항에 모호함 없음</li>
</ul>
</li>
</ul>
...
<li>Informal goal(비공식적 목표): 사용 편의성과 같은 일반적인 의도.
<ul>
<li>시스템 사용자의 의도를 전달하므로 개발자에게 여전히 유용할 수 있음.</li>
</ul>
</li>
...
<li>Privacy(사생활) 요구사항: 참가자의 일기는 본인의 동의 없이 다른 초대 참가자에게 절대 공개될 수 없음.</li>
<li>Integrity(무결성) 요구사항: 도서 반납은 도서관 직원에 의해서만 정확하게 인코딩되어야 함.</li>
<li>Availability(가용성) 요구사항:
<ul>
<li>불량 이용자 블랙리스트는 도서관 직원이 언제든지 사용할 수 있어야 함.</li>
<li>열차 위치 정보는 언제든지 중앙역 컴퓨터에서 사용할 수 있어야 함.</li>
</ul>
</li>
<li>Reliability(신뢰성) 요구사항: 열차 가속 제어 소프트웨어는 100시간 단위의 mean time between failures(평균 고장 간격)를 가져야 함.</li>
<li>Accuracy(정확성) 요구사항:
<ul>
<li>도서 대출 소프트웨어는 책이 실제로 도서관 서가에 이용 가능한 경우에만, 그리고 그 경우에만 이용 가능하다고 명시해야 함.</li>
<li>열차 관제사가 사용하는 열차 위치 정보는 실제 열차 위치를 최대 4미터 이내로 정확하게 반영해야 함.</li>
<li>회의 스케줄러가 사용하는 제약 조건은 초대된 참가자의 실제 제약 조건을 정확하게 반영해야 함.</li>
</ul>
</li>
<li>Performance(성능) 요구사항:
<ul>
<li>서지 정보 질의에 대한 응답은 2초 미만이어야 함.</li>
<li>가속 명령은 3초마다 모든 열차에 발행되어야 함.</li>
<li>회의 스케줄러는 최대 9개의 요청을 병렬로 처리할 수 있어야 함.</li>
<li>새로운 e-구독 기능은 30%의 비용 절감을 보장해야 함.</li>
</ul>
</li>
...
<li>Knowledge representation – 일단 포착되면, 어떻게 가장 효과적으로 표현하는가?
<ul>
<li>누구를 위해 표현하는가?</li>
<li>다른 사람들에게 일관되게 전달되는가?</li>
</ul>
...
<ul>
<li>System-as-is의 분석된 결함을 해결하기 위해</li>
<li>비즈니스 목표에 맞춰</li>
<li>기술 기회를 활용하여</li>
</ul>
</li>
...
<ul>
<li>도메인 지식 획득</li>
<li>대안 평가(예: 동일한 목표를 만족시키는 대안적 방법)</li>
<li>문제-기회 연결 및 평가: 영향, 관련 위험</li>
<li>상충하는 목표 처리
Objectives</li>
</ul>
</li>
...
<ul>
<li>식별된 목표를 만족시키기 위해</li>
<li>품질 제약(보안, 성능 등)에 따라</li>
<li>환경에 대한 현실적인 가정에 기반하여</li>
</ul>
</li>
<li>예: 공항 열차 제어
“안전한 열차 가속 계산”
“열차 내 승객을 위한 유용한 정보 표시”</li>
<li>어려움
<ul>
<li>올바른 기능 집합 식별</li>
<li>모든 당사자가 이해할 수 있도록 정확하게 명시</li>
<li>시스템 목표에 대한 backward traceability(역추적성) 보장
요구사항, 제약, 가정</li>
</ul>
</li>
</ul>
...
<ul>
<li>그들의 역량과 시스템의 목표에 기반하여</li>
<li>소프트웨어-환경 경계를 산출</li>
</ul>
</li>
<li>예: 공항 열차 제어
<ul>
<li>“안전한 열차 가속”... software-to-be(개발될 소프트웨어)의 직접적인 책임 하에(무인 옵션) 또는 소프트웨어 지시를 따르는 운전자 책임 하에?</li>
<li>“열차 속도/위치 정확한 추정”... 추적 시스템 책임 하에 또는 선행 열차 책임 하에?</li>
</ul>
</li>
<li>어려움
<ul>
<li>올바른 자동화 수준을 결정하기 위해 대안 평가</li>
</ul>
</li>
</ul>
...
<li>Domain understanding(도메인 이해)</li>
<li>Requirements elicitation(요구사항 도출)(인터뷰 등)</li>
<li>Evaluation and agreement(평가 및 합의)(충돌, 우선순위 지정, 위험 등)</li>
...
<li>Requirements Document(RD)(요구사항 문서) 항목의 Consistency</li>
<li>요구사항, 가정, domain props(도메인 속성)의 Adequacy(적절성)</li>
<li>RD 항목의 Unambiguity(명확성)</li>
<li>요구사항, 가정의 Measurability(측정 가능성)</li>
<li>요구사항, 가정의 Pertinence(관련성)</li>
<li>요구사항의 Feasibility(실현 가능성)</li>
<li>RD 항목의 Comprehensibility(이해 가능성)</li>
<li>RD의 Good structuring(좋은 구조화)</li>
...
<ul>
<li>Omission(누락)</li>
<li>Contradiction(모순)</li>
<li>Inadequacy(부적절성)</li>
<li>Ambiguity(모호성)</li>
<li>Unmeasurability(측정 불가능성)</li>
<li>Noise(노이즈), overspecification(과잉 명세)</li>
<li>Unfeasibility(실현 불가능성)(희망 사항)</li>
<li>Unintelligibility(이해 불가능성)</li>
<li>Poor structuring(구조 불량), forward reference(전방 참조), remorse(후회)</li>
<li>Opacity(불투명성)
<ul>
<li>Errors(오류)</li>
<li>Flaws(결함)</li>
...
<ul>
<li>예: 비상 정지 시 열차 문 상태에 대한 요구사항 없음</li>
</ul>
</li>
<li>Contradiction: RD 항목들이 문제 세계 피처를 호환되지 않는 방식으로 기술
<ul>
<li>“문은 플랫폼 사이에서 항상 닫혀 있어야 함”</li>
<li>그리고 “비상 정지 시 문이 열려야 함”</li>
</ul>
</li>
<li>Inadequacy: RD 항목이 문제 세계 피처를 적절하게 기술하지 못함
<ul>
<li>“열차 내부 패널은 다음 정류장에서 서비스되는 모든 항공편을 표시해야 함”</li>
</ul>
</li>
<li>Ambiguity: RD 항목이 문제 세계 피처를 다른 방식으로 해석할 여지를 둠
<ul>
<li>“열차가 플랫폼에 정지하자마자 문이 열려야 함”</li>
</ul>
</li>
<li>Unmeasurability: RD 항목이 옵션 비교나 솔루션 테스트를 배제하는 방식으로 문제 세계 피처를 기술
<ul>
<li>“열차 내부 패널은 사용자 친화적이어야 함”</li>
</ul>
</li>
</ul>
...
<ul>
<li>“금연 표지판을 열차 창문에 부착해야 함”</li>
</ul>
</li>
<li>Overspecification: 문제 세계가 아닌 기계 솔루션의 피처를 기술하는 RD 항목
<ul>
<li>“<code v-pre>Alarm</code> 메시지 수신 시 <code v-pre>setAlarm</code> 메서드를 호출해야 함”</li>
</ul>
</li>
<li>Unfeasibility: 예산/일정 내에 구현할 수 없는 RD 항목
<ul>
<li>“열차 내 패널은 다음 정류장의 모든 지연된 항공편을 표시해야 함”</li>
</ul>
</li>
<li>Unintelligibility: RD 항목을 사용해야 하는 사람이 이해할 수 없음
<ul>
<li>5개의 약어를 포함하는 요구사항 기술</li>
</ul>
</li>
<li>Poor structuring: 합리적이고 가시적인 구조화 규칙에 따라 구성되지 않은 RD 항목
<ul>
<li>가속 제어와 열차 추적 문제의 뒤섞임</li>
</ul>
</li>
<li>Forward reference: 아직 정의되지 않은 문제 세계 피처를 사용하는 RD 항목
<ul>
<li>최악의 경우 정지 거리 개념이 RD에서 몇 페이지 뒤에 정의되기 전에 여러 번 사용됨</li>
</ul>
</li>
<li>Remorse: 문제 세계 피처를 늦게 또는 부수적으로 기술하는 RD 항목
<ul>
<li>정의되지 않은 최악의 경우 정지 거리 개념을 여러 번 사용한 후, 마지막 사용에서 괄호 안에 부수적인 정의가 바로 이어짐</li>
</ul>
</li>
<li>Poor modifiability(낮은 수정 용이성): 변경 사항이 RD 전체에 전파되어야 하는 RD 항목
<ul>
<li>변경될 수 있는 수량에 대해 고정된 숫자 값 사용</li>
</ul>
</li>
<li>Opacity: 근거, 작성자 또는 종속성이 보이지 않는 RD 항목
<ul>
<li>“명령된 열차 속도는 항상 물리적 속도보다 최소 7 mph 높아야 함”이라는 기술에 대한 근거 설명 없음</li>
</ul>
</li>
</ul>
...
<li>Structured text(구조화된 텍스트)</li>
