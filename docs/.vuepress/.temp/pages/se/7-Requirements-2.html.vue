...
<li>효과적인 requirements interviews(요구사항 인터뷰) 수행에 대한 기본 능력 시연</li>
...
<ul>
<li>이해관계자 식별</li>
<li>Understand the domain(도메인 이해)
<ul>
<li>Analyze artifacts(산출물 분석), 이해관계자와의 상호작용</li>
</ul>
</li>
<li>Discover the real needs(실제 요구 파악)
<ul>
<li>이해관계자 인터뷰</li>
</ul>
</li>
...
<li>이해관계자:
<ul>
<li>End users(최종 사용자)</li>
<li>System administrators(시스템 관리자)</li>
<li>Engineers maintaining the system(시스템 유지보수 엔지니어)</li>
<li>Business managers(비즈니스 관리자)</li>
<li>...또 누가 있을까?</li>
</ul>
</li>
</ul>
...
<ul>
<li>조직 내 관련 직위</li>
<li>시스템에 대한 의사 결정에 있어 효과적인 역할</li>
<li>Level of domain expertise(도메인 전문성 수준)</li>
<li>인지된 문제에 대한 노출</li>
...
<li>Strengths(강점)
<ul>
<li>이해관계자가 무엇을 하고, 느끼고, 선호하는지</li>
<li>그들이 시스템과 어떻게 상호작용하는지</li>
<li>현재 시스템의 문제점</li>
</ul>
</li>
<li>Weaknesses(약점)
<ul>
<li>Subjective(주관적), inconsistencies(비일관성)</li>
<li>Capturing domain knowledge(도메인 지식 포착)</li>
<li>Familiarity(친숙성)</li>
<li>Technical subtlety(기술적 미묘함)</li>
</ul>
</li>
...
<li>인터뷰 수행.
-(structured(구조적)/unstructured(비구조적), individual/group)</li>
<li>인터뷰 녹음 + 전사</li>
<li>중요한 발견 사항 보고.</li>
...
<ul>
<li>이것이 왜 문제인가? 현재는 그 문제를 어떻게 해결하는가? 이상적으로는 어떻게 해결하고 싶은가?</li>
</ul>
</li>
<li>인터뷰 대상자가 설명할 문제가 더 이상 없을 때까지 후속 질문(“다른 문제는 없는가?”, “당신을 괴롭히는 다른 것들이 있는가?”)을 계속 질문.</li>
<li>그렇다면, 제가 이해하기로는 다음과 같은 문제/요구를 겪고 계신 것 같음
<ul>
<li>인터뷰 대상자의 문제와 요구를 자신의 말로 설명 – 종종 당신이 동일한 그림을 공유하지 않는다는 것을 발견할 것임.</li>
<li>처음에는 서로 이해했다고 생각하더라도 실제로는 그렇지 않은 경우가 매우 흔함</li>
</ul>
</li>
...
<li>사용자의 교육 수준이나 훈련 정도는 어떠한가?</li>
<li>사용자는 어떤 컴퓨터 기술을 가지고 있는가?</li>
<li>사용자가 이런 유형의 IT 시스템에 익숙한가?</li>
<li>현재 어떤 technical platforms(기술 플랫폼)를 사용하고 있는가?</li>
<li>향후 시스템이나 플랫폼에 대한 계획을 알고 있는가?</li>
<li>조직이 현재 사용 중인 다른 IT 시스템 중 새 시스템과 연결되어야 하는 것은 무엇인가?</li>
<li>System usability(시스템 사용성)에 대한 기대치는 어떠한가?</li>
...
<ul>
<li>이해관계자로부터 도출</li>
<li>Policies(정책) 또는 다른 문서로부터 추출</li>
<li>위 사항들 + estimation(추정) 및 invention(창안)으로부터 synthesize(종합)</li>
</ul>
</li>
<li>이해관계자가 항상 자신이 무엇을 원하는지 아는 것은 아니기 때문에, 엔지니어는...
<ul>
<li>이해관계자의 요구와 기대에 충실해야 함</li>
<li>추가적인 요구와 risks(위험)를 예측해야 함</li>
<li>“추가적인 요구”가 필요하거나 바람직한지 validate(검증)해야 함</li>
</ul>
</li>
</ul>
...
<li>인터뷰 전에 인터뷰 질문 검토</li>
<li>구체적인 질문, 제안으로 구체적으로 시작; prototype(프로토타입) 또는 scenario(시나리오)를 통해 작업
<ul>
<li>해당되는 경우, 현재 시스템과 연관.</li>
</ul>
</li>
<li>개방적인 태도를 유지; 자연스럽게 발생하는 추가 이슈 탐색. 단, 시스템에 계속 집중.</li>
...
<ul>
<li>다양한 책임, 전문성, 과업, 문제 노출</li>
</ul>
</li>
<li>적절한 시기에 적절한 문제에 집중할 수 있도록 준비된 상태로 임함
<ul>
<li>배경 연구 우선 수행</li>
<li>해당 인터뷰 대상자를 위한 질문 순서 사전 설계</li>
</ul>
</li>
<li>인터뷰 대상자의 업무와 관심사에 인터뷰의 초점을 맞춤</li>
<li>인터뷰에 대한 통제력 유지</li>
<li>인터뷰 대상자를 편안하게 만듦
<ul>
<li>시작: 어색함 해소, 동기 부여, 쉬운 질문</li>
<li>역할뿐만 아니라 사람 자체도 고려</li>
<li>항상 신뢰할 수 있는 파트너로 보여야 함</li>
</ul>
</li>
<li>집중하고, 개방형 질문은 마지막에 함</li>
<li>예상치 못한 답변에 대비해 개방적이고 유연한 태도 유지</li>
<li>불쾌감을 주지 않으면서 왜(why) 질문을 함</li>
<li>특정 유형의 질문 회피...
<ul>
<li>의견 또는 biased(편향된) 질문</li>
<li>Affirmative(긍정/부정) 질문(예/아니오 질문)</li>
<li>해당 인터뷰 대상자에게 명백하거나 불가능한 답변의 질문</li>
</ul>
</li>
<li>기억이 생생할 때 인터뷰 전사본 편집 및 구조화
<ul>
<li>개인적인 반응, 태도 등 포함</li>
</ul>
</li>
<li>인터뷰 대상자가 계속 참여하도록 함
<ul>
<li>검증 및 refinement(개선)을 위해 인터뷰 전사본 공동 검토</li>
</ul>
</li>
</ul>
...
<li>WIREFRAME(와이어프레임)
<ul>
<li>Purpose(목적): Communicate structure and get early feedback(구조 전달 및 초기 피드백 확보)</li>
<li>Fidelity(충실도): Low</li>
<li>Functionality(기능성): No</li>
<li>Skill requirement(필요 기술): Low</li>
<li>Resources(자원): Minimal</li>
<li>Time needed(소요 시간): Very low</li>
<li>Product cycle stage(제품 주기 단계): Discovery</li>
</ul>
</li>
<li>MOCKUP(목업)
<ul>
<li>Purpose: Showcase design(디자인 시연)</li>
<li>Fidelity: High</li>
<li>Functionality: No</li>
<li>Skill requirement: High</li>
<li>Resources: Design tool</li>
<li>Time needed: Medium</li>
<li>Product cycle stage: Design</li>
</ul>
</li>
<li>PROTOTYPE(프로토타입)
<ul>
<li>Purpose: Showcase design and functionality(디자인 및 기능성 시연)</li>
<li>Fidelity: High</li>
<li>Functionality: Yes</li>
<li>Skill requirement: High</li>
<li>Resources: Design tool</li>
<li>Time needed: High</li>
<li>Product cycle stage: Prototyping and testing</li>
</ul>
</li>
</ul>
...
<li>Mock-ups/prototypes는 요구사항의 uncertainty(불확실성)을 탐색하는 데 도움.
<ul>
<li>우리가 올바른 요구사항을 가졌는지 검증.</li>
<li>시스템의 “경계”에 있는 요구사항 도출.</li>
<li>솔루션 공간의 feasibility(실현 가능성) 주장.</li>
<li>후보 솔루션에 대한 피드백 획득.</li>
</ul>
</li>
...
<ul>
<li>Who(누가) 플레이어인가</li>
<li>What(무엇이) 그들에게 일어나는가</li>
<li>How(어떻게) 특정 에피소드를 통해 일어나는가</li>
<li>Why(왜) 이것이 일어나는가</li>
...
<ul>
<li>스토리보드는 최소한 누가, 무엇을, 어떻게 하는지를 명시적으로 다룸.</li>
</ul>
</li>
<li>다른 유형:
<ul>
<li>Positive(긍정적) vs negative(부정적)(일어나야 하는 것과 일어나지 않아야 하는 것)</li>
<li>Normal(정상) vs abnormal(비정상)</li>
</ul>
</li>
<li>도출의 일부로서:
<ul>
<li>실제 또는 가상의 순서를 따라 현재 또는 제안된 시스템에 대해 학습</li>
<li>구체적인 질문 가능</li>
<li>근본적인 목표 도출, 원하는 행동 모델로 일반화.</li>
<li>충돌 식별 및 해결</li>
</ul>
</li>
...
<li>Unstructured(비구조적) vs structured(구조적)</li>
<li>Text(텍스트) vs diagrams(다이어그램)</li>
...
<li>Alice: 두 가지 중요한 기본 유형은 <code v-pre>student</code>와 <code v-pre>course</code>. 또한 이진 관계 <code v-pre>enrolled</code>가 있음</li>
<li>Alice는 이러한 요소들을 다음과 같이 정의함:</li>
...
<li>Level(수준): User level, subprocess level</li>
<li>Primary actor(주요 액터):(액터는 primary(주요), supporting(지원), or offstage(무대 밖)일 수 있음)</li>
<li>Stakeholder, interests(이해관계): 중요! 유즈케이스는 이해관계자의 이익을 충족시키는 데 필요한 모든 것을 포함해야 함.</li>
<li>Preconditions(사전 조건): 시나리오가 시작되기 전에 항상 참이어야 하는 것. 테스트되지 않음; 가정됨. 무의미한 노이즈로 채우지 말 것.</li>
<li>Success guarantees(성공 보장): Postconditions(사후 조건)이라고도 함</li>
<li>Main success scenario(주요 성공 시나리오): Basic flow(기본 흐름), “happy path”(행복 경로), typical flow. 모든 조건은 확장으로 연기. 단계 기록: 액터 간의 상호작용, 유효성 검사, 시스템에 의한 상태 변경.</li>
<li>Extensions(확장): Alternate flows(대안 흐름)라고도 함. 종종 텍스트의 대부분을 차지. 때때로 다른 유즈케이스로 분기됨.</li>
<li>Special requirements(특별 요구사항): Non-functional/quality requirements(비기능/품질 요구사항)가 위치하는 곳.</li>
<li>Technology and data variations list(기술 및 데이터 변형 목록): 피할 수 없는 기술 제약; I/O 기술로 한정하도록 노력.</li>
...
<ul>
<li>
<p>Use-case: InitiateMonitoring</p>
</li>
<li>
<p>Primary actor: Homeowner</p>
</li>
<li>
<p>Goal in context: 집주인이 집을 떠나거나 내부에 머무를 때 센서를 모니터링하도록 시스템을 설정하기 위함</p>
</li>
<li>
<p>Preconditions: 시스템이 암호 및 다양한 센서 인식을 위해 프로그래밍되었음</p>
</li>
<li>
<p>Trigger(트리거): 집주인이 시스템을 “설정”하기로 결정, 즉 알람 기능 켬</p>
</li>
<li>
<p>Scenario:</p>
<ol>
<li>Homeowner: 제어판 관찰</li>
<li>Homeowner: 암호 입력</li>
<li>Homeowner: “stay” 또는 “away” 선택</li>
<li>Homeowner: SafeHome이 작동되었음을 나타내는 빨간색 알람 불빛 관찰</li>
</ol>
</li>
<li>
<p>Exceptions(예외):</p>
<ul>
<li>1a. 제어판이 준비되지 않음: 집주인이 모든 센서를 확인하여 어느 것이 열려있는지 파악; 닫음</li>
<li>2a. 암호가 정확하지 않음</li>
</ul>
</li>
<li>
<p>Priority: Essential(필수), 반드시 구현되어야 함</p>
</li>
<li>
<p>When available: first increment(첫 번째 증분)</p>
</li>
<li>
<p>Frequency of use: 하루에 여러 번</p>
</li>
<li>
<p>Channel to actor: 제어판 인터페이스를 통해</p>
</li>
<li>
<p>Secondary actors: Support technician(지원 기술자)</p>
</li>
<li>
<p>Channels to secondary actors: 지원 기술자: 전화선</p>
</li>
<li>
<p>Open issues(미해결 문제):</p>
<ul>
<li>암호 입력에 시간제한을 두어야 하는가?</li>
</ul>
</li>
</ul>
...
<ul>
<li>Full use case model(전체 유즈케이스 모델)(전체 시스템, 상위 수준)</li>
<li>“Agile” use case: 구현될 시스템 기능의 작고 구체적인 조각(때때로 “사용자 스토리”와 혼용됨)</li>
</ul>
</li>
<li>여러 단계에서 사용됨:
<ul>
<li>Requirements elicitation(요구사항 도출)(설명, 검증, 요구사항; 충돌 강조, 요구사항 우선순위 지정 등)</li>
<li>Requirements documentation(요구사항 문서화).</li>
<li>Concrete design(구체적인 설계): UML 다이어그램</li>
</ul>
</li>
</ul>
...
<li>세부 사항은 나중에 고객과의 협상을 위해 남겨두거나 실제 요구사항에 대한 포인터</li>
...
<li>너무 큼, 분할:
<ul>
<li>파워 유저로서, 파일 크기, 생성 날짜, 수정 날짜를 기반으로 백업할 파일이나 폴더를 지정할 수 있음.</li>
<li>사용자로서, 백업 드라이브가 필요 없는 것들로 채워지지 않도록 백업하지 않을 폴더를 표시할 수 있음.</li>
</ul>
</li>
</ul>
...
<ul>
<li>예: 도서관 관리: “borrower” vs. “patron”</li>
</ul>
</li>
<li>Designation clash(명칭 충돌): 다른 개념에 대해 다른 기술에서 동일한 이름 사용
<ul>
<li>예: “도서관 이용자” vs. “도서관 소프트웨어 사용자”에 대해 “user” 사용</li>
</ul>
</li>
<li>Structure clash(구조 충돌): 동일한 개념이 다른 기술에서 다르게 구조화됨
<ul>
<li>예: “latest return date”가 시점(예: 금요일 오후 5시) vs. 시간 간격(예: 금요일)</li>
</ul>
</li>
<li>Strong conflict(강한 충돌): 함께 만족될 수 없는 기술들
<ul>
<li>예: “참가자 제약 조건은 다른 누구에게도 공개될 수 없음” vs. “회의 개시자는 참가자 제약 조건을 알아야 함”</li>
</ul>
</li>
<li>Weak conflict(divergence)(약한 충돌(불일치)): 일부 boundary condition(경계 조건) 하에서 함께 만족될 수 없는 기술들
<ul>
<li>예: “이용자는 X주 이내에 빌린 도서를 반납해야 함” vs “이용자는 필요한 만큼 빌린 도서를 소지해야 함”은 “필요한 &gt; X주”인 경우에만 모순</li>
</ul>
</li>
</ul>
...
<li>약한, 강한 충돌: 협상 필요
<ul>
<li>원인: 이해관계자들의 다른 목표 =&gt; 요구사항 외부에서 해결</li>
<li>원인: quality tradeoffs(품질 절충) =&gt; 선호도 탐색</li>
</ul>
</li>
</ul>
