...
<li>quality attributes(품질 속성)를 염두에 두고 software architecture(소프트웨어 아키텍처)에 접근</li>
<li>software architecture와(object-oriented) software design(객체 지향 소프트웨어 설계) 구별</li>
...
<li>&quot;신속하게 응답해야 함, accuracy(정확성)은 덜 관련됨&quot;.</li>
...
<li>Aids in communication with stakeholders(이해관계자와의 의사소통)
<ul>
<li>그들이 이해할 수 있는 수준에서 &quot;how(방법)&quot;를 보여주어, 그것이 그들의 요구를 충족하는지에 대한 질문 제기</li>
</ul>
</li>
<li>Defines constraints on implementation(구현에 대한 제약)
<ul>
<li>설계 결정이 애플리케이션의 &quot;load-bearing walls(내력벽)&quot; 형성</li>
</ul>
</li>
<li>Dictates organizational structure(조직 구조)
<ul>
<li>팀이 서로 다른 components(구성요소) 작업</li>
</ul>
</li>
<li>Inhibits or enables quality attributes
<ul>
<li>design patterns(설계 패턴)과 유사</li>
</ul>
</li>
<li>Supports predicting cost, quality, and schedule(비용, 품질, 일정 예측 지원)
<ul>
<li>일반적으로 각 구성요소의 정보를 예측함으로써</li>
</ul>
</li>
<li>Aids in software evolution(소프트웨어 진화 지원)
<ul>
<li>변경의 비용, 설계, 영향에 대해 추론</li>
</ul>
</li>
<li>Aids in prototyping(프로토타이핑 지원)
<ul>
<li>architectural skeleton(아키텍처 스켈레톤)을 조기에 구현 가능</li>
</ul>
</li>
</ul>
...
<li>Modifiability(수정 용이성), portability(이식성)</li>
<li>Scalability(확장성)</li>
<li>Security(보안)</li>
<li>Testability(테스트 용이성)</li>
<li>Usability(사용성)</li>
<li>구축 비용, 운영 비용</li>
</ul>
</li>
...
<li>Monolithic(모놀리식): 원시 데이터베이스, memcache 관리, 사이트 렌더링, 공용 API 제공을 하나의 코드베이스에서 수행</li>
<li>시스템 이해가 점점 어려워짐; 엔지니어링 팀 관리 및 병렬화가 조직적으로 어려움</li>
<li>스토리지 시스템(MySQL)의 throughput(처리량) 한계 도달; 데이터베이스 전반의 읽기 및 쓰기 hot spots(핫스팟)</li>
<li>문제 해결을 위해 기계 투입; 기계당 낮은 처리량(CPU + RAM 한계, 네트워크는 포화되지 않음)</li>
...
<li>Reduce number of machines 10x(기계 수 10배 감소)</li>
<li>Isolate failures(장애 격리)</li>
<li>&quot;우리는 '관련된' 로직이 한곳에 있는 더 깨끗한 경계를 원했음&quot;
-(클래스, 모듈, 패키지 수준이 아닌) 시스템 수준에서의 encapsulation(캡슐화) 및 modularity(모듈성)</li>
<li>Quicker release of new features(신규 기능의 더 빠른 출시)
<ul>
<li>&quot;다른 팀과 독립적으로 로컬 결정을 내리고 사용자 대상 변경 사항을 배포할 수 있는, 작고 권한이 부여된 엔지니어링 팀 운영&quot;</li>
</ul>
</li>
</ul>
...
<li>Twitter 응답은 여러 서비스에서 집계됨 – 응답 시간 합산</li>
<li>&quot;시스템을 서비스로 분해하기 시작하면서 각 팀은 약간씩 다른 접근 방식을 취했음. 예를 들어, 클라이언트에서 서비스로의 장애 의미론이 잘 상호작용하지 않았음: 서버가 클라이언트에 다시 신호를 보낼 일관된 back-pressure(역압력) 메커니즘이 없었고, 지연 시간이 긴 서비스를 공격적으로 재시도하는 클라이언트로 인한 'thundering herds(몰려드는 무리)' 현상 경험.&quot;</li>
<li>목표: 동시성에 대해 생각하는 단일하고 통일된 방식
<ul>
<li>RPC(Finagle), connection pooling(커넥션 풀링), failover strategies(장애 극복 전략), load balancing(로드 밸런싱)을 위한 라이브러리로 구현</li>
</ul>
</li>
</ul>
...
<li>모놀리식 시스템에서 다중 서비스로
<ul>
<li>RPC interfaces(RPC 인터페이스) 합의, 시스템 내부 독립적 개발</li>
<li>Self-contained teams(자체 완결형 팀)</li>
</ul>
</li>
</ul>
...
<li>Temporal clustering(시간적 클러스터링)
<ul>
<li>단기 해결책</li>
<li>편중된 로드 밸런스</li>
<li>3주마다 기계 1대 + replications(복제)</li>
</ul>
</li>
...
<li>RPC framework(RPC 프레임워크) 위에 자체 dynamic analysis tool(동적 분석 도구) 구축</li>
<li>많은 수의 기계를 구성하기 위한 프레임워크
<ul>
<li>사용자 일부에게만 기능을 노출하는 기능 포함</li>
</ul>
</li>
</ul>
...
<li>Abstract(추상적), 다양한 시나리오에 대한 다양한 추상화</li>
<li>quality attributes에 대해 조기에 추론</li>
...
<ul>
<li>high-level “what”(상위 수준 &quot;무엇을&quot;)</li>
</ul>
</li>
<li>Architecture(High-level design)(아키텍처(상위 수준 설계))
<ul>
<li>high-level “how”, mid-level “what”</li>
</ul>
</li>
<li>OO-Design(Low-level design, e.g., design patterns)(OO 설계(하위 수준 설계, 예: 설계 패턴))
<ul>
<li>mid-level “how”, low-level “what”</li>
</ul>
</li>
<li>Code(코드)
<ul>
...
<li>어떤 threads(스레드)가 존재하며 어떻게 coordinate(조정)되는가?</li>
<li>Google은 하루 수십억 건의 히트를 어떻게 scale(확장)하는가?</li>
...
<li>Eclipse에 메뉴 항목을 쉽게 추가하려면 어떻게 해야 하는가?</li>
<li>어떤 lock(잠금)이 이 데이터를 보호하는가?</li>
<li>Google은 페이지 순위를 어떻게 매기는가?</li>
<li>보안 통신을 위해 어떤 encoder(인코더)를 사용해야 하는가?</li>
...
<ul>
<li>조기 분석을 위한 산출물</li>
<li>quality attributes의 주요 전달자</li>
<li>배포 후 유지보수 및 향상의 핵심</li>
</ul>
</li>
<li>문서화는 현재와 20년 후의 아키텍트를 대변함
<ul>
<li>시스템이 문서화된 아키텍처에 따라 구축, 유지보수, 진화하는 동안</li>
</ul>
</li>
...
<li>Static View(정적 뷰)
<ul>
<li>Modules(모듈)(subsystems, structures) 및 그들의 relations(관계)(dependencies(의존성), …)</li>
</ul>
</li>
<li>Dynamic View(동적 뷰)
<ul>
<li>Components(processes(프로세스), runnable entities) 및 connectors(커넥터)(messages(메시지), data flow(데이터 흐름), …)</li>
</ul>
</li>
<li>Physical View(Deployment)(물리적 뷰(배포))
<ul>
<li>Hardware structures(하드웨어 구조) 및 그들의 connections(연결)</li>
</ul>
</li>
</ul>
...
<li>서로 다른 뷰는 서로 다른 추론 측면(다른 품질 목표)에 적합함. 예:
<ul>
<li>Performance</li>
<li>Extensibility</li>
<li>Security</li>
...
<ul>
<li>목적에 적합</li>
<li>간결한 표현을 위해 종종 시각적</li>
<li>일반적으로 상자와 화살표</li>
<li>UML 가능(semi-formal(준정형)), 하지만 제약이 있을 수 있음
<ul>
<li>다른 추상화 수준에 유의 – 클래스나 객체가 아닌 하위 시스템 또는 프로세스</li>
</ul>
</li>
...
<ul>
<li>Ambiguous(모호한) 상자-선 다이어그램</li>
<li>일관성 없는 표기법 사용</li>
<li>혼란스러운 뷰 유형의 조합</li>
</ul>
</li>
<li>많은 것들이 명시되지 않은 채 남아 있음:
<ul>
<li>어떤 종류의 요소인가?</li>
<li>어떤 종류의 관계인가?</li>
<li>상자와 화살표는 무엇을 의미하는가?</li>
<li>레이아웃의 중요성은 무엇인가?</li>
</ul>
</li>
</ul>
...
<li>A가 B에게 데이터 전달</li>
<li>A가 B로부터 값 획득</li>
<li>A가 B에게 데이터 스트리밍</li>
<li>A가 B에게 메시지 전송</li>
<li>A가 B 생성</li>
<li>A가 B보다 먼저 발생</li>
<li>B가 A로부터 전기 공급받음</li>
</ul>
</li>
</ul>
...
<li>상자가 의미하는 바를 정확하게 정의</li>
<li>선이 의미하는 바를 정확하게 정의</li>
<li>의도치 않게 뷰 유형 혼합 금지
<ul>
<li>상기: Module(classes), C&amp;C(components)</li>
</ul>
</li>
<li>그래픽을 설명으로 보충
<ul>
<li>매우 중요: rationale(이론적 근거)(architectural intent - 아키텍처 의도)</li>
</ul>
</li>
<li>하나의 다이어그램에 너무 많은 것을 하려고 시도 금지
<ul>
<li>아키텍처의 각 뷰는 한 페이지에 맞아야 함</li>
<li>계층 구조 사용</li>
</ul>
</li>
</ul>
