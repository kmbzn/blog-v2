...
<li>A가 B로 데이터를 stream(스트림)</li>
<li>A가 B로 message(메시지)를 전송</li>
...
<li>Specifying(명세화), constructing(구축), documenting(문서화)를 위한 visual language(시각적 언어)</li>
<li>OMG (Object Management Group)에 의해 유지 관리</li>
<li>웹사이트: <a href="http://www.omg.org" target="_blank" rel="noopener noreferrer">http://www.omg.org</a></li>
<li>Object-oriented(객체 지향)</li>
...
<li>Sketch(스케치)
<ul>
<li>Conceptual perspective(개념적 관점)</li>
<li>Problem 또는 solution space의 어려운 부분을 탐색하기 위해 비공식적이고 불완전한 diagram 생성 → 상호 소통 매체</li>
</ul>
</li>
<li>Blueprint(청사진)
<ul>
<li>Specification perspective(명세 관점)</li>
<li>Code generation(코드 생성)을 위해 <strong>비교적 상세한</strong> design diagram 사용</li>
</ul>
</li>
<li>Programming language(프로그래밍 언어)
<ul>
<li>Implementation perspective(구현 관점)</li>
<li>UML로 소프트웨어 system의 완전한 executable specification(실행 가능 명세) 작성</li>
<li>실행 가능한 코드가 자동으로 생성</li>
<li>이론, 도구 robustness(견고성), usability(사용성) 측면에서 여전히 개발 중</li>
</ul>
</li>
</ul>
...
<ul>
<li>UML은 소프트웨어 system을 개발하는 체계적인 방법이 아님.</li>
</ul>
</li>
<li>UML은 Object-Oriented 사고방식을 가르쳐주지 않음.
<ul>
<li>객체 structure(구조)나 behavior(행위)를 어떻게 설계해야 하는지 알려주지 않음.</li>
<li>design이 좋은지 나쁜지 알려주지 않음.</li>
</ul>
</li>
</ul>
...
<li>(다이어그램 구성 요소)</li>
<li>Use Case</li>
<li>Use Case Name (유스케이스 이름)</li>
<li>Subject Name (주체 이름)</li>
<li>Subject(주체)</li>
<li>System Boundary(시스템 경계)</li>
<li>Association(연관)</li>
<li>Dependency(의존)</li>
<li>Actor(행위자)</li>
</ul>
...
<li>유용한 질문들
<ul>
<li>누가 system의 주요 functionality(기능)을 사용할 것인가 (<strong>primary actors</strong>(주요 행위자))?</li>
<li>누가 일상 업무를 위해 system의 지원을 필요로 하는가?</li>
<li>누가 system을 유지보수, 관리하고 작동하도록 유지해야 하는가 (<strong>secondary actors</strong>(보조 행위자))?</li>
<li>System이 처리해야 할 hardware devices(하드웨어 장치)는 무엇인가?</li>
<li>System이 상호작용해야 할 다른 system은 무엇인가?</li>
<li>누가 또는 무엇이 system이 생성하는 결과(가치)에 관심이 있는가?</li>
</ul>
</li>
</ul>
...
<li>하나 이상의 Actor와 system 간의 상호작용</li>
<li>Use case 식별하기
<ul>
<li>Actor가 system에 요구하는 function(기능)은 무엇인가?</li>
<li>Actor가 system의 특정 정보를 읽기, 생성, 삭제, 수정 또는 저장해야 하는가?</li>
<li>Actor가 system의 event에 대해 notify(알림)을 받아야 하는가?</li>
<li>System의 새로운 기능을 통해 Actor의 일상 업무가 단순화되거나 효율화될 수 있는가?</li>
</ul>
</li>
</ul>
...
<li>Main Success Scenario(주 성공 시나리오):
<ol>
<li>고객이 카탈로그를 탐색하고 구매할 아이템 선택</li>
<li>고객이 결제 진행</li>
<li>고객이 배송 정보 입력 (주소; 익일 또는 3일 배송)</li>
<li>System이 배송비 포함 전체 가격 정보 제시</li>
<li>고객이 신용카드 정보 입력</li>
<li>System이 구매 승인</li>
<li>System이 즉시 판매 확인</li>
<li>System이 고객에게 확인 이메일 발송</li>
</ol>
</li>
<li>Extensions(확장):
<ul>
<li>3a: 고객이 regular customer(단골 고객)인 경우
<ul>
<li>.1: system이 현재 배송, 가격, 청구 정보 표시</li>
<li>.2: 고객이 이 default(기본값)을 수락하거나 변경 가능, MSS 6단계로 복귀</li>
</ul>
</li>
<li>6a: system이 신용카드 구매 승인 실패
<ul>
<li>.1: 고객이 신용카드 정보를 다시 입력하거나 취소 가능</li>
</ul>
</li>
</ul>
</li>
...
<li>Use Case를 텍스트로 정의할 때, noun(명사)과 verb(동사)를 정확하고 일관되게 사용하여 interaction diagrams을 위한 objects와 message 도출에 도움</li>
<li>여러 Use Case에 필요한 공통 usage(사용)을 Factor out(분리)
<ul>
<li>usage이 required(필수적)이면 사용</li>
<li>Base Use Case가 완전하고 usage이 optional일 수 있다면 사용 고려</li>
</ul>
</li>
<li>Use Case diagram은
<ul>
<li>동일한 level of abstraction(추상화 수준)의 Use Case만 포함해야 함</li>
<li>필요한 Actor만 포함해야 함</li>
</ul>
</li>
...
<ul>
<li>Showing the types of objects ...</li>
</ul>
</li>
<li>(다이어그램 구성 요소)</li>
<li>Class Name(클래스 이름)</li>
<li>Class Attributes(클래스 속성)</li>
<li>Class Operations(클래스 오퍼레이션)</li>
<li>Association</li>
<li>Multiplicity(다중성)</li>
<li>Generalization(일반화)</li>
<li>Guard(가드)</li>
<li>&quot;...&quot;는 요소가 더 있을 수 있음을 의미</li>
<li>공백은 &quot;알 수 없음&quot; 또는 &quot;멤버 없음&quot;을 의미</li>
<li>Static structure(정적 구조)에 대한 설명</li>
...
<li>모든 Object-Oriented system의 가장 중요한 building block(구성 요소)</li>
<li>Object set(집합)에 대한 설명</li>
<li>Entity의 추상화</li>
...
<li>Attributes(속성)
<ul>
<li>모델링되는 대상의 property(속성)을 표현</li>
<li>Syntax(구문): attributeName: Type</li>
</ul>
</li>
<li>Operations
<ul>
<li>Class의 object로부터 요청된 service(서비스)의 구현</li>
<li>Syntax: operationName(param1:type, param2:type, ...) : Result</li>
</ul>
</li>
</ul>
...
<li>Association
<ul>
<li>Class instance(인스턴스) 간의 connection(연결)을 명시하는 Class 간의 relationship</li>
<li>Association의 이름은 주로 동사를 씀.</li>
</ul>
</li>
<li>Multiplicity
<ul>
<li>다른 Class의 <em>하나</em>(ONE)의 instance와 관련된 한 Class의 instance 수</li>
</ul>
</li>
...
<li>Aggregation(집합)
<ul>
<li>Element 간의 Weak(약한) &quot;whole-part&quot;(전체-부분) relationship</li>
<li>예: 공항(Airport)은 많은 비행기(Airplane)를 가짐.</li>
</ul>
</li>
<li>Composition(복합)
<ul>
<li>Element 간의 Strong(강한) &quot;whole-part&quot; relationship</li>
<li>예: 창문(Window)은 스크롤바(Scrollbar)를 'contains'.</li>
</ul>
</li>
</ul>
...
<li>Class(클래스) 찾기
<ul>
<li>저장되거나 분석되어야 할 data가 있는가?</li>
<li>External system(외부 시스템)이 있는가?</li>
<li>External system은 Class로 모델링됨.</li>
<li>pattern(패턴), class libraries(클래스 라이브러리), component 등이 있는가?</li>
<li>System이 처리해야 할 device(장치)가 있는가?</li>
</ul>
</li>
<li>가능할 때마다 명시적인 traceability(추적성) 확보
<ul>
<li>Use Case의 noun로부터 Class/Attributes를, verb로부터 Operations를 도출하려 노력</li>
<li>항상 어떤 형태의 behavioral diagrams(행위 다이어그램)과 함께 Class diagram 작성</li>
</ul>
</li>
</ul>
...
<li>time ordering(시간 순서) 강조</li>
<li>(다이어그램 구성 요소)</li>
<li>Lifeline(생명선)</li>
<li>Message name(메시지 이름)</li>
<li>Messages line(메시지 라인)</li>
<li>ReferenceFrame(참조 프레임)</li>
...
<li>Subsystem(서브시스템) / object / Class</li>
<li>Actor</li>
...
<li>Value를 전달하는 parameter를 가질 수 있음.</li>
...
<li>Interaction operators(연산자)는 내용이 behavior를 어떻게 기술하는지 정의</li>
<li>Alt: 각 섹션이 하나의 <strong>alternative</strong>(대안). 예: <code v-pre>alt [a&gt;0]</code></li>
<li>Ref: 다른 Use Case <strong>참조</strong></li>
...
<ul>
<li>예: 하나의 Use Case</li>
</ul>
</li>
<li>Flow(흐름)을 왼쪽에서 오른쪽으로, 위에서 아래로 표현</li>
...
<li>표시 항목
<ul>
<li>한 State에서 다른 state로의 transition를 유발하는 사건</li>
<li>State 변경으로 인한 action
<img src="@source/se/image-11.png" alt="alt text"></li>
</ul>
</li>
</ul>
...
<li>Guard Condition이 true일 때만 발생</li>
...
<li>Event, Guard, activity를 state box 내부에 배치</li>
<li>두 가지 특별한 activity
<ul>
<li>Entry(진입) 및 exit(진출) activity</li>
</ul>
</li>
<li>Internal activity는 self-transition와 유사</li>
<li>그러나 Internal activity는 entry 및 exit activity를 촉발하지 않음.</li>
</ul>
<table>
<thead>
<tr>
...
<tbody>
<tr>
<td><code v-pre>entry/highlight all</code><br><code v-pre>exit/update field</code><br><code v-pre>character/ handle character</code><br><code v-pre>help[verbose]/ open help page</code><br><code v-pre>help[quiet]/ update status bar</code></td>
</tr>
</tbody>
</table>
...
<ul>
<li>Instantaneous(순간적인) behavior</li>
<li>Interrupt(중단)될 수 없음.</li>
</ul>
</li>
<li>Normal state는 조용하며 다음 event를 기다린 후 무언가를 수행</li>
<li>Do-activities(수행-활동)
<ul>
<li>Finite time(유한 시간) 소요</li>
<li>Interrupt될 수 있음. (멈출 수 있음.)</li>
</ul>
</li>
...
<li>공유된 behavior를 superstate로 이동</li>
...
<li>System topology의 물리적인 설명</li>
<li>Hardware unit의 구조와 각 유닛에서 실행되는 소프트웨어 기술</li>
</ul>
<p><img src="@source/se/image-16.png" alt="alt text"></p>
<ul>
<li>Node
<ul>
<li>Artifact가 실행을 위해 배포될 수 있는 Computational resource(컴퓨팅 자원)</li>
</ul>
</li>
<li>Communication path
<ul>
<li>Node 간의 connection 표시</li>
<li>Stereotype은 통신 protocol이나 사용되는 network에 사용 가능</li>
</ul>
</li>
<li>Artifact
<ul>
<li>Software development process 또는 system의 배포 및 운영에 의해 사용되거나 생성되는 물리적 piece of information의 Specification(명세, 사양)</li>
<li>예: 모델 파일, 소스 파일, 스크립트, 바이너리 실행 파일, 데이터베이스 시스템의 table, 개발 결과물, 워드 프로세싱 문서, 메일 message</li>
</ul>
</li>
</ul>
</div></template>
