# 17. Neural Networks and Deep Learning (3)
...
- Regression
  ...
  - Loss (error) function: A sum-of-squares (L2 or squared) error function
  - Training set: Input vectors $\{\mathbf{x}_n\}$, target vectors $\{\mathbf{t}_n\}$. Error function $E(\mathbf{w})$ 최소화
  - $E(\mathbf{w}) = \frac{1}{2} \sum_{n=1}^{N} \|\mathbf{y}(\mathbf{x}_n,~\mathbf{w}) - \mathbf{t}_n\|^2$
- Binary classification
  - Single target variable (or label) $t$: $t = 1$ (class $C_1$), $t = 0$ (class $C_2$)
  - Output activation function: Sigmoid $y = \sigma(a)$
  - Loss (error) function: Negative log likelihood or cross-entropy error function
  - $E(\mathbf{w}) = - \sum_{n=1}^{N} \{t_n \log y_n + (1 - t_n) \log(1 - y_n)\}$
  - $y_n$은 $y(\mathbf{x}_n,~\mathbf{w})$를 의미
- Multiclass classification
  - Input을 $K$개의 상호 배타적인 class 중 하나에 할당
  - Binary target variables $t_k \in \{0,~1\}$: 1-of-$K$ coding scheme
  - Network outputs: $y_k(\mathbf{x},~\mathbf{w}) = P(t_k = 1|\mathbf{x})$로 해석
  - Output activation function: Softmax $y_k = \exp(a_k) / (\sum_j \exp(a_j))$
  - Loss (error) function: Negative log likelihood or cross-entropy error function
  - $E(\mathbf{w}) = - \sum_{n=1}^{N} \sum_{k=1}^{K} t_{nk} \log y_k(\mathbf{x}_n,~\mathbf{w})$
- Next slide
  - Maximum likelihood estimation과 cross-entropy 간의 관계
- Summary
  - 해결하려는 problem type에 따라 output unit activation function과 matching error function의 자연스러운 선택 존재
...
- Entropy
  ...
  - $H(X) = -\mathbb{E}_{X \sim P}[\log P(x)] = \mathbb{E}_{X \sim P}[\log \frac{1}{P(x)}]$
- Kullback-Leibler (KL) Divergence
  - 동일한 random variable $X$에 대한 두 확률 분포 $P(X)$와 $Q(X)$의 차이 측정
  - $D_{KL}(P \| Q) = \mathbb{E}_{X \sim P(X)}[\log \frac{P(x)}{Q(x)}] = \mathbb{E}_{X \sim P}[\log P(x) - \log Q(x)]$
- Cross-entropy
  - $H(P,~Q) = -\mathbb{E}_{X \sim P}[\log Q(x)] = H(X) + D_{KL}(P \| Q)$
  - $Q$에 대한 cross-entropy 최소화는 KL divergence 최소화와 동일
...
- Maximum Likelihood Estimation (MLE)
  ...
  - MLE principle: $\mathbf{w}_{\text{MLE}} = \arg\max_{\mathbf{w}} P_{\text{model}}(\mathbf{x};~\mathbf{w})$
...
- Connection between cross-entropy and MLE
  ...
  - Scaling (Argmax 불변): $\mathbf{w}_{\text{MLE}} = \arg\max_{\mathbf{w}} \frac{1}{N} \sum_{n=1}^{N} \log P_{\text{model}}(\mathbf{x}_n;~\mathbf{w}) = \arg\max_{\mathbf{w}} \mathbb{E}_{\mathbf{x} \sim P_{\text{empirical}}(\mathbf{x})}[\log P_{\text{model}}(\mathbf{x};~\mathbf{w})]$
  - $P(\mathbf{x}) = P_{\text{empirical}}(\mathbf{x})$, $Q(\mathbf{x}) = P_{\text{model}}(\mathbf{x};~\mathbf{w})$로 치환
  - $\mathbf{w}_{\text{MLE}} = \arg\max_{\mathbf{w}} \mathbb{E}_{P(\mathbf{x})}[\log Q(\mathbf{x})] = \arg\min_{\mathbf{w}} - \mathbb{E}_{P(\mathbf{x})}[\log Q(\mathbf{x})] = \arg\min_{\mathbf{w}} H(P, Q)$
- In conclusion,
...
- Process:
  1. Weight space에서 임의의 시작점 선택
  2. Gradient 추정치 계산 후 가장 가파른 내리막 방향으로 조금 이동
  3. (Local) minimum loss 지점에 수렴할 때까지 반복
- The algorithm is as follows:
  - $\mathbf{w} \leftarrow$ any point in the parameter space
  - while not converged do
  - $\quad \mathbf{w}^{(\tau+1)} \leftarrow \mathbf{w}^{(\tau)} - \alpha \nabla E(\mathbf{w})$ ($E(\mathbf{w}) = \sum_{n=1}^{N} E_n(\mathbf{w})$)
- Parameter $\alpha$: `learning rate` 또는 `step size`
  - 고정 상수이거나, learning process 진행에 따라 decay 가능
...
- In a general feed-forward network,
  - 각 unit은 weighted sum 계산: $a_j = \sum_i w_{ji} z_i$
  - $z_i$: Unit $j$로 connection을 보내는 unit/input의 activation
  - $w_{ji}$: Connection과 연관된 weight
  - $a_j$는 nonlinear activation function $h(\cdot)$에 의해 변환되어 unit $j$의 activation $z_j$ 생성: $z_j = h(a_j)$
...
- (Slides 20-33: `Backpropagation: a simple example`)
