...
2. Key와 Query 간의 쌍별(pairwise) 유사도 계산 및 $\text{softmax}$로 정규화
  $$e_{ij} = q_i^T k_j$$
...
- **Learned absolute position representations**(학습된 절대 위치 표현)
 - 모든 $p_i$를 학습 가능한 파라미터로 설정
 - 행렬 $P \in \mathbb{R}^{d \times n}$을 학습하고 각 $p_i$를 해당 행렬의 열로 사용
 - 대부분의 시스템에서 이 방식을 사용
 - 장점: **유연성**(Flexibility)
  - 각 위치가 데이터에 적합하도록 학습됨.
 - 단점: $1, \dots, n$ 의 **범위를 벗어난** 인덱스로는 절대 외삽 불가능
...
$$m_i = MLP(\text{\text{output}}_i)$$
$$= W_2 \cdot \text{ReLU}(W_1 \cdot \text{\text{output}}_i + b_1) + b_2$$

## Masking the Future in Self-Attention
- Decoder에서 Self-Attention을 사용하려면 미래를 볼 수 없도록 해야 함.
 - 매 timestep마다 Key와 Query 집합을 과거 단어만 포함하도록 변경하는 것도 가능
 - Parallelization(병렬화)를 가능하게 하기 위해, 미래 단어에 대한 Attention 점수를 $-\infty$로 설정하여 마스킹 처리
...
- 오직 Attention 메커니즘으로만 설계된 신경망 아키텍처 (CNN이나 RNN 없이).
 ...
 - Self-Attention 메커니즘이 위치
 - 표현의 병렬 계산 가능 $\Rightarrow$ Scalability(확장성) $\uparrow$
 - 본래 Machine Translation(Seq2Seq 아키텍처)을 위해 제안됨.
![](image-111.png)

## The Transformer Decoder
- **Transformer Decoder**는 **Language Model**과 같은 시스템을 구축하는 방식
 - 최소한의 Self-Attention 아키텍처와 유사하지만 **몇 가지 구성 요소**가 추가됨.
...
- 행렬을 통한 Key-Query-Value Attention 계산 방식
 - $X = [x_1; \dots; x_n] \in \mathbb{R}^{n \times d}$를 입력 벡터의 concatenation으로 정의
$$XK \in \mathbb{R}^{n \times d}, XQ \in \mathbb{R}^{n \times d}, XV \in \mathbb{R}^{n \times d}$$
 - 출력 정의
...
- 여러 개의 $Q, K, V$ 행렬을 통해 다수의 Attention "Head"를 정의
 - $Q_l, K_l, V_l \in \mathbb{R}^{d \times \frac{d}{h}}$ ($h$는 Attention Head의 수, $l$은 1부터 $h$까지).
 - 각 Attention Head는 독립적으로 Attention 수행
$$\text{output}_l = \text{softmax}(XQ_l K_l^T X^T) - XV_l$$
 - 여기서 $\text{output}_l \in \mathbb{R}^{d/h}$.
 - 그 후 모든 head의 출력을 결합!
$$\text{output} = [\text{output}_1; \dots; \text{output}_h]Y$$
 - 여기서 $Y \in \mathbb{R}^{d \times d}$.
...
- $h$개의 Attention head를 계산하더라도 비용이 크게 증가하지 않음.
 - $XQ \in \mathbb{R}^{n \times d}$를 계산한 후 $\mathbb{R}^{n \times h \times \frac{d}{h}}$로 reshape함 ($XK$, $XV$도 동일).
 - 그 후 $\mathbb{R}^{h \times n \times \frac{d}{h}}$로 transpose.
...
- "Scaled Dot Product" Attention은 학습을 도움.
 ...
 - 이로 인해 $\text{softmax}$ 함수의 입력값이 커져 기울기(Gradient)가 작아짐.
- 해결책
$$\text{output}_l = \text{softmax}(XQ_l K_l^T X^T) - XV_l$$
 - 기존에 본 Self-Attention 함수 대신, Attention 점수를 $\sqrt{d/h}$로 나누어 차원 수($d/h$)에 따라 점수가 커지는 것을 방지
...
- Residual connections는 모델 학습을 돕는 trick
 - $X^{(i)} = \text{Layer}(X^{(i-1)})$ ($i$는 레이어) 대신,
 - $X^{(i)} = X^{(i-1)} + \text{Layer}(X^{(i-1)})$ 로 설정 (이전 레이어로부터의 "잔차"만 학습하면 됨)
 ![](image-98.png)
...
- Decoder에서는 이전에 본 것과 더 유사한 형태의 Attention을 가짐.
 - $h_1, \dots, h_n$을 Transformer Encoder의 출력 벡터라 함 ($h_i \in \mathbb{R}^d$).
 - $z_1, \dots, z_n$을 Transformer Decoder의 입력 벡터라 함 ($z_i \in \mathbb{R}^d$).
