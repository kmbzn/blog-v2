...
- 여러 전략을 결합하는 defense-in-depth tactic 채택
  - String handling에 대한 secure technique + 하나 이상의 runtime detection
- String handling
...
- 첫 번째 메모리 관리 model (caller allocates, caller frees)
- `C11` Annex K
  - 대안적인 라이브러리 함수 제공
  ...
  - ISO/IEC TR 24731-1로 게시 $\to$ 나중에 `C11`에 optional extensions 세트로 통합
...
- Original constraint handler
  - 라이브러리 함수가 constraint handler를 설정하는 경우, 함수는 반환 또는 종료 전에 original constraint handler를 복원해야 함

## Dynamic Allocation Functions
- 두 번째 메모리 관리 model (callee allocates, caller frees)
  - `ISO/IEC TR 24731-2`
- 이러한 함수 사용은 나중에 buffers를 free하기 위한 추가 호출이 필요
- `ISO/IEC TR 24731-2`에 설명된 함수는 버퍼가 항상 필요한 데이터를 담을 수 있도록 자동 크기 조정 $\to$ 더 큰 보장 제공
...
- `basic_string class`
  ...
  - "callee allocates, callee frees" 메모리 관리 전략 $\to$ 가장 안전한 접근 방식
- Example
...
## `strncpy_s()`
- Example
  - `r1 = strncpy_s(dst1, sizeof(dst1), src1, sizeof(src1));` $\to$ `r1 = 0, dst1 = “hello\0”`
  - `r2 = strncpy_s(dst2, sizeof(dst2), src2, 4);` $\to$ `r2 = 0, dst2 = “good\0”`
  - `r3 = strncpy_s(dst3, sizeof(dst3), src1, sizeof(src1));` $\to$ `r3 = nonzero value, dst3[0] = ‘\0’`

## Summary of `strcpy()` and `strcat()`
- 요약 tables

## Runtime Protection Strategies
- Programmer가 불완전한 detection and recovery strategy를 사용하여 문제를 해결했다고 믿을 위험
