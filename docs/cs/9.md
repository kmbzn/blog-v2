...
## Operating System
- 효율성을 위해 필요한 multiprogramming
  - 단일 사용자는 CPU와 I/O 장치를 항상 바쁘게 유지할 수 없음.
  - Multiprogramming은 CPU가 항상 실행할 작업을 갖도록 작업(코드 및 데이터)을 구성함
...
- Dual-mode operation은 OS가 자신과 다른 시스템 구성 요소를 보호할 수 있게 함
...
| 32 bit | 16 bit | 8 bit (high) | 8 bit (low) | 용도 (Use)
| - | - | - | - | -
...
- Exploit 예
  - $“A” \times 80 + “B” \times 4 + “\backslash xe0\backslash x8a\backslash x05\backslash x40” + “AAAA” + “\backslash xf9\backslash xbf\backslash x0f\backslash x40”$
  - $0x40058ae0$
  - $0x400fbff9$
...
- 악성 코드를 주입할 필요 없이 임의의(튜링 완전한) 계산 수행
  ...
  - 원본 코드 수정 필요 없음.
  - $\rightarrow$ 하지만 여전히 stack 내용(반환 주소 포함)을 변경해야 함
...
- 접근 방식
  - 전체 함수를 사용하는 대신 작은 명령어 시퀀스(예: libc의)를 사용함
...
- 1) Gadget 찾기 $\rightarrow$ BoF 공격 $\rightarrow$ Return $\rightarrow$ Sequence 1 시작
  - 입력 값: Pattern 1 + Pattern 2 + Ret_addr_1 + “\x8D\xAB\x10\x80” + Ret_addr_2

## Gadget Example: Memory Load (2/4)
- 2) $0x8010AB8D$를 `%eax` register에 pop
- 목표: ($0x8010ABCD$가 가리키는) 단어 $0xDEADBEEF$를 `%eax` register에 로드하기

## Gadget Example: Memory Load (3/4)
- 3) 제어 흐름은 Sequence 1의 `ret` 명령어에 의해 Sequence 2로 이동함
- 목표: ($0x8010ABCD$가 가리키는) 단어 $0xDEADBEEF$를 `%eax` register에 로드하기

## Gadget Example: Memory Load (4/4)
- 4) $0xDEADBEEF$를 `%eax` register로 move
- 목표: ($0x8010ABCD$가 가리키는) 단어 $0xDEADBEEF$를 `%eax` register에 로드하기
