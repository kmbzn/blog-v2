...
본 보고서에서는 국내 중소기업(SyncView)에서 판매된 IP 카메라 모델의 공식 업데이트 파일인
`74.2.64.31-libPPPP_API_20160721.bin`을 분석 대상으로 선정하였다.
해당 파일은 제작사 자료실/다운로드/프로그램 페이지에서 다운로드하였으며,  
...
부팅 과정에서 네트워크 설정, 프로세스 시작, 장치 초기화 등을 담당했을 가능성이 있다.
...
패스워드 해시(`vRNT.4lCplkng`)는 특정 해시 알고리즘(주로 DES/MD5 기반)으로 암호화된 값이다.  
여기서 앞의 두 글자(`vR`)은 salt값에 해당한다.  
단방향 알고리즘에 의해 해시된 값이기 때문에 복호화하여 원래 passwd를 얻어내기에는 어려움이 있었다.

## 4.2 외부 서버로 username/password를 평문 전송하는 코드 발견
...
username/password가 `HTTP` `GET` 요청에 직접 포함되어 있어, 암호화되지 않은 채 외부로 유출된다.
...
- 펌웨어는 `ZIP` 기반으로 패키징되어 있어 압축만으로 file system 추출 가능
- encoder 바이너리 안에 `root` 하드코딩 계정 존재
- username/password를 평문으로 외부 서버로 전송하는 취약 설계 존재
- 웹 UI 기반 admin 계정 흔적 다수
- 종합적으로, 보안 수준이 매우 낮고 **사용자 정보 보호 측면에서도 위험성**이 있음
