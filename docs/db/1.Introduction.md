...
- 데이터베이스
  - High level concept: 관련된 데이터의 집합
  - 주요 목표
    - 어떻게 관련된 데이터를 '논리적으로 조직화'할 것인가?
    - 데이터 간 관계를 유지하면서
    - 데이터 중복을 최소화하면서
- Database Management System(DBMS)
  - High level concept: 관련된 데이터를 제공하는 지능적 서비스
  - 주요 목표
    - 어떻게 관련된 데이터를 '물리적으로 조직화'할 것인가?
...
- 논리 스키마: 데이터베이스의 전반적인 논리적 구조 (디자인)  
...
- DDL 컴파일러는 데이터 사전에 저장된 테이블 템플릿의 집합을 생성함.
...
- 크게 두 종류가 있음
  ...
  - Declarative DML 선언적 DML
    - 해당 데이터들을 어떻게 얻을 것인지에 대한 구체화 없이 무엇을 필요로 하는지 사용자에게 요구
    - 절차적 DML에 비해서 배우고 사용하기 쉬움
...
- 복잡한 함수 연산을 가능케 하기 위해서, SQL은 주로 더 높은 level의 언어에 삽입되곤 한다.

애플리케이션 프로그램은 일반적으로 다음 중 하나의 데이터베이스에 접근한다.
- embedded SQL을 가능하게 하기 위한 언어 확장
- 데이터베이스에 SQL 쿼리를 보낼 수 있게끔 하는 애플리케이션 프로그램 인터페이스(ODBC나 JDBC와 같은)  
...
- 논리적 디자인
  ...
  - 사업적 결정: 데이터베이스에 어떤 속성들을 기록해야 할까?
...
데이터베이스 시스템은 전체 시스템의 각 책임을 처리하는 모듈로 분할됨
...
- 트랜잭션 매니저, 파일 매니저, 버퍼 매니저

스토리지 매니저는 물리적 시스템 구현의 부분으로서 다양한 데이터 구조를 구현해야 한다.
...
- DML 컴파일러: Query language에서 DML 구문을 가져와서 query evaluation 엔진이 이해할 수 있는 low-level 지침에 의해 구성된 evaluation 계획으로 변환
  - DML 컴파일러는 다양한 대안들 중 최소 비용 evaluation 계획을 채택하는 쿼리 최적화를 수행한다.
...
트랜잭션 관리자는 복구 관리자(Recovery Manager)와 병행 제어 관리자(Concurrency Control Manager)로 구성됨
...
- 복구 관리자(Recovery Manager)
  ...
  - 트랜잭션의 원자성(Atomicity)과 지속성(Durability) 속성을 보장합니다.

- 병행 제어 관리자(Concurrency-Control Manager)
