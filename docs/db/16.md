...
2. Selection 연산은 교환 가능(commutative)
...
6. (a) Natural join은 결합 가능(associative)
...
7. Selection 연산은 다음 두 조건 하에 theta join에 분배 가능
  - (a) $\theta_0$의 모든 attribute가 join 되는 표현식 중 하나($E_1$)의 attribute만 포함
    - $\sigma_{\theta_0}(E_1 \bowtie_{\theta} E_2) \equiv (\sigma_{\theta_0}(E_1)) \bowtie_{\theta} E_2$
  - (b) $\theta_1$이 $E_1$의 attribute만, $\theta_2$가 $E_2$의 attribute만 포함
...
8. Projection 연산은 theta join에 다음과 같이 분배 가능
  - (a) $L_1, L_2$가 각각 $E_1, E_2$의 attribute이고 $\theta$가 $L_1 \cup L_2$의 attribute만 포함
...
- Set operations:
  ...
  - 다른 relation에 대한 연산:
    - $r \cup s$ 추정 크기 = $n_r + n_s$: $r \cap s$ 추정 크기 = $\min(n_r,~n_s)$: $r - s$ 추정 크기 = $n_r$
