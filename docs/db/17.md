...
- Schedule a 구성
  ...
  - $T_2$: `read(Y)`, `write(Y)`
  - $T_3$: `read(Z)`, `write(Z)`
  - $T_4$: `read(V)`, `read(U)`, `write(U)`
  - $T_5$: `read(W)`, `read(Y)`, `write(Y)`, `read(Z)`, `write(Z)`, `read(U)`
- precedence graph 분석
- 가능한 serializability order
  - $T_5 \to t_1 \to t_3 \to t_2 \to t_4$
  - $T_5 \to t_1 \to t_2 \to t_3 \to t_4$
  - $T_1 \to t_2 \to t_3 \to t_4 \to t_5$
  - 그 외 등등...
