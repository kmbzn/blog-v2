...
| $\text{read}$($B$)<br>$B:=B-50$<br>write($B$)<br>$\text{unlock}$($B$)
| | $\text{lock-S}$($A$)
| | | $\text{grant-S}$($A,~T_2$)
...
| $\text{lock-X}$($A$)
| | | $\text{grant-X}$($A,~T_1$)
| $\text{read}$($A$)<br>$A:=A+50$<br>write($A$)<br>$\text{unlock}$($A$)
...
| $T_3$ | $T_4$
| - | -
| $\text{lock-X}$($B$)<br>read($B$)<br>$B:=B-50$
| | $\text{lock-S}$($A$)<br>read($A$)<br>$\text{lock-S}$($B$)
| $\text{lock-X}$($A$)
...
- Lock Conversions
  - $T_1$: $\text{read}$($a_1$), $\text{read}$($a_2$), $\dots$, $\text{read}$($a_n$), write($a_1$)
  - $T_2$: $\text{read}$($a_1$), $\text{read}$($a_2$), $\dots$, display($a_1+a_2$)
...
```
if Ti has a lock on D then $\text{read}$(D);
else begin
...
- 간단한 추가 정보: Database 아이템에 접근하는 순서
  - 모든 Data 아이템의 집합 $\mathbf{D} = \{d_{1},~d_{2},~\dots,~d_{h}\}$에 partial ordering (→)를 부과
...
| $T_3$ | $T_4$
| - | -
| $\text{lock-X}($B$)<br>read($B$)<br>$B:=B-50$<br>write($B$)
| | $\text{lock-S}$($A$)<br>read($A$)<br>$\text{lock-S}$($B$)
| $\text{lock-S}$($A$)
...
- Wait-for Graph: transaction을 정점으로 하는 Directed Graph
  - $T_i \to \text{T_j로의 Edge: $T_i$가 $T_j$가 Conflicting 모드로 보유한 Lock을 기다리고 있는 경우
...
- Transaction $T_i$가 $\text{write}(Q)$를 발행한다고 가정
  ...
  3. 그렇지 않으면, Write Operation은 실행되고 W-timestamp}(Q)$는 $\text{TS}(T_i)$로 설정됨.

## Example of Schedule under TSO

- 이 스케줄이 TSO 하에서 유효한가?
  - 초기 가정: $\text{R-TS}(A) = \text{W-TS}(A) = 0$, $\text{R-TS}(B) = \text{W-TS}(B) = 0$
...
| Time | $T_1$ | $T_2$ | $\text{TS}$ | Read Set | Write Set
| - | - | - | - | - | -
| $1$ | $\text{read}$(A)$ | | | $\{A\}$ | $\emptyset$
| $2$ | $\text{read}$(B)$ | | | $\{A, B\}$ | $\emptyset$
| $3$ | | $\text{read}$(B)$ | | $\emptyset$ | $\emptyset$
| $4$ | | $\text{read}$(\text{C})$ | | $\{B, \text{C}\}$ | $\emptyset$
| $5$ | | compute $B=B+1$ | | $\{B, \text{C}\}$ | $\{B\}$
| $6$ | | compute $A=A+1$ | | $\{B, \text{C}\}$ | $\{A, B\}$
| $7$ | | Validation $T_2$ ($\text{TS}=7$) | $7$ |
| $8$ | | $\text{write}(A)$ | |
| $9$ | | $\text{write}(B)$ | |
| $10$ | compute $A=A+1$ | | | $\{A, B\}$ | $\{A\}$
| $11$ | Validation $T_1~(\text{TS}=11$) | | $11$ |
- $T_2$ $(\text{TS}=7) < T_1 ~ (\text{TS}=11)$에 대한 Validation Test
  - FinishTS $(T_2) = 9$. StartTS($T_1$) = 1$
  - FinishTS $(T_2) \nless$ StartTS($T_1$) (즉, $9 \nless 1$)
  - Write Set of $T_2 = \{A, B\}$
  - Read Set of $T_1 = \{A, B\}$
  - Write Set of $T_2 ~\cap~$ Read set of $T_1 = \{A, B\} \ne \emptyset$
- Validation Test 실패 → $T_1$ Abort
