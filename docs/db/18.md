...
- **Tree Protocol**: X-lock만 사용하도록 제한된 간단한 Graph-based Protocol
  - 각 transaction $T_i$는 Data 아이템을 최대 한 번 Lock할 수 있으며, 다음 규칙을 준수해야 함.
    ...
    - 그 후, Data 아이템 Q는 $Q$의 Parent가 현재 $T_i$에 의해 Lock된 경우에만 $T_i$에 의해 Lock될 수 있음.
...
- Transaction $T_i$의 실행은 세 단계로 수행
  ...
  2. **Validation Phase**: $T_i$는 Serializability를 위반하지 않고 로컬 변수를 Write할 수 있는지 결정하기 위한 **Validation test**(유효성 검증)를 수행. transaction이 Validation test에 실패하면 Abort됨 (Rollback 필요 없음).
