...
- Disk failure: 헤드 크래시나 유사한 디스크 오류로 인해 디스크 저장소의 일부 또는 전체 파괴
  - 파괴는 감지 가능하다고 가정: 디스크 드라이브는 체크섬을 사용하여 실패 감지
...
- 수정 중 하나만 이루어지고 디스크에 출력된 후(즉, $T_i$가 아직 uncommitted 상태) 실패 발생 가능
  - 이러한 실패는 데이터베이스를 비일관적인 상태로 만듦 $\rightarrow$ (uncommitted) $T_i$의 Atomicity가 중요
- 두 가지 모두 수정되고 $T_i$가 commit 되었으나, $A$와 $B$가 디스크에 출력되기 전 실패 발생 가능
  - 이러한 실패는 Lost updates를 초래 $\rightarrow$ (committed) $T_i$의 Durability가 중요
...
## Log-based Recovery
- Log는 데이터베이스의 업데이트 활동 정보를 담은 로그 레코드의 시퀀스
- Log는 Stable storage에 유지
- Transaction $T_i$ 시작 시, $<T_i~ \mathbf{start}>$ 로그 레코드를 작성하여 등록
- $T_i$가 $\text{write}(X)$를 실행하기 전, $<T_i,~ X,~ V_1,~ V_2>$ 로그 레코드 작성
  - $V_1$: $\text{write}$ 전의 $X$ 값(이전 값)
  - $V_2$: $X$에 쓰일 값(새로운 값)
- $\text{write}(X)$ 연산의 실제 실행은 데이터베이스 수정 방식에 따름
- $T_i$가 마지막 구문을 마치면, 로그 레코드 $<T_i~ \mathbf{commit}>$ 작성
- Transaction의 commit 로그 레코드가 Stable storage에 출력되면 해당 Transaction이 commit 되었다고 간주(해당 Transaction의 이전 모든 로그 레코드가 이미 출력되어 있어야 함)
...
- Immediate-modification scheme
  ...
  - 데이터베이스 수정 전에 업데이트 로그 레코드 작성 필수
  - 로그 레코드는 Stable storage로 직접 출력된다고 가정
  - (로그 레코드 출력을 어느 정도 연기하는 방법은 나중에 확인)
...
- Deferred-modification scheme
  ...
  - Recovery의 일부 측면 단순화
  - 하지만 업데이트된 모든 데이터 아이템의 로컬 복사본을 저장해야 하는 overhead 존재
...
- 가정 사항
  ...
  - 그렇지 않다면 복구 시 문제 발생(예: $T_1$이 $A$ 업데이트 $\rightarrow$ $T_2$가 $A$ 업데이트 및 commit $\rightarrow$ $T_1$이 abort 해야 하는 경우 undo 수행 방법? - Recoverability issue)
- 업데이트된 아이템에 대해 X-locks를 획득하고 Transaction 종료 시까지 유지함으로써 보장 가능(Strict two-phase locking)
- 다른 Transactions의 로그 레코드들이 로그 내에 섞여 있을 수 있음

## Undo and Redo Operations
- Transactions의 Undo와 Redo
- $\text{undo}(T_i)$: $T_i$에 의해 업데이트된 모든 데이터 아이템의 값을 이전 값으로 복구($T_i$의 마지막 로그 레코드부터 역방향 진행)
  - 데이터 아이템 $X$가 이전 값 $V$로 복구될 때마다, 특별한 redo-only 로그 레코드 $<T_i,~ X,~ V>$ 작성($V=V_1$은 복구된 이전 값)
  - 이 redo-only 로그 레코드는 undone 된 Transaction을 다시 redo 해야 하는 특수한 경우(다음 슬라이드)에 사용
  - Transaction의 undo가 완료되면 로그 레코드 $<T_i~ \mathbf{abort}>$ 작성
- $\text{redo}(T_i)$: $T_i$에 의해 업데이트된 모든 데이터 아이템의 값을 새로운 값으로 설정($T_i$의 첫 로그 레코드부터 정방향 진행)
  - 이 경우 별도의 로깅 수행 안 함

## Recovering from Failure
- 실패 후 복구 시
  - Transaction $T_i$를 undo 해야 하는 경우 로그 상태
    - 레코드 $<T_i~ \mathbf{start}>$ 포함
    - 하지만 레코드 $<T_i~ \mathbf{commit}>$ 또는 $<T_i~ \mathbf{abort}>$ 미포함
  - Transaction $T_i$를 redo 해야 하는 경우 로그 상태
    - 레코드 $<T_i~ \mathbf{start}>$ 포함
    - 그리고 레코드 $<T_i~ \mathbf{commit}>$ 또는 $<T_i~ \mathbf{abort}>$ 포함
- Transaction $T_i$가 이전에 undo 되었고 $<T_i~ \mathbf{abort}>$ 레코드가 로그에 작성된 후 실패가 발생했다고 가정
- 실패 복구 시, Transaction $T_i$는 redo 됨
  - $T_i$에 의해 생성된 로그 레코드를 redo 하면 각 아이템이 새로운 값으로 설정(old $\rightarrow$ new)
  - 그 후, 이전 undo 작업에 의해 생성된 redo-only 로그 레코드를 redo 하면 모든 업데이트된 아이템이 다시 이전 값으로 설정(new $\rightarrow$ old)
...
| | | $<T_1~\mathbf{commit}>$
- 각 경우의 Recovery 조치
  - (a) $\text{undo}(T_0)$: $B$는 2000으로 복구된 후 $A$는 1000으로 복구, 로그 레코드 $<T_0,~ B,~ 2000>$, $<T_0,~ A,~ 1000>$, $<T_0,~ \mathbf{abort}>$ 작성
  - (b) $\text{redo}(T_0)$ 및 $\text{undo}(T_1)$: $A$와 $B$는 950과 2050으로 설정되고, $C$는 700으로 복구. 로그 레코드 $<T_1,~ C,~ 700>$ 및 $<T_1,~ \mathbf{abort}>$ 작성
  - (c) $\text{redo}(T_0)$ 및 $\text{redo}(T_1)$: $A$와 $B$는 각각 950과 2050으로 설정. 그 후 $C$는 600으로 설정
- Recovery 조치가 취해지는 도중에도 실패 발생 가능
- $\text{redo}()$와 $\text{undo}()$ 연산 모두 Idempotent(멱등성)여야 함

## Checkpoints
- 로그에 기록된 모든 Transactions를 Redo/Undo 하는 것은 매우 느릴 수 있음
  - 시스템이 오랫동안 실행되었다면 전체 로그 처리는 시간 소요 큼
  - 이미 데이터베이스에 업데이트를 출력한 Transactions를 불필요하게 redo 할 수도 있음
- 주기적으로 Checkpointing을 수행하여 Recovery 절차 효율화
  1. 현재 메인 메모리에 있는 모든 로그 레코드를 Stable storage로 출력
  2. 변경된 모든 Buffer blocks를 디스크로 출력
  3. Stable storage에 로그 레코드 $<\mathbf{checkpoint}~ L>$ 기록($L$은 Checkpoint 시점에 활성 상태인 모든 Transactions 목록)
  4. Checkpointing 수행 중에는 모든 업데이트 중지
- Recovery 시, Checkpoint 이전에 완료된 Transactions는 고려할 필요 없음
  - Checkpoint 이전에 commit 되거나 abort 된 Transactions는 Checkpoint 이전 혹은 Checkpointing 과정에서 이미 모든 업데이트가 Stable storage로 출력되었기 때문
- 로그의 끝에서부터 역방향으로 스캔하여 가장 최근의 $<\mathbf{checkpoint}~ L>$ 레코드 탐색
- $L$에 포함되어 있거나 Checkpoint 이후에 시작된 Transactions만 redo 또는 undo 대상
- Undo 작업을 위해 로그의 더 이전 부분이 필요할 수 있음
- $L$에 있는 모든 Transaction $T_i$에 대해 레코드 $<T_i~ \mathbf{start}>$가 발견될 때까지 역방향 스캔 지속
- 위에서 찾은 가장 빠른 $<T_i~ \mathbf{start}>$ 레코드 이전의 로그 부분은 Recovery에 필요하지 않으므로 원할 때 삭제 가능
- 예시
...
- Transaction rollback(정상 작동 중)
  ...
  - 로그를 끝에서부터 역방향 스캔: $<T_i,~ X_j,~ V_1,~ V_2>$ 형태의 각 $T_i$ 로그 레코드에 대해
    - $X_j$에 $V_1$을 써서 undo 수행,
    - 특별한 redo-only 로그 레코드 $<T_i,~ X_j,~ V_1>$ 작성
    - 이러한 로그 레코드는 compensation log records라고도 불림
  - 레코드 $<T_i~ \mathbf{start}>$를 찾으면 스캔을 중지하고 로그 레코드 $<T_i~ \mathbf{abort}>$ 작성
- Recovery from failure: 두 단계(Two phases)
  - Redo phase: commit, abort 여부나 미완료 상태에 상관없이 모든 Transactions의 업데이트 재생(replay)
  - Undo phase: 모든 미완료 Transactions undo
- Redo phase
  1. 마지막 $<\mathbf{checkpoint}~ L>$ 레코드를 찾고 undo-list를 $L$로 설정
  2. $<\mathbf{checkpoint}~ L>$ 레코드 위쪽부터 정방향으로 스캔하며 redo 수행 및 undo-list 구축
    1. 레코드 $<T_i,~ X_j,~ V_1,~ V_2>$ 또는 $<T_i,~ X_j,~ V>$가 발견될 때마다 $X_j$에 $V_2$ 또는 $V$를 써서 redo
    2. 레코드 $<T_i~ \mathbf{start}>$가 발견되면 $T_i$를 undo-list에 추가
    3. 레코드 $<T_i~ \mathbf{commit}>$ 또는 $<T_i~ \mathbf{abort}>$가 발견되면 $T_i$를 undo-list에서 제거
- Undo phase
  - 로그를 끝에서부터 역방향 스캔하며 undo-list에 있는 Transactions 롤백
  1. $T_i$가 undo-list에 있는 상태에서 로그 레코드 $<T_i,~ X_j,~ V_1,~ V_2>$가 발견되면 Transaction rollback과 동일한 작업 수행
    1. $X_j$에 $V_1$을 써서 undo 수행
    2. Compensation log record $<T_i,~ X_j,~ V_1>$ 작성
  2. $T_i$가 undo-list에 있는 상태에서 로그 레코드 $<T_i~ \mathbf{start}>$가 발견되면,
    1. 로그 레코드 $<T_i~ \mathbf{abort}>$ 작성
    2. $T_i$를 undo-list에서 제거
  3. Undo-list가 비게 되면 중지
- Undo phase가 완료된 후 정상적인 Transaction 처리 시작 가능

## Buffer Management
- Log record buffering: 로그 레코드는 Stable storage로 직접 출력되는 대신 메인 메모리에 버퍼링 됨
- 버퍼 내 로그 레코드 블록이 꽉 차거나 Log force 연산이 실행될 때 로그 레코드가 Stable storage로 출력
- Log force는 Transaction을 commit 하기 위해 모든 로그 레코드(commit 레코드 포함)를 Stable storage로 강제 출력하는 작업
- 여러 로그 레코드를 단일 출력 연산으로 내보낼 수 있어 I/O 비용 감소
- 로그 레코드가 버퍼링 될 경우 준수해야 할 규칙
  - 로그 레코드는 생성된 순서대로 Stable storage에 출력
  - 참고: 데이터 블록(데이터베이스 버퍼) 출력은 순서가 바뀔 수 있음
  - Transaction $T_i$는 로그 레코드 $<T_i~ \mathbf{commit}>$이 Stable storage에 출력되었을 때만 commit 상태로 진입
  - 메인 메모리의 데이터 블록이 데이터베이스로 출력되기 전, 해당 블록의 데이터와 관련된 모든 로그 레코드가 반드시 Stable storage에 출력되어야 함
...
- Recovery 알고리즘은 no-force policy 지원
  ...
  - Force policy: commit 시 업데이트된 블록 기록 요구
  - 더 비싼 commit 비용
- Recovery 알고리즘은 steal policy 지원
  - Uncommitted Transactions의 업데이트를 포함한 블록도 Transaction commit 전에 디스크에 기록 가능
  - Uncommitted 업데이트가 있는 블록이 디스크로 출력될 경우, 해당 업데이트의 undo 정보를 담은 로그 레코드가 먼저 Stable storage의 로그에 출력되어야 함
  - Write ahead logging
- 블록이 디스크로 출력될 때 해당 블록에 대한 업데이트가 진행 중이어서는 안 됨
- 다음과 같이 보장 가능
  - 데이터 아이템을 쓰기 전, Transaction은 해당 데이터 아이템을 포함하는 블록에 대해 X-lock 획득
  - 쓰기가 완료되면 락 해제 가능
  - 이렇게 짧은 기간 동안 유지되는 락을 Latches라고 지칭
- 블록을 디스크로 출력하는 과정
  1. 블록에 업데이트가 진행 중이지 않음을 보장하기 위해 블록에 대한 Exclusive latch를 먼저 획득
  2. **Log flush** 수행(Write-Ahead Logging을 위함)
  3. 블록을 디스크로 출력
