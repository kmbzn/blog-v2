...
## 1. Design
- 본 과제의 목표는 $B^+$-tree index 구조로 저장된 두 개의 데이터 테이블(Table 1, Table 2)에 대하여 Natural join 연산을 수행하는 알고리즘을 구현하는 것입니다. 각 테이블의 레코드는 `<Key, Value>` pair로 구성되며, Key는 8-byte의 정수(`int64_t`), Value는 최대 120-byte의 문자열으로 주어졌습니다. 명세에 따라 Key는 중복되지 않는 Unique key로 가정합니다.
...
- $B^+$-tree의 leaf node들은 Key 순서대로 정렬되어 있으며, linked list 형태로 서로 연결되어 있습니다. 이는 별도의 추가적인 정렬 알고리즘을 도입하지 않아도 이미 데이터가 sorted state라는 것을 의미합니다.
- 따라서 최종적으로 다음과 같은 최적화된 설계를 도출할 수 있었습니다.
...
#### 1.3.1. 결론
- 이 설계를 통해 별도의 정렬 비용 없이 $O(N + M)$의 시간복잡도로 효율적인 natural join을 수행할 수 있으로 기대합니다.
- 또한, 한 번에 필요한 leaf page만 메모리에 적재하게 되므로 `4 MiB` 메모리 제약 조건을 충분히 준수할 수 있습니다.
...
## 3. Result
- 구현된 join 기능의 정확성 및 성능을 검증하기 위해 수행한 테스트에 대해서 다루고자 합니다.
...
- 터미널 출력 결과를 확인한 결과, key값이 일치하는 튜플들에 대해서만 정확하게 `Key, Value1, Value2` 형식이 출력됨을 확인하였습니다.
- $O(N+M)$ 알고리즘이 적용되어, 약 1만 건 이상의 데이터에 대해서도 큰 지연 없이(대략 0.1초 이내에) 즉각적인 결과가 도출됨을 확인하였습니다.

![](image-38.png)

## 4. Troubleshooting
- 구현 과정에서 중대한 어려움은 없었습니다.
- 하지만 test 과정에서 약간의 착오가 발생한 부분을 기술하고자 합니다.
...
2. 원인을 분석하던 도중 macOS의 일부 메모리 모니터링 도구가 메모리 단위를 `KB`가 아닌 `Bytes` 단위로, 혹은 Page 단위로 다르게 표시할 수 있다는 점을 이해하였습니다.
3. 단위를 보정하여 다시 계산해본 결과, 실제 사용량은 허용 범위인 `4 MiB` 이내임을 확인하였습니다.
4. 또한, `db_join` 함수 내에서 `malloc`으로 할당한 `page` 구조체들이 loop 종료 후 또는 함수 return 전에 올바르게 `free` 되는지 코드를 재검토하여 안전성을 확보했습니다.

#### 4.1.3. 결론
- 단순한 메모리 수치 해석의 오류였으며, 실제 구현된 $B^+$-tree 기반의 join 알고리즘은 필요한 페이지만을 메모리에 적재하게 되므로 메모리 제약 조건을 충분히 만족하고 있음을 확인하였습니다.