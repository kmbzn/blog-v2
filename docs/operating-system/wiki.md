# xv6 RISC-V Kernel-Level Threads Implementation: wiki
...
이번 project는 xv6의 kernel과 사용자 영역 양쪽에 걸쳐 **thread 시스템 전반을 아우르는 구현 변경**을 필요로 한다. 구현은 크게 다음 4가지 레이어로 나뉜다:
...
#### [TEST#1] Basic Thread Creation and Exit Order
...
이와 같은 반복적 디버깅과 코드 개선을 통해, xv6에 안정적이고 정확한 kernel 수준 thread 시스템을 구현할 수 있었다.
