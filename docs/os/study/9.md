...
3. 세그먼트 테이블에서:
  - 페이지 사상표(page table)의 시작 주소 $s\prime$를 얻음
4. (s, p) 쌍이 TLB(연관 저장 장치)에 있는지 먼저 확인
  - 있으면 TLB에 저장된 $p\prime$ 값으로 바로 매핑
  - 없으면:
   - 페이지 사상표에서 $p$를 인덱스로 사용하여 페이지 프레임 번호 $p\prime$를 조회
   - TLB에 새 항목으로 저장 가능
5. 페이지 프레임 번호 $p\prime$와 변위 d를 더해 실제 주소 r을 계산
  $$ r = p\prime + d $$
...
TLB에서 (s, p) 조회
  ...
  └─ 없으면
    └─ 세그먼트 테이블 시작 주소 b + s 접근
      └─ 세그먼트 s의 page table 주소 $s\prime$ 획득
        └─ page table에서 p 인덱스로 $p\prime$ 조회
          └─ TLB에 (s, p) → $p\prime$ 저장
...
## 해결 시도
- Hashing 기반 탐색 구조 사용
- Chain이 없는 hash function 설계가 # Segmentation
