...
본 project의 목표는 xv6 RISC-V 기반 운영체제에 **kernel-level thread** (커널 수준 thread) 기능을 추가하는 것이다. xv6는 MIT에서 교육 목적으로 설계한 간결한 UNIX 기반 OS로, 다중 프로세스(multiprocessing)는 지원하지만 다중 스레드(multithreading)는 지원하지 않는다. 본 project는 **기존 xv6의 process 관리 구조를 최대한 활용하면서**, thread의 실행 컨텍스트(context), 자원 공유 방식, 동기화 시나리오 등을 구현하는 데 중점을 둔다.
...
각 영역에 대해 세부 구현과 변경 포인트를 순서대로 설명할 것이다.
...
- 모든 test의 기능은 정확히 구현되었으며, trap 발생 또한 메모리 보호 기법이 올바르게 작동했음을 의미한다.
...
이상으로 본 project의 기술적 배경과 연관된 모든 요소들을 포함하였다.