...
- 입력 → 함수 → 출력의 관계로, 상태 변경 없이 결과를 계산하는 것이 - Solid theoretical basis (oriented more to particular programming paradigms or methodologies than to efficiency)
...
- LISP는 람다 표현식, 재귀, 함수 자체를 값처럼 다루는 방식 등 함수형 프로그래밍의 개념을 담고 있음
...
- Lisp의 철학 중 하나인 코드와 데이터의 동일한 표현 (Code-as-Data)
