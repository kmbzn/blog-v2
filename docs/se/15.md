...
## Code Review Goals
- Finding defects
    - Low-level 및 High-level issue 모두 포함 (Requirements/Design/Code)
- Code improvement
...
- 보통 4-5명 (최소 3명)
- Author: 작성자
- Inspector(s): 결함(Faults) 및 광범위한 issue 발견
...
- 참여자
    - Author
    - Moderator
    - Inspectors (Scribe, Reader, Verifier 포함)
...
- Meeting (1시간)
    - Reader가 Code 제시
    - 모든 Reviewers가 issue 식별
    - 회의에서는 issue 발견만 하고, 솔루션 토의나 실제 issue 여부는 논쟁하지 않음
- Rework
...
- Authors는 Code를 설명하거나 방어하지 않음 (객관적이지 않음)
    - Author $\neq$ Moderator $\neq$  Scribe, $\neq$  Reader
    - Author는 질문과 오해를 관찰하고 필요시 issue를 명확히 하기 위해 회의 참석
- Reader(optional)는 Code를 한 줄씩 읽으며 설명
...
- 결함을 식별하되 대안 제시는 지양, Authors를 비판하지 않음
    - "너는 변수 `a`를 초기화하지 않았어" $\rightarrow$ "나는 변수 `a`가 어디서 초기화되는지 못 찾겠어"
...
- 대부분의 issue는 Meeting이 아닌 Preparation 단계에서 발견됨
- 제안된 시너지 효과는 낮은 영향만 미침
- 주장: Meetings에서 발견되는 결함은 종종 더 미묘함(Subtle)

## False Positives
- 발견된 issue의 약 25%는 False positives
- Meeting 중 토론 회피
- Meeting 중 혼란은 문서가 더 명확해야 한다는 지표(Indicator)

## Self-checks Can Find Half the Issues
![alt text](image-71.png)
- Authors가 Inspection 전 문서를 Self-check함

## The Goal Is Not To Be “Right” (it’s to save $)
> *"피로스의 승리(Pyrrhic victory)는 승리자에게 막대한 대가를 치르게 하여 패배와 다름없는 승리이다.  
> 장기적 발전을 저해하거나 진정한 성취감을 무효화한다."*
