...
- 정적 분석이 왜 Sound, Complete, Terminating할 수 없는지에 대한 고차원적 설명 및 분석 설계의 트레이드오프 평가
...
- Error 존재 여부와 보고 여부에 따른 분류
  - Error 존재함 + Error 보고됨: True positive (올바른 분석 결과)
  - Error 존재 안함 + Error 보고됨: False positive
  - Error 존재함 + Error 보고 안됨: False negative
  - Error 존재 안함 + Error 보고 안됨: True negative (올바른 분석 결과)
...
- `while (rv > 0)` 루프가 포함된 변형된 `foo()` 함수 예제 및 흐름도
...
- Abstraction
  ...
  - Interrupt가 비활성화된 상태로 함수 끝에 도달할 수 있는지 경고
- Transfer function
  - Basic block에 `cli()` 호출이 포함된 경우, 분석 상태를 Enabled에서 Disabled로 변경
  - Basic block에 `restore_flags()` 호출이 포함된 경우, 분석 상태를 Disabled에서 Enabled로 변경
...
- `// do_stuff`, `restore_flags();` 수행
- 블록 후 프로그램 지점: Interrupt enabled
...
- $y → MZ$ 등 상태 업데이트
- 3차 반복(Iteration)
- 이전 상태와 비교하여 변경 사항 없음 (Fixed point 도달)
- 경고: 가능한 0으로 나누기(Div-by-zero) 오류

## Abstraction at Work
- 가능한 상태의 수는 거대함
  - $n$개의 32비트 변수는 $2^{32*n}$개의 상태
...
- `int foo()` 함수 내 `Integer` 객체 생성 및 null 체크 로직 예제 코드
...
- Control-flow 분석은 모든 가능한 경로 분석 (전역 속성)
