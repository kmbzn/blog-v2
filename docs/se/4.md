...
## Capability Maturity Model(CMM)
- Common features of key practices(프랙티스의 공통 특징)
  - 모든 key process area(프로세스 영역)의 key practices(프랙티스)는 common features(공통 특징)라 불리는 5가지 categories(범주)로 구성됨
  - Common features(공통 특징)는 Key Process Area(KPA)의 implementation(구현)이 effective(효과적), repeatable(반복 가능), lasting(지속적)인지를 나타내는 key practices(프랙티스)의 attributes(속성)임
...
- Application of the CMM(CMM의 적용)
  - 조직이 특정 maturity(성숙도) level(수준)에 도달하기 위해서는, 해당 level(수준) 및 모든 이전 levels(수준)의 모든 KPAs(프로세스 영역)의 모든 goals(목표)가 충족되어야 함
