...
declare const FadeInExpandTransition: import("vue").DefineComponent<import("vue").ExtractPropTypes<{
...
/**
 * Determines whether the user is currently focusing a text control. In this
 * case, the search plugin shouldn't hijack any hotkeys because the user might
 * be typing into a text field, using type-ahead search in a `select` element,
 * etc.
 *
 * 判断用户当前是否正在聚焦文本控件。 在这种情况下，搜索插件不应该劫持任何热键，因为 用户可能正在文本字段中输入，在 `select`
 * 元素中使用预输入搜索等。
 *
 ...
 * 检查插槽当前是否为空
 *
 * @param normalizedSlotContent - The normalized slot content, which should be
 *   the results of the slot function / 标准化的插槽内容，应该是插槽函数的结果
 * @returns True if the slot content is empty, false otherwise / 如果插槽内容为空则返回
 *   true，否则返回 false
 */
...
//#endregion
export { DarkModeRef, type DeepRequired, type ExactLocaleConfig, FadeInExpandTransition, type GetHeadersOptions, HeaderItem, type HeaderLevels, HeadersRef, type KeyOptions, type LiteralUnion, LoadingIcon, LoadingIconProps, Message, RenderDefault, type RequiredSlot, type RequiredSlotContent, type SingleSlot, type SingleSlotContent, type Slot, type SlotContent, type UnionToIntersection, darkModeSymbol, dateSorter, decodeData, deepAssign, encodeData, encodeSVG, endsWith, ensureEndingSlash, ensureLeadingSlash, entries, fromEntries, getDarkMode, getDate, getHeaders, getHeadersFromDom, hasGlobalComponent, isArray, isBoolean, isDef, isFocusingTextControl, isFunction, isIOS, isKeyMatched, isLinkAbsolute, isLinkExternal, isLinkHttp, isLinkRelative, isLinkWithProtocol, isMacOS, isMobile, isNumber, isPlainObject, isRegExp, isSafari, isSlotContentEmpty, isString, isWindows, isiPad, isiPhone, keys, normalizePath, removeEndingSlash, removeLeadingSlash, resolveHeaders, startsWith, useDarkMode, useHeaders, useKeys, useLocale, useLocaleConfig, useRoutePaths, values, wait };
