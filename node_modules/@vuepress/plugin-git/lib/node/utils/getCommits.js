...
const getCoAuthorsFromCommitBody = (body) => body
    ? Array.from(body.matchAll(RE_CO_AUTHOR)).map(([, name, email]) => ({
        name: name.trim(),
        email: email.trim(),
    }))
    : [];
const getGitLogFormat = ({ contributors, changelog, }) => {
 ...
 */
const runGitLog = (args, cwd) => new Promise((resolve, reject) => {
    ...
    gitProcess.on('close', (code) => {
        if (code === 0) {
            ...
            reject(new Error(`'git log' failed with exit code ${code}: ${stderrData.trim()}`));
        }
    });
});
/**
...
export const getRawCommits = async (filepath, cwd, options) => {
    ...
    try {
        ...
        return stdout
            ...
            .filter(Boolean)
            .map((rawString) => {
            const [hash, time, author = '', email = '', body = '', message = '', refs = '',] = rawString.split(INFO_SPLITTER).map((v) => v.trim());
            return {
                filepath,
                hash,
...
export const mergeRawCommits = (commits) => {
    ...
    commits.forEach(({ filepath, ...commit }) => {
        ...
        if (_commit)
            ...
            commitMap.set(commit.hash, { ...commit, filepaths: [filepath] });
    });
    const result = Array.from(commitMap.values());
    return result;
};
export const getCommits = async (filepaths, cwd, options) => {
    const rawCommits = (await Promise.all(filepaths.map((filepath) => getRawCommits(filepath, cwd, options)))).flat();
